...
- **Application Layer**: Orchestrates workflows, manages commands, and coordinates agents
...
- **MemoryPort**: Unified interface for memory operations, supporting ChromaDB, SQLite, and JSON backends
- **ChromaDBMemoryStore**: Vector database for semantic search and scalable artifact storage
- **SQLiteMemoryStore**: Structured data storage for project metadata
...
- **Automated Knowledge Base**: Documentation and code are indexed for semantic search and agentic retrieval (see [Comprehensive Enhancement Plan](../../Comprehensive%20DevSynth%20Enhancement%20Plan.md)).
...
- **SDLC Policy Corpus** ([policies/README.md](../policies/README.md)): Comprehensive SDLC, security, ethics, and testing policies for both human and agentic contributors. See also [SDLC Policies and Repository Artifacts for Agentic LLM Projects](../../scratch_2.md) for rationale and best practices.
...
- **Repository Map & Index**: A [RepoStructure.md](../RepoStructure.md) (planned) and autogenerated indices provide a navigable map for both human and agentic contributors.
...
A full [documentation restructuring plan](../roadmap/documentation_plan.md) and [RepoStructure.md](../RepoStructure.md) (planned) provide a map for both human and agentic contributors.
