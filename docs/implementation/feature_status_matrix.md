...
| Test Marker Verification | Complete | scripts/verify_test_markers.py, tests/ | 2 | 3 | Testing Infrastructure | | Enforce single speed marker per test. Behavior scenario modules now carry explicit `@pytest.mark.fast` coverage, eliminating collection-time warnings and restoring full compliance. |
