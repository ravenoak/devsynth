...
31.9 [ ] Update WebUI/UXBridge scenario loaders to point at existing `.feature` files, regenerate traceability manifests, and log results in issues/test-collection-regressions-20251004.md and issues/coverage-below-threshold.md. The 2025-10-08 collector transcript documents the current gaps to address next.【F:issues/test-collection-regressions-20251004.md†L16-L33】【F:docs/release/v0.1.0a1_execution_plan.md†L50-L87】【F:diagnostics/pytest_collect_only_20251008_051340.log†L5645-L6406】
31.9.1 [ ] Restore the `tests/behavior/requirements_wizard/features/general/logging_and_priority.feature` asset (and any sibling feature files) so the requirements-wizard steps stop raising `FileNotFoundError` during collection; archive the refreshed `pytest --collect-only -q` transcript.【F:diagnostics/pytest_collect_20251007T0151Z.log†L5389-L5405】
31.9.2 [ ] Reapply exactly one speed marker to the WSDE/UXBridge/UI integration suites flagged by the latest collector warnings (e.g., `tests/integration/general/test_wsde_edrr_integration_end_to_end.py` and `tests/unit/interface/test_webui_bridge_progress.py`) and rerun marker verification scripts.【F:diagnostics/pytest_collect_20251007T0151Z.log†L5861-L5939】
31.10 [ ] After tasks 31.6–31.9, rerun `pytest --collect-only -q` and `pytest -k nothing --collect-only` to prove collection hygiene, archiving logs in `diagnostics/pytest_collect_*.log` for the release bundle. The new log captures the full-suite and behavior-step collectors; `-k nothing` remains outstanding.【F:docs/release/v0.1.0a1_execution_plan.md†L68-L87】【F:diagnostics/pytest_collect_only_20251008_051340.log†L5634-L5650】【F:diagnostics/pytest_collect_only_20251008_051340.log†L6397-L6406】
