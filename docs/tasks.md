...
29.4 [ ] Build asynchronous provider-system and retry/back-pressure fixtures to lift `src/devsynth/adapters/provider_system.py` to ≥60 % coverage while augmenting `docs/implementation/provider_system_invariants.md` with the new metrics evidence.【d361cd†L1-L2】
      ...
      - [x] Removed the `devsynth.adapters.llm.llm_adapter` override after introducing a local factory protocol, a frozen provider config, and typed lookup errors validated by strict mypy with 100 % targeted fast-test coverage.【F:src/devsynth/adapters/llm/llm_adapter.py†L16-L94】【F:tests/unit/adapters/llm/test_llm_adapter.py†L1-L139】【55dbeb†L1-L33】
      ...
      - [x] Removed the `devsynth.adapters.jira_adapter` override by introducing typed dataclass payloads, an injectable HTTP client protocol, strict mypy checks, and targeted unit coverage validating Jira schema serialization and error paths.【F:pyproject.toml†L268-L303】【F:src/devsynth/adapters/jira_adapter.py†L1-L155】【F:tests/unit/adapters/test_jira_adapter.py†L1-L86】
