...
- ❌ Release preparation (`task release:prep`): BLOCKED - Taskfile §23 now parses cleanly and executes through marker verification, but the workflow exits at `poetry build` because duplicate `overrides` keys remain in `pyproject.toml`; capture lives at `diagnostics/release_prep_20251005T035109Z.log` for the maintainer bundle.【F:diagnostics/release_prep_20251005T035109Z.log†L1-L25】
...
- 🔴 Test collection hygiene: NEW - 2025-10-05 targeted `pytest -k nothing` runs now fail due to SyntaxErrors (misplaced `pytestmark`), missing WebUI `.feature` files, `_ProgressIndicatorBase` alias timing, and integration suites lacking pytest imports. No fresh smoke or coverage evidence can be generated until these regressions are resolved.【d62a9a†L12-L33】【6cd789†L12-L28】【68488c†L1-L27】【e85f55†L1-L22】
