...
- Locked FastAPI to the 0.116.x series together with Starlette 0.46.x so smoke
  tests keep passing under Python 3.12 while the upstream TestClient MRO bug is
  outstanding (see `issues/run-tests-smoke-fast-fastapi-starlette-mro.md`). Revisit
  once Starlette ships a Python 3.12 fix and the `sitecustomize` shim can be
  removed.
