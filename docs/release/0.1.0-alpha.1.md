...
validates the runner. Use `task --version` to confirm the binary is installed.
...
All Python commands in this guide should be prefixed with `poetry run` to ensure they execute within the repository's virtual environment.
