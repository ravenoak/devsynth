...
- **Hot-spot delta tracking (2025-10-05 fast sweep)**: Targeted reruns for `methodology/edrr/reasoning_loop.py` capture a 68.89 % fast-only snapshot while refactors are staged, preserving evidence under `20251015T000000Z-fast-medium/` for cross-checking against the ≥90 % objective before the final aggregate rerun.【F:artifacts/releases/0.1.0a1/fast-medium/20251015T000000Z-fast-medium/reasoning_loop_fast.json†L1-L25】
- **2025-10-07 coverage follow-up**: Fresh fast tests now exercise deterministic seeding (random + NumPy), retry exhaustion/backoff logging, coordinator phase routing, and the max-total-seconds budget exit, with a targeted coverage JSON stored at `artifacts/reasoning_loop_additional_branches.json`. The aggregate rerun of `poetry run devsynth run-tests --speed=fast --speed=medium --report --no-parallel` still hits the 300 s collection guardrails for both speed filters before falling back to `-m "not memory_intensive"`, where it was manually interrupted without emitting new knowledge-graph IDs or manifests.【F:tests/unit/methodology/edrr/test_reasoning_loop_additional_branches.py†L1-L177】【cfe636†L1-L28】【99a14b†L1-L40】【F:artifacts/reasoning_loop_additional_branches.json†L1-L27】【15b39c†L1-L2】【1c9a14†L1-L4】【988205†L1-L1】
