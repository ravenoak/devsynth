...
2. Install dependencies with development and test extras:
   ...
   poetry env info --path
   ```
3. Configure environment variables for optional providers as needed:
   ```bash
   export OPENAI_API_KEY=<your_openai_key>
   export ANTHROPIC_API_KEY=<your_anthropic_key>
   export DEVSYNTH_RESOURCE_LMSTUDIO_AVAILABLE=true  # enable LM Studio tests
   export LM_STUDIO_ENDPOINT=http://localhost:8888   # LM Studio endpoint
   ```
   Omit these to skip external providers; the test suite defaults to offline implementations.
...
5. Validate deployment configuration:
   ...
   ```
6. Prepare release artifacts with Taskfile and run the dialectical audit (rerun `bash scripts/install_dev.sh` first if `task --version` fails):
   ```bash
   task --version
   task release:prep
   ```
...
7. Verify the repository is in a release-ready state:
   ```bash
