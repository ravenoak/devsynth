# DevSynth v0.1.0a1 Execution Plan (Updated 2025-10-06)
...
- **Behavior evidence (UX/BDD discipline)** – WebUI behavior suites reference `general/*.feature` paths that no longer exist; pytest-bdd attempts to open `tests/behavior/general/webui.feature` and fails before step bindings execute.【6cd789†L12-L28】
...
1. **Consolidate pytest plugin wiring** so pytest-bdd loads exactly once by hoisting nested `pytest_plugins` declarations into root configuration and auditing all plugin imports before rerunning any aggregates.【F:logs/devsynth_run-tests_fast_medium_20251006T033632Z.log†L1-L84】
2. **Unblock collection hygiene quickly** by repairing misplaced `pytestmark` declarations and reintroducing missing pytest imports so unit/integration suites import cleanly again.【d62a9a†L12-L33】【e85f55†L1-L22】
3. **Realign behavior specifications** by correcting scenario paths and regenerating WebUI/UXBridge feature evidence, restoring traceability promised in the improvement plan.【6cd789†L12-L28】
4. **Recover CLI progress foundations** by moving the `_ProgressIndicatorBase` definition ahead of dependent classes and adding regression tests that prove long-running progress instrumentation imports under strict typing.【68488c†L1-L27】
5. **Finish outstanding optional-backend gating and pytest plug-in migrations** so resource-dependent suites skip gracefully and pytest 8+ recognises plugin configuration without runtime errors.【F:issues/test-collection-regressions-20251004.md†L16-L33】
6. **Re-run automation and coverage gates** (task release prep, strict mypy, fast+medium coverage) after hygiene fixes land, capturing refreshed artifacts and updating UAT documentation.
7. **Re-evaluate documentation alignment recommendations** from the critical-improvements dossier once the gates are green, prioritising docs/plan.md follow-ups that close implementation/documentation gaps.【F:docs/analysis/critical_recommendations.md†L1-L74】

## Pull Request Work Breakdown
| PR | Goal | Key Tasks | Dependencies | Parallelizable? |
|----|------|-----------|--------------|-----------------|
| **PR-0: pytest Plugin Consolidation** | Prevent duplicate plugin registration so pytest can collect tests again. | - Hoist interface-level `pytest_plugins` exports into `tests/conftest.py` or dedicated helper modules.<br>- Add regression check (e.g., `pytest --collect-only -q`) that proves pytest-bdd loads once.<br>- Record evidence in issues/test-collection-regressions-20251004.md. | None | Yes — foundational hygiene fix. |
| **PR-1: Test Hygiene Hotfix** | Restore import-time stability so the suite collects. | - Move `pytestmark` assignments outside import tuples across the affected unit/domain suites.<br>- Add missing `import pytest` statements where only `pytestmark` is used.<br>- Capture targeted `pytest -k nothing` transcripts showing clean import to verify the fix. | PR-0 (ensures plugin manager stabilizes first). | Yes — once PR-0 lands. |
| **PR-2: Behavior Asset Realignment** | Point behavior scenarios at the actual feature tree and regenerate traceability evidence. | - Update `pytest_bdd.scenarios(...)` paths to `features/general/...` and rerun targeted BDD collections.<br>- Refresh docs/tasks.md §13.1.5 with the corrected locations and link regenerated manifests. | PR-0 (plugin wiring) + PR-1 (imports). | Yes — can progress alongside PR-3 after prerequisites. |
| **PR-3: CLI Progress & Protocol Foundations** | Fix `_ProgressIndicatorBase` alias timing and audit remaining Protocol generics. | - Hoist the Protocol definition above runtime subclasses, adding regression tests for the CLI progress helpers.<br>- Revisit `_VectorStore`/`MemoryStore` generics to ensure runtime + mypy compatibility.<br>- Update issues/test-collection-regressions-20251004.md with fresh evidence. | PR-0 (plugin wiring) + PR-1 (imports). | Yes — orthogonal to PR-2. |
| **PR-4: Optional Backend Guardrails** | Finish fixture migrations so optional services skip cleanly under pytest 8+. | - Adjust `skip_if_missing_backend` to tolerate `ModuleSpec` objects with `None` values.<br>- Guard optional imports with `pytest.importorskip` and speed/resource markers.<br>- Document resource toggles in docs/tasks.md and docs/plan.md. | PR-0/PR-1 (clean collection) | Yes — can run parallel to PR-2/PR-3. |
| **PR-5: Gate Refresh & Documentation Harmony** | Re-run automation, regenerate coverage/typing manifests, and realign release docs with the improvement dossier. | - Execute `task release:prep`, `task mypy:strict`, and the fast+medium aggregate, storing new artifacts.<br>- Update docs/plan.md, docs/tasks.md, and issues/coverage-below-threshold.md with the new evidence.<br>- Cross-check docs/analysis/critical_recommendations.md items and schedule any remaining documentation alignment follow-ups. | PR-0 → PR-4 (requires passing collection and smoke). | Limited parallelism — final integrative pass. |
| **PR-6: UAT Bundle & Post-Tag Preparation** | Compile UAT evidence, refresh readiness issues, and queue workflow reactivation. | - Capture passing smoke/release-prep/typing logs for UAT sign-off.<br>- Update release readiness and post-tag issues with the new sequencing.<br>- Draft the CI trigger reactivation PR for human reviewers. | PR-5 (needs green gates). | Sequential finalization step. |
...
- PR-2 through PR-4 address orthogonal domains (BDD assets, CLI runtime, optional services) and can proceed in parallel immediately after PR-1 merges.
- PR-5 consolidates automation and evidence once the earlier fixes land; it should branch only after PR-2 through PR-4 pass their targeted collections.
- PR-6 remains the integration checkpoint wrapping up UAT and post-tag planning.
...
1. Deliver **PR-0** by consolidating pytest plugin registration so pytest-bdd loads once, then capture clean `pytest --collect-only -q` output as proof that the plugin manager no longer raises `ValueError`.【F:logs/devsynth_run-tests_fast_medium_20251006T033632Z.log†L1-L84】
2. Follow with **PR-1** to fix misplaced `pytestmark` declarations and missing pytest imports, capturing clean `pytest -k nothing` transcripts as proof of collection recovery.【d62a9a†L12-L33】【e85f55†L1-L22】
3. Launch **PR-2** and **PR-3** in parallel: update behavior scenario paths and hoist `_ProgressIndicatorBase` so BDD and CLI progress suites import successfully, recording fresh issue notes as evidence.【6cd789†L12-L28】【68488c†L1-L27】
4. Execute **PR-4** to finish optional backend guardrails, ensuring resource checks no longer halt collection before coverage can run.【F:issues/test-collection-regressions-20251004.md†L16-L33】
5. After PR-0 through PR-4 merge, pursue **PR-5** and **PR-6** sequentially to regenerate automation artifacts, align documentation with the critical recommendations dossier, and compile the UAT/post-tag bundles for maintainer review.【F:docs/analysis/critical_recommendations.md†L1-L74】【F:issues/release-finalization-uat.md†L49-L60】【F:issues/re-enable-github-actions-triggers-post-v0-1-0a1.md†L1-L18】
