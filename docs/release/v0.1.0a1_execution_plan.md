...
- `task release:prep` now runs until `poetry build` hits the duplicate `overrides` keys in `pyproject.toml`, while `task mypy:strict` succeeds and logs the refreshed memory report; §23 literal blocks plus the `task lint:taskfile` dry-run (`task -n maintainer:must-run`) keep go-task parsing clean.【F:diagnostics/release_prep_20251005T035109Z.log†L1-L25】【F:diagnostics/mypy_strict_20251005T035128Z.log†L1-L20】【F:diagnostics/mypy_strict_application_memory_20251005T035144Z.txt†L1-L1】【F:Taskfile.yml†L561-L668】【F:Taskfile.yml†L669-L681】
