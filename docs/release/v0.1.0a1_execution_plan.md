...
1. **Unblock collection hygiene quickly** by repairing misplaced `pytestmark` declarations and reintroducing missing pytest imports so unit/integration suites import cleanly again.【d62a9a†L12-L33】【e85f55†L1-L22】
2. **Realign behavior specifications** by correcting scenario paths and regenerating WebUI/UXBridge feature evidence, restoring traceability promised in the improvement plan.【6cd789†L12-L28】
3. **Recover CLI progress foundations** by moving the `_ProgressIndicatorBase` definition ahead of dependent classes and adding regression tests that prove long-running progress instrumentation imports under strict typing.【68488c†L1-L27】
4. **Finish outstanding optional-backend gating and pytest plug-in migrations** so resource-dependent suites skip gracefully and pytest 8+ recognises plugin configuration without runtime errors.【F:issues/test-collection-regressions-20251004.md†L16-L33】
5. **Re-run automation and coverage gates** (task release prep, strict mypy, fast+medium coverage) after hygiene fixes land, capturing refreshed artifacts and updating UAT documentation.
6. **Re-evaluate documentation alignment recommendations** from the critical-improvements dossier once the gates are green, prioritising docs/plan.md follow-ups that close implementation/documentation gaps.【F:docs/analysis/critical_recommendations.md†L1-L74】

## Pull Request Work Breakdown
| PR | Goal | Key Tasks | Dependencies | Parallelizable? |
|----|------|-----------|--------------|-----------------|
| **PR-0: Test Hygiene Hotfix** | Restore import-time stability so the suite collects. | - Move `pytestmark` assignments outside import tuples across the affected unit/domain suites.<br>- Add missing `import pytest` statements where only `pytestmark` is used.<br>- Capture targeted `pytest -k nothing` transcripts showing clean import to verify the fix. | None | Yes — stands alone and unblocks later work. |
| **PR-1: Behavior Asset Realignment** | Point behavior scenarios at the actual feature tree and regenerate traceability evidence. | - Update `pytest_bdd.scenarios(...)` paths to `features/general/...` and rerun targeted BDD collections.<br>- Refresh docs/tasks.md §13.1.5 with the corrected locations and link regenerated manifests. | PR-0 (ensures module imports succeed). | Yes — once PR-0 merges, can run alongside PR-2. |
| **PR-2: CLI Progress & Protocol Foundations** | Fix `_ProgressIndicatorBase` alias timing and audit remaining Protocol generics. | - Hoist the Protocol definition above runtime subclasses, adding regression tests for the CLI progress helpers.<br>- Revisit `_VectorStore`/`MemoryStore` generics to ensure runtime + mypy compatibility.<br>- Update issues/test-collection-regressions-20251004.md with fresh evidence. | PR-0 (shared test harness). | Yes — independent of behavior assets. |
| **PR-3: Optional Backend & pytest Plugin Guardrails** | Finish fixture migrations so optional services skip cleanly under pytest 8+. | - Adjust `skip_if_missing_backend` to tolerate `ModuleSpec` objects with `None` values.<br>- Move lingering `pytest_plugins` declarations into root-level conftest modules.<br>- Document resource toggles in docs/tasks.md and docs/plan.md. | PR-0 (requires clean imports). | Yes — can run parallel to PR-1/PR-2 once hygiene fixed. |
| **PR-4: Gate Refresh & Documentation Harmony** | Re-run automation, regenerate coverage/typing manifests, and realign release docs with the improvement dossier. | - Execute `task release:prep`, `task mypy:strict`, and the fast+medium aggregate, storing new artifacts.<br>- Update docs/plan.md, docs/tasks.md, and issues/coverage-below-threshold.md with the new evidence.<br>- Cross-check docs/analysis/critical_recommendations.md items and schedule any remaining documentation alignment follow-ups. | PR-0 → PR-3 (requires passing collection and smoke). | Limited parallelism — final integrative pass. |
| **PR-5: UAT Bundle & Post-Tag Preparation** | Compile UAT evidence, refresh readiness issues, and queue workflow reactivation. | - Capture passing smoke/release-prep/typing logs for UAT sign-off.<br>- Update release readiness and post-tag issues with the new sequencing.<br>- Draft the CI trigger reactivation PR for human reviewers. | PR-4 (needs green gates). | Sequential finalization step. |

## Parallelization Notes
- PR-0 is the new entry point; without it, every downstream task hits SyntaxError/NameError walls.
- PR-1, PR-2, and PR-3 address orthogonal domains (BDD assets, CLI runtime, optional services) and can proceed in parallel immediately after PR-0 merges.
- PR-4 consolidates automation and evidence once the earlier fixes land; it should branch only after PR-1 through PR-3 pass their targeted collections.
- PR-5 remains the integration checkpoint wrapping up UAT and post-tag planning.
...
1. Deliver **PR-0** by fixing the misplaced `pytestmark` declarations and missing pytest imports, then capture clean `pytest -k nothing` transcripts as proof of collection recovery.【d62a9a†L12-L33】【e85f55†L1-L22】
2. Launch **PR-1** and **PR-2** in parallel: update behavior scenario paths and hoist `_ProgressIndicatorBase` so BDD and CLI progress suites import successfully, recording fresh issue notes as evidence.【6cd789†L12-L28】【68488c†L1-L27】
3. Execute **PR-3** to finish optional backend/pytest plugin guardrails, ensuring resource checks no longer halt collection before coverage can run.【F:issues/test-collection-regressions-20251004.md†L16-L33】
4. After PR-0 through PR-3 merge, pursue **PR-4** and **PR-5** sequentially to regenerate automation artifacts, align documentation with the critical recommendations dossier, and compile the UAT/post-tag bundles for maintainer review.【F:docs/analysis/critical_recommendations.md†L1-L74】【F:issues/release-finalization-uat.md†L49-L60】【F:issues/re-enable-github-actions-triggers-post-v0-1-0a1.md†L1-L18】
