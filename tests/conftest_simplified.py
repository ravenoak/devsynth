...
def _configure_coverage_enforcement(config: pytest.Config) -> None:
    ...
    strict_coverage = os.getenv("DEVSYNTH_FULL_COVERAGE") or os.getenv("DEVSYNTH_STRICT_COVERAGE")
...
def _create_test_environment(tmp_path: Path, monkeypatch) -> Dict[str, Path]:
    ...
    (devsynth_dir / "config.json").write_text('{"model": "test-model", "project_name": "test-project"}')
    ...
    for item in items:
        for marker in item.iter_markers(name="requires_resource"):
            ...
            if not is_resource_available(resource):
                item.add_marker(pytest.mark.skip(f"Resource '{resource}' not available"))


def _apply_speed_marker_validation(items):
    """Validate that tests have appropriate speed markers."""
    for item in items:
        speed_markers = [name for name in ("fast", "medium", "slow") 
...
def _apply_isolation_management(items):
    ...
    if os.environ.get("PYTEST_XDIST_WORKER") and not os.environ.get("DEVSYNTH_ALLOW_ISOLATION_IN_XDIST"):
        for item in items:
...
def tmp_project_dir(tmp_path):
    ...
    # Create basic configuration
    (devsynth_dir / "config.json").write_text('{"model": "gpt-4", "project_name": "test-project"}')
