...
def random_date(start_date: Optional[datetime]=None, end_date: Optional[
...
def random_project_structure() ->Dict[str, Any]:
    ...
    return {'src': {'main.py': random_python_code(), 'utils': {'helpers.py':
        random_python_code(), 'config.py': random_python_code()}, 'models':
        {'user.py': random_python_code(), 'project.py': random_python_code(
        )}}, 'tests': {'test_main.py': random_python_code(), 'test_utils':
        {'test_helpers.py': random_python_code(), 'test_config.py':
        random_python_code()}}, 'README.md':
        f"""# Test Project
...
def _create_project_files(base_dir: Union[str, Path], structure: Dict[str, Any]
    ) ->None:
    """
...
def generate_test_user(user_id: Optional[str]=None) ->Dict[str, Any]:
    ...
    return {'id': user_id or str(uuid.uuid4()), 'username': random_string(8
        ).lower(), 'email': random_email(), 'first_name': random_string(6),
        'last_name': random_string(8), 'created_at': random_date().
        isoformat(), 'is_active': random.choice([True, False])}


def generate_test_project(project_id: Optional[str]=None, owner_id:
    Optional[str]=None) ->Dict[str, Any]:
    """Generate a test project with random data."""
    return {'id': project_id or str(uuid.uuid4()), 'name':
        f'Project {random_string(5)}', 'description': random_string(50,
        include_spaces=True), 'owner_id': owner_id or str(uuid.uuid4()),
        'created_at': random_date().isoformat(), 'updated_at': random_date(
        ).isoformat(), 'is_public': random.choice([True, False]), 'tags': [
        random_string(5) for _ in range(random.randint(1, 5))]}


def generate_test_memory_item(item_id: Optional[str]=None) ->Dict[str, Any]:
    """Generate a test memory item with random data."""
    return {'id': item_id or str(uuid.uuid4()), 'content': random_string(
        100, include_spaces=True), 'embedding': [random.random() for _ in
        range(10)], 'metadata': {'source': random.choice(['user', 'system',
        'agent']), 'timestamp': random_date().isoformat(), 'importance':
        random.randint(1, 10)}}
