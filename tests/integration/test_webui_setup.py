...
def _setup_streamlit(monkeypatch, button_return=False, toggle_return=False):
    ...
    modules = ['langgraph', 'langgraph.checkpoint',
        'langgraph.checkpoint.base', 'langgraph.graph', 'langchain',
        'langchain_openai', 'langchain_community', 'tiktoken', 'tinydb',
        'tinydb.storages', 'tinydb.middlewares', 'duckdb', 'lmdb', 'faiss',
        'httpx', 'openai', 'openai.types', 'openai.types.chat', 'torch',
        'transformers', 'astor']
    for name in modules:
...
ReqID: N/A"""
    ...
    mock_run = MagicMock()
    monkeypatch.setattr(webui, 'SetupWizard', MagicMock(return_value=
...
ReqID: N/A"""
    ...
    cfg = ProjectUnifiedConfig(ConfigModel(project_root=str(tmp_path)), 
        tmp_path / 'project.yaml', False)
    monkeypatch.setattr(webui, 'load_project_config', lambda : cfg)
    saved = {}
    monkeypatch.setattr(webui, 'save_config', lambda conf, *, use_pyproject,
        path: saved.update({'offline': conf.offline_mode}))
