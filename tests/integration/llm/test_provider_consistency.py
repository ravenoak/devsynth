...
class TestProviderInterfaceConsistency:
    ...
    def test_provider_has_required_methods(self, provider_name):
        ...
        if provider_name in ["openrouter", "openai"] and not _is_provider_available(
            provider_name
        ):
        ...
        try:
            ...
            assert hasattr(
                provider, "generate"
            ), f"{provider_name} missing generate method"
            assert hasattr(
                provider, "generate_with_context"
            ), f"{provider_name} missing generate_with_context method"
            assert hasattr(
                provider, "get_embedding"
            ), f"{provider_name} missing get_embedding method"

            # Test identical behavior - basic generation
            response = provider.generate("Hello")
            assert isinstance(
                response, str
            ), f"{provider_name} generate() should return string"
            assert (
                len(response) > 0
            ), f"{provider_name} generate() should return non-empty string"
    ...
    def test_provider_method_signatures_consistent(self, provider_name):
        ...
        try:
            ...
            assert (
                "prompt" in generate_params
            ), f"{provider_name}.generate() missing prompt parameter"
            assert (
                "prompt" in context_params
            ), f"{provider_name}.generate_with_context() missing prompt parameter"
            assert (
                "text" in embedding_params
            ), f"{provider_name}.get_embedding() missing text parameter"
...
class TestProviderErrorConsistency:
    ...
    def test_authentication_error_consistency(self, provider_name):
        ...
        try:
            ...
            assert (
                "error" in error_msg
            ), f"{provider_name} error message should contain 'error'"
...
class TestProviderPerformanceConsistency:
    ...
    def test_response_time_consistency(self, provider_name):
        ...
        try:
            ...
            assert (
                response_time < 30.0
            ), f"{provider_name} response time too slow: {response_time}s"
            assert (
                response_time > 0.001
            ), f"{provider_name} response time suspiciously fast: {response_time}s"
    ...
    def test_token_usage_consistency(self, provider_name):
        ...
        try:
            ...
            ), f"{provider_name} missing token tracker"
            assert (
                provider.token_tracker is not None
            ), f"{provider_name} token tracker not initialized"
            ...
            assert isinstance(
                token_count, int
            ), f"{provider_name} token count should be integer"
...
class TestProviderBehaviorConsistency:
    ...
    def test_generation_behavior_consistency(self, provider_name):
        ...
        try:
            ...
            assert (
                0 < len(response) < 10000
            ), f"{provider_name} response length out of range"

            # Should contain expected content (allowing for some variation)
            response_lower = response.lower()
            assert any(
                term in response_lower for term in ["hello", "world", "hi", "greetings"]
    ...
    def test_context_behavior_consistency(self, provider_name):
        ...
        try:
            ...
            assert any(
                ...
                for term in ["python", "programming", "language", "code", "syntax"]
