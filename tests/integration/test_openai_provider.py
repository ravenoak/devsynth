...
ReqID: N/A"""
        with patch('devsynth.application.llm.openai_provider.OpenAI'
...
ReqID: N/A"""
        with patch.dict(os.environ, {}, clear=True), patch(
            ...
            mock_settings.return_value = {'provider': 'openai', 'api_base':
                '', 'model': '', 'max_tokens': 1024, 'temperature': 0.7,
                'auto_select_model': False, 'openai_api_key': None,
                'openai_model': 'gpt-3.5-turbo', 'openai_embedding_model':
...
ReqID: N/A"""
        with patch('devsynth.application.llm.openai_provider.OpenAI'
            ...
            messages_arg = mock_client.chat.completions.create.call_args[1][
                'messages']
            assert len(messages_arg) == 4
            assert messages_arg[-1]['content'
...
ReqID: N/A"""
        with patch('devsynth.application.llm.openai_provider.OpenAI'
            ...
            mock_settings.return_value = {'openai_api_key': 'test_key',
                'openai_model': 'gpt-3.5-turbo', 'max_tokens': 2000,
                'temperature': 0.7, 'api_base': None}
            mock_response = MagicMock()
            mock_response.choices = [MagicMock()]
...
ReqID: N/A"""
        with patch('devsynth.application.llm.openai_provider.OpenAI'
            ...
            mock_settings.return_value = {'openai_api_key': 'test_key',
                'openai_model': 'gpt-3.5-turbo', 'openai_embedding_model':
                'text-embedding-ada-002', 'max_tokens': 2000, 'temperature':
                0.7, 'api_base': None}
            mock_response = MagicMock()
            mock_response.data = [MagicMock()]
