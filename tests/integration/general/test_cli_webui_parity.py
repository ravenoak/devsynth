...
ReqID: N/A"""
    ...
    cli_module.init_cmd(path='demo', project_root='demo', language='python',
        goals=None, bridge=cli_bridge)
...
ReqID: N/A"""
    ...
    monkeypatch.setattr('rich.console.Console.print', lambda self, msg, *,
        highlight=False: out_cli.append(str(msg) if hasattr(msg, '__str__')
         else msg))
    st = sys.modules['streamlit']
    out_web: list[str] = []
    st.write = MagicMock(side_effect=lambda msg: out_web.append(str(msg) if
        hasattr(msg, '__str__') else msg))
    st.markdown = MagicMock(side_effect=lambda msg, **kwargs: out_web.
        append(str(msg) if hasattr(msg, '__str__') else msg))
    st.info = MagicMock(side_effect=lambda msg: out_web.append(str(msg) if
        hasattr(msg, '__str__') else msg))
    st.error = MagicMock(side_effect=lambda msg: out_web.append(str(msg) if
        hasattr(msg, '__str__') else msg))
    st.warning = MagicMock(side_effect=lambda msg: out_web.append(str(msg) if
        hasattr(msg, '__str__') else msg))
    st.success = MagicMock(side_effect=lambda msg: out_web.append(str(msg) if
        hasattr(msg, '__str__') else msg))
    st.header = MagicMock(side_effect=lambda msg: out_web.append(str(msg) if
        hasattr(msg, '__str__') else msg))
    st.subheader = MagicMock(side_effect=lambda msg: out_web.append(str(msg
        ) if hasattr(msg, '__str__') else msg))
