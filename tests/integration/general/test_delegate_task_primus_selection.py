...
def test_delegate_task_calls_select_primus_by_expertise_and_updates_primus_succeeds():
    ...
    consensus = {
        "consensus": "x",
        "contributors": ["doc", "coder"],
        "method": "consensus_synthesis",
    }
    with (
        patch.object(coordinator.team, "build_consensus", return_value=consensus),
        patch.object(
            coordinator.team,
            "select_primus_by_expertise",
            wraps=coordinator.team.select_primus_by_expertise,
        ) as spy,
    ):
        result1 = coordinator.delegate_task(
            {"team_task": True, "type": "coding", "language": "python"}
        )
        first_primus = coordinator.team.get_primus().name
        result2 = coordinator.delegate_task(
            {"team_task": True, "type": "documentation"}
        )
