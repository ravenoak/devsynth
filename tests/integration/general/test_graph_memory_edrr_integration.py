...
class TestGraphMemoryEDRRIntegration:
    ...
    def mock_dependencies(self):
        ...
        implementation_evaluation = {"quality": "good", "issues": [], "suggestions": []}
    ...
    def coordinator(self, graph_adapter, mock_dependencies):
        ...
        config = {
            "edrr": {"max_recursion_depth": 3, "phase_transition": {"auto": False}},
            "features": {"automatic_phase_transitions": False},
        }
    ...
    def test_edrr_cycle_with_graph_memory_succeeds(
        ...
        if isinstance(task_items[0].content, str):
            if task_items[0].content.strip().startswith("{") and task_items[
                0
    ...
    def test_memory_volatility_with_edrr_succeeds(
        ...
        ReqID: N/A"""
        coordinator.start_cycle(sample_task)
        graph_adapter.add_memory_volatility(
            decay_rate=0.3, threshold=0.5, advanced_controls=True
        )
        task_items = graph_adapter.search({"type": MemoryType.TASK_HISTORY.value})
        if not task_items:
            task_items = graph_adapter.search({"type": "TASK"})
    ...
    def test_cross_store_sync_with_memory_manager(
        ...
        assert any(
            item.metadata.get("edrr_phase") == Phase.EXPAND.name
