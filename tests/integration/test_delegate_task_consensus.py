...
ReqID: N/A"""
    ...
    def fake_consensus(_task):
        ...
        return {'consensus': 'final', 'contributors': [primus_name, writer.
            name], 'method': 'consensus_synthesis'}
    with patch.object(coordinator.team, 'build_consensus', side_effect=
        fake_consensus):
...
ReqID: N/A"""
    ...
    for a in [a1, a2, a3]:
        coordinator.add_agent(a)
    task = {'team_task': True, 'type': 'critical_decision', 'is_critical': 
        True, 'options': [{'id': 'o1'}, {'id': 'o2'}]}
    with patch.object(coordinator.team, 'vote_on_critical_decision', wraps=
        coordinator.team.vote_on_critical_decision) as vote_spy, patch.object(
        coordinator.team, 'build_consensus', wraps=coordinator.team.
        build_consensus) as consensus_spy:
...
ReqID: N/A"""
    ...
    for a in [a1, a2, a3, a4]:
        coordinator.add_agent(a)
    consensus_result = {'consensus': 'combined', 'contributors': [a.name for
        a in [a1, a2, a3, a4]], 'method': 'consensus_synthesis'}
    task = {'team_task': True, 'type': 'critical_decision', 'is_critical': 
        True, 'options': [{'id': 'o1'}, {'id': 'o2'}]}
    with patch.object(coordinator.team, 'build_consensus', return_value=
        consensus_result) as consensus_spy, patch.object(coordinator.team,
        'vote_on_critical_decision', wraps=coordinator.team.
        vote_on_critical_decision) as vote_spy:
        result = coordinator.delegate_task(task)
    vote_spy.assert_called_once_with(task)
