...
def small_workflow(bridge: CLIUXBridge | APIBridge) -> list[str]:
    ...
    # Collect messages when using CLI bridge by reading from DummyProgress
...
def _setup_cli_bridge(monkeypatch) -> CLIUXBridge:
    ...
    monkeypatch.setattr(
        ...
        lambda self, msg, *, highlight=False: out.append(sanitize_output(msg)),
    )
...
def test_cli_and_api_bridges_consistent(monkeypatch):
    cli_bridge = _setup_cli_bridge(monkeypatch)
    cli_msgs = small_workflow(cli_bridge)

    api_bridge = APIBridge(["demo", True])
    api_msgs = small_workflow(api_bridge)
    api_msgs = api_bridge.messages
