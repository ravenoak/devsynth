...
#
# Purpose:
# - Ensure exactly one speed marker at the function level for tests under tests/integration/**.
...
#
# Rationale:
# - Aligns with docs/plan.md Verification Protocol and docs/tasks.md P1.1 subtask 1.3.
# - Mirrors the behavior-suite centralization pattern to reduce scattered, fragile per-file hooks.
#
# Notes:
# - We intentionally inspect item.own_markers (function/class level) instead of get_closest_marker,
#   because the latter may include module-level markers, which our verifier does not accept for speed categories.
...
def pytest_collection_modifyitems(
    ...
    for item in items:
        ...
        if _has_function_level_speed_marker(item):
            continue
        # Apply default medium speed for integration tests missing a function-level speed marker.
