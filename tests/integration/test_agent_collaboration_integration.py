...
from devsynth.application.collaboration.agent_collaboration import AgentCollaborationSystem, CollaborationTask, AgentMessage, MessageType, TaskStatus
from devsynth.application.collaboration.message_protocol import MessageProtocol, MessageStore, MessageType as ProtocolMessageType
...
class MockAgent(Agent):
    ...
    def __init__(self, agent_id: str, capabilities: list=None):
        ...
        self._process_mock = MagicMock(return_value={'status': 'success',
            'message': f'Processed by {agent_id}'})
...
ReqID: N/A"""
        ...
        assert collaboration_system.agent_capabilities[agent_id] == {'planning'
            , 'coding'}
...
ReqID: N/A"""
        ...
        team = collaboration_system.create_team('team1', ['agent1',
            'agent2', 'agent3'])
        ...
        role_counts = {'Worker': sum(1 for agent in team.agents if agent.
            current_role == 'Worker'), 'Supervisor': sum(1 for agent in
            team.agents if agent.current_role == 'Supervisor'), 'Designer':
            sum(1 for agent in team.agents if agent.current_role ==
            'Designer'), 'Evaluator': sum(1 for agent in team.agents if 
...
ReqID: N/A"""
        ...
        collaboration_system.register_agent(agent1)
        collaboration_system.register_agent(agent2)
        task = collaboration_system.create_task(task_type='coding',
            description='Implement a feature', inputs={'feature': 'login'},
...
ReqID: N/A"""
        ...
        collaboration_system.register_agent(agent1)
        collaboration_system.register_agent(agent2)
        message = collaboration_system.send_message(sender_id='agent1',
            recipient_id='agent2', message_type=MessageType.QUESTION,
...
ReqID: N/A"""
        ...
        collaboration_system.register_agent(agent2)
        collaboration_system.register_agent(agent3)
        task1 = collaboration_system.create_task(task_type='planning',
            description='Plan the feature', inputs={'feature': 'login'},
            required_capabilities=['planning'])
        task2 = collaboration_system.create_task(task_type='coding',
            description='Implement the feature', inputs={'feature': 'login'
            }, required_capabilities=['coding'])
        task3 = collaboration_system.create_task(task_type='testing',
            description='Test the feature', inputs={'feature': 'login'},
...
ReqID: N/A"""
        ...
        proto.send_message(sender='a', recipients=['b'], message_type=
            ProtocolMessageType.STATUS_UPDATE, subject='s', content='c')
...
ReqID: N/A"""
        ...
        result = run_peer_review(work_product={'text': 'demo'}, author=
            author, reviewers=[reviewer1, reviewer2])
