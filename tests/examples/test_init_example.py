...
@pytest.mark.fast
def test_init_example_runs_quickly(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    script = Path(__file__).resolve().parents[2] / "examples" / "init_example" / "main.py"
    assert script.exists(), f"Example script not found: {script}"

    result = subprocess.run([sys.executable, str(script)], capture_output=True, text=True, check=False)

    assert result.returncode == 0, f"Example exited with non-zero code: {result.returncode}\nSTDERR: {result.stderr}"
