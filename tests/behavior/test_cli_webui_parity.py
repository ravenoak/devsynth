...
def parity_env(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules,
        'devsynth.application.cli.setup_wizard', setup_wizard_mod)
    ...
    monkeypatch.setitem(sys.modules,
        'devsynth.application.cli.commands.doctor_cmd', doctor_mod)
    cli_stub.doctor_cmd = doctor_mod.doctor_cmd
    analyze_mod = ModuleType(
        'devsynth.application.cli.commands.inspect_code_cmd')
    analyze_mod.inspect_code_cmd = MagicMock()
    monkeypatch.setitem(sys.modules,
        'devsynth.application.cli.commands.inspect_code_cmd', analyze_mod)
    edrr_mod = ModuleType('devsynth.application.cli.commands.edrr_cycle_cmd')
    edrr_mod.edrr_cycle_cmd = MagicMock()
    monkeypatch.setitem(sys.modules,
        'devsynth.application.cli.commands.edrr_cycle_cmd', edrr_mod)
    align_mod = ModuleType('devsynth.application.cli.commands.align_cmd')
    align_mod.align_cmd = MagicMock()
    monkeypatch.setitem(sys.modules,
        'devsynth.application.cli.commands.align_cmd', align_mod)
    for cmd_name in ['alignment_metrics_cmd', 'validate_manifest_cmd',
        'validate_metadata_cmd', 'test_metrics_cmd', 'generate_docs_cmd',
        'analyze_manifest_cmd', 'inspect_config_cmd']:
        ...
        monkeypatch.setitem(sys.modules,
            f'devsynth.application.cli.commands.{cmd_name}', mod)
...
def _capture_webui_messages(webui_cls, bridge, st):
    ui = webui_cls.WebUI()
    ui.display_result = lambda msg, highlight=False: bridge.display_result(msg,
        highlight=highlight)
    ui.create_progress = lambda desc, total=100: bridge.create_progress(desc,
        total=total)
    return ui
...
ReqID: N/A"""
    ...
    cli_messages = list(bridge_cli.messages)
    bridge_web = APIBridge()
    st.text_input.side_effect = ['requirements.md', 'requirements.md',
        'specs.md']
