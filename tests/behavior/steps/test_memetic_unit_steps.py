...
from src.devsynth.application.memory.memetic_unit_ingestion import MemeticUnitIngestionPipeline
from src.devsynth.application.memory.cognitive_type_classifier import CognitiveTypeClassifier
...
def step_raw_data_from_sources(context):
    ...
    context.raw_data_samples = [
        ...
        },
        {
            ...
            "data": {"function": "authenticate", "result": "success", "execution_time": 0.1},
...
def step_valid_memetic_unit_created(context):
    ...
    assert context.ingestion_error is None, f"Ingestion failed: {context.ingestion_error}"
...
def step_metadata_fields_populated(context):
    ...
    for unit in context.created_units:
        ...
        assert unit.metadata.content_hash is not None and len(unit.metadata.content_hash) > 0
...
def step_cognitive_type_classified(context):
    ...
    for i, unit in enumerate(context.created_units):
        ...
        assert unit.metadata.cognitive_type == expected_type, \
...
def step_data_from_different_sources(context):
    ...
    context.classification_samples = [
        (MemeticSource.USER_INPUT, "What should I implement next?", CognitiveType.WORKING),
        (MemeticSource.CODE_EXECUTION, {"error": "authentication failed", "trace": "stack trace"}, CognitiveType.EPISODIC),
        (MemeticSource.DOCUMENTATION, "The system provides user authentication features", CognitiveType.SEMANTIC),
        (MemeticSource.API_RESPONSE, {"method": "POST", "endpoint": "/login", "response": 200}, CognitiveType.PROCEDURAL)
    ]
...
def step_system_classifies_cognitive_type(context):
    ...
    for source, data, expected_type in context.classification_samples:
        ...
        metadata = MemeticMetadata(
            source=source,
            cognitive_type=CognitiveType.WORKING  # Default
        )
        unit = MemeticUnit(metadata=metadata, payload=data)

        # Classify using the classifier
        classified_type = context.classifier.classify(unit, {"classification_test": True})
...
def step_user_input_working_memory(context):
    ...
    working_classifications = [
        (unit, classified, expected) for unit, classified, expected in context.classification_results
        if expected == CognitiveType.WORKING
    ]

    for unit, classified, expected in working_classifications:
        assert classified == CognitiveType.WORKING, \
...
def step_code_execution_episodic(context):
    ...
    for unit, classified, expected in episodic_classifications:
        assert classified == CognitiveType.EPISODIC, \
...
def step_documentation_semantic(context):
    ...
    for unit, classified, expected in semantic_classifications:
        assert classified == CognitiveType.SEMANTIC, \
...
def step_api_response_procedural(context):
    ...
    for unit, classified, expected in procedural_classifications:
        assert classified == CognitiveType.PROCEDURAL, \
...
def step_access_patterns_influence_retention(context):
    ...
    recent_access_result = next(
        r for r in context.governance_results
    ...
    assert recent_access_result["updated_salience"] >= old_access_result["updated_salience"]
...
def step_identical_content_identified(context):
    ...
    assert results["deduplication_ratio"] == 1.0, \
...
def step_storage_efficiency_improved(context):
    ...
    assert efficiency_ratio <= 0.5, f"Efficiency ratio {efficiency_ratio} is above 50% threshold"
    ...
    context.diverse_content = [
        {
            ...
            "expected_keywords": ["user", "authentication", "secure", "access", "application"]
        },
        {
            "type": "code",
            "content": "def authenticate_user(username: str, password: str) -> bool:\n    return validate_credentials(username, password)",
            "expected_keywords": ["authenticate", "user", "username", "password", "validate"]
        },
        {
            "type": "structured",
            "content": {"user_id": 123, "role": "admin", "permissions": ["read", "write", "delete"]},
            "expected_keywords": ["user", "role", "admin", "permissions", "read", "write"]
        }
    ]
...
def step_topics_classified_correctly(context):
    ...
    for unit in context.enhanced_units:
        ...
        assert unit.metadata.topic != "general" or len(unit.metadata.keywords) < 3, \
...
def step_content_similarity_computable(context):
    ...
    auth_units = [unit for unit in context.enhanced_units if "auth" in str(unit.payload).lower()]
