...
@given(
    'the EDRR coordinator is initialized with enhanced phase transition features'
    )
def edrr_coordinator_initialized_with_enhanced_features(context):
    ...
    context.wsde_team = WSDETeam(name=
        'TestEdrrEnhancedPhaseTransitionsStepsTeam')
    ...
    context.edrr_coordinator = EDRRCoordinator(memory_manager=context.
        ...
        context.documentation_manager, enable_enhanced_logging=True, config
        ={'edrr': {'phase_transitions': {'adaptive_duration': True,
        'quality_thresholds': {'expand': 0.7, 'differentiate': 0.7,
        'refine': 0.8, 'retrospect': 0.8}, 'use_historical_data': True,
...
def task_with_high_complexity(context):
    ...
    context.task = {'id': 'complex-task-123', 'description':
        'Implement a distributed caching system with failover',
        'complexity_score': 8, 'estimated_effort': 'high'}
...
def start_edrr_cycle_with_complex_task(context):
    ...
    original_execute_differentiate = (context.edrr_coordinator.
        _execute_differentiate_phase)
    original_execute_refine = context.edrr_coordinator._execute_refine_phase
    original_execute_retrospect = (context.edrr_coordinator.
    ...
    context.edrr_coordinator._execute_expand_phase = track_duration('EXPAND',
        original_execute_expand)
    context.edrr_coordinator._execute_differentiate_phase = track_duration(
        'DIFFERENTIATE', original_execute_differentiate)
    context.edrr_coordinator._execute_refine_phase = track_duration('REFINE',
        original_execute_refine)
...
def verify_more_time_for_complex_phases(context):
    ...
    assert context.edrr_coordinator.performance_metrics['EXPAND'].get(
        'duration_adjustment_factor', 1.0) > 1.0
    assert context.edrr_coordinator.performance_metrics['DIFFERENTIATE'].get(
        'duration_adjustment_factor', 1.0) > 1.0
...
def verify_phase_has_extended_duration(context, phase_name):
    ...
    assert 'duration_adjustment_factor' in context.edrr_coordinator.performance_metrics[
        phase.name]
    assert context.edrr_coordinator.performance_metrics[phase.name][
...
def verify_durations_proportional_to_complexity(context):
    ...
    for phase in ['EXPAND', 'DIFFERENTIATE', 'REFINE', 'RETROSPECT']:
        if phase in context.edrr_coordinator.performance_metrics:
            adjustment_factor = context.edrr_coordinator.performance_metrics[
                phase].get('duration_adjustment_factor', 1.0)
            assert abs(adjustment_factor - (1.0 + complexity_score / 10.0)
                ) < 0.5
...
def verify_metrics_include_duration_adjustments(context):
    ...
    for phase in ['EXPAND', 'DIFFERENTIATE', 'REFINE', 'RETROSPECT']:
        if phase in context.edrr_coordinator.performance_metrics:
            assert 'duration_adjustment_factor' in context.edrr_coordinator.performance_metrics[
                phase]
            assert 'original_duration' in context.edrr_coordinator.performance_metrics[
                phase]
            assert 'adjusted_duration' in context.edrr_coordinator.performance_metrics[
                phase]
...
def edrr_coordinator_with_detailed_metrics(context):
    ...
    context.edrr_coordinator.config['edrr']['metrics'] = {'detailed_tracking':
        True, 'quality_metrics': True, 'completion_percentage': True,
        'resource_utilization': True, 'effectiveness_score': True}
...
def complete_full_edrr_cycle_with_metrics(context):
    ...
    context.task = {'id': 'metrics-task-123', 'description':
        'Implement a feature with metrics tracking'}
    context.edrr_coordinator.start_cycle(context.task)
    context.edrr_coordinator.results[Phase.EXPAND] = {'completed': True,
        ...
        'metrics': {'quality_score': 0.85, 'completion_percentage': 100,
        'resource_utilization': 0.65, 'effectiveness_score': 0.78}}
    ...
    context.edrr_coordinator.results[Phase.DIFFERENTIATE] = {'completed': 
        ...
        'metrics': {'quality_score': 0.82, 'completion_percentage': 100,
        'resource_utilization': 0.7, 'effectiveness_score': 0.75}}
    ...
    context.edrr_coordinator.results[Phase.REFINE] = {'completed': True,
        ...
        'description': 'Implemented solution'}, 'metrics': {'quality_score':
        0.88, 'completion_percentage': 100, 'resource_utilization': 0.75,
    ...
    context.edrr_coordinator.results[Phase.RETROSPECT] = {'completed': True,
        ...
        'is_valid': True, 'metrics': {'quality_score': 0.9,
        'completion_percentage': 100, 'resource_utilization': 0.6,
    ...
    for phase in [Phase.EXPAND, Phase.DIFFERENTIATE, Phase.REFINE, Phase.
        ...
        if phase.name in context.edrr_coordinator.results:
            context.phase_metrics[phase.name
                ] = context.edrr_coordinator.results[phase.name].get('metrics',
                {})
...
ReqID: N/A"""
        if data_type == 'PHASE_METRICS':
            test_storage[edrr_phase] = {'data': data, 'data_type':
                data_type, 'metadata': metadata}
    ...
    try:
        for phase in [Phase.EXPAND, Phase.DIFFERENTIATE, Phase.REFINE,
            Phase.RETROSPECT]:
            if (phase.name in context.edrr_coordinator.results and 
                'metrics' in context.edrr_coordinator.results[phase.name]):
                context.memory_manager.store_with_edrr_phase(context.
                    edrr_coordinator.results[phase.name]['metrics'],
                    'PHASE_METRICS', phase.name, {'cycle_id': context.
                    edrr_coordinator.cycle_id})
        for phase in [Phase.EXPAND, Phase.DIFFERENTIATE, Phase.REFINE,
            Phase.RETROSPECT]:
...
def edrr_coordinator_with_quality_thresholds(context):
    ...
    context.quality_thresholds = {'EXPAND': 0.7, 'DIFFERENTIATE': 0.7,
        'REFINE': 0.8, 'RETROSPECT': 0.8}
    context.edrr_coordinator.config['edrr']['phase_transitions'][
        'quality_thresholds'] = {'expand': context.quality_thresholds[
        'EXPAND'], 'differentiate': context.quality_thresholds[
        'DIFFERENTIATE'], 'refine': context.quality_thresholds['REFINE'],
...
def phase_produces_low_quality_results(context, phase_name):
    ...
    phase = Phase[phase_name.upper()]
    context.task = {'id': 'quality-task-123', 'description':
        'Implement a feature with quality checks'}
    context.edrr_coordinator.start_cycle(context.task)
    context.edrr_coordinator.results[phase] = {'completed': True,
        ...
        'Low quality approach', 'code': 'def approach1(): pass'}]}
...
@then(
    'the coordinator should trigger additional processing in the current phase'
    )
def verify_additional_processing_triggered(context):
    """Verify that additional processing is triggered in the current phase."""
    assert 'additional_processing' in context.edrr_coordinator.results[Phase
        .EXPAND]
    assert context.edrr_coordinator.results[Phase.EXPAND][
        'additional_processing'] is True
...
def verify_quality_issue_logged(context):
    ...
    assert len(context.edrr_coordinator.results[Phase.EXPAND]['quality_issues']
        ) > 0
    assert 'remediation_attempts' in context.edrr_coordinator.results[Phase
        .EXPAND]
    assert len(context.edrr_coordinator.results[Phase.EXPAND][
...
ReqID: N/A"""
    ...
    try:
        ...
        assert 'quality_score' in test_storage['DIFFERENTIATE']['metadata'][
            'quality_metrics']
    finally:
        context.memory_manager.store_with_edrr_phase = original_store_method
...
def edrr_coordinator_with_completed_cycles(context):
    ...
    context.historical_data = {'similar_task_1': {'cycle_id': 'cycle-001',
        'task': {'id': 'task-001', 'description': 'Similar task 1'},
        'phases': {'EXPAND': {'duration': 10, 'quality_score': 0.85,
        'strategies': ['strategy_a', 'strategy_b']}, 'DIFFERENTIATE': {
        'duration': 15, 'quality_score': 0.82, 'strategies': ['strategy_c']
        }, 'REFINE': {'duration': 20, 'quality_score': 0.88, 'strategies':
        ['strategy_d', 'strategy_e']}, 'RETROSPECT': {'duration': 8,
        'quality_score': 0.9, 'strategies': ['strategy_f']}}, 'success': 
        True}, 'similar_task_2': {'cycle_id': 'cycle-002', 'task': {'id':
        'task-002', 'description': 'Similar task 2'}, 'phases': {'EXPAND':
        {'duration': 12, 'quality_score': 0.8, 'strategies': ['strategy_b',
        'strategy_g']}, 'DIFFERENTIATE': {'duration': 18, 'quality_score': 
        0.78, 'strategies': ['strategy_h']}, 'REFINE': {'duration': 25,
        'quality_score': 0.85, 'strategies': ['strategy_i']}, 'RETROSPECT':
        {'duration': 10, 'quality_score': 0.88, 'strategies': ['strategy_j'
        ]}}, 'success': True}}
    for task_id, data in context.historical_data.items():
        context.memory_manager.store(data, 'HISTORICAL_CYCLE_DATA', {
            'task_id': task_id})
    original_search = context.memory_manager.search

    def search_with_historical_data(query=None, memory_type=None, metadata=
        None, limit=None):
        if memory_type == 'HISTORICAL_CYCLE_DATA':
            return [{'id': task_id, 'content': data} for task_id, data in
                context.historical_data.items()]
...
def start_new_cycle_with_similar_task(context):
    ...
    context.task = {'id': 'similar-task-123', 'description':
        'Implement a feature similar to previous tasks', 'tags': ['similar',
        'feature']}
    context.edrr_coordinator.start_cycle(context.task)


@then(
    'the coordinator should use historical data to optimize phase transitions')
...
def verify_criteria_adjusted_based_on_patterns(context):
    ...
    for phase in ['EXPAND', 'DIFFERENTIATE', 'REFINE', 'RETROSPECT']:
        ...
        assert 'strategies' in context.edrr_coordinator._adjusted_criteria[
            phase]
        ...
        for data in context.historical_data.values():
            if phase in data['phases'] and 'strategies' in data['phases'][phase
                ...
                historical_strategies.update(data['phases'][phase][
                    'strategies'])
...
def verify_effective_strategies_prioritized(context):
    ...
    for phase in ['EXPAND', 'DIFFERENTIATE', 'REFINE', 'RETROSPECT']:
        if phase in context.edrr_coordinator._adjusted_criteria:
            assert 'prioritized_strategies' in context.edrr_coordinator._adjusted_criteria[
                phase]
            assert len(context.edrr_coordinator._adjusted_criteria[phase][
                'prioritized_strategies']) > 0
            for strategy in context.edrr_coordinator._adjusted_criteria[phase][
                'prioritized_strategies']:
                found = False
                for data in context.historical_data.values():
                    if data['success'] and phase in data['phases'
...
ReqID: N/A"""
    ...
    try:
        ...
        assert 'historical_data_references' in test_storage['EXPAND'][
            'metadata']
        assert len(test_storage['EXPAND']['metadata'][
            'historical_data_references']) > 0
        historical_cycle_ids = [data['cycle_id'] for data in context.
...
def edrr_coordinator_in_specific_phase(context, phase_name):
    ...
    phase = Phase[phase_name.upper()]
    context.task = {'id': 'context-task-123', 'description':
        'Implement a feature with context preservation'}
    context.edrr_coordinator.start_cycle(context.task)
    if phase == Phase.EXPAND:
        context.edrr_coordinator.results[Phase.EXPAND] = {'completed': True,
            ...
            'def approach2(): pass'}], 'key_insights': [{'id': 'insight-1',
            'description': 'Important insight 1'}, {'id': 'insight-2',
            'description': 'Important insight 2'}], 'constraints': [{'id':
            'constraint-1', 'description':
            'Must be compatible with Python 3.8+'}, {'id': 'constraint-2',
            'description': 'Must handle edge cases gracefully'}]}
        context.context_preservation['key_insights'
            ] = context.edrr_coordinator.results[Phase.EXPAND]['key_insights']
        context.context_preservation['constraints'
            ] = context.edrr_coordinator.results[Phase.EXPAND]['constraints']
        context.context_preservation['approaches'
            ] = context.edrr_coordinator.results[Phase.EXPAND]['approaches']
    context.edrr_coordinator.progress_to_phase(phase)
...
@then(
    'the context should include all key insights from the "{phase_name}" phase'
    )
def verify_context_includes_key_insights(context, phase_name):
    ...
    phase = Phase[phase_name.upper()]
    assert 'key_insights' in context.edrr_coordinator._preserved_context[phase
        .name]
...
def verify_context_includes_constraints(context, phase_name):
    ...
    phase = Phase[phase_name.upper()]
    assert 'constraints' in context.edrr_coordinator._preserved_context[phase
        .name]
    preserved_constraints = context.edrr_coordinator._preserved_context[phase
        .name]['constraints']
...
@then(
    'the context should include all approaches generated in the "{phase_name}" phase'
    )
def verify_context_includes_approaches(context, phase_name):
    ...
    phase = Phase[phase_name.upper()]
    assert 'approaches' in context.edrr_coordinator._preserved_context[phase
        .name]
    preserved_approaches = context.edrr_coordinator._preserved_context[phase
        .name]['approaches']
...
@then(
    'the "{phase_name}" phase should have access to this comprehensive context'
    )
