...
def documentation_ingestion_system_initialized(context):
    ...
    context.ingestion_manager = DocumentationIngestionManager(
        storage_path=context.base_path
    )
...
def memory_system_available(context):
    ...
    context.ingestion_manager.set_memory_manager(context.memory_manager)
...
def have_url_documentation(context):
    ...
    # Mock the URL fetching in the ingestion manager
    original_fetch_from_url = context.ingestion_manager.fetch_from_url
    
    def mock_fetch_from_url(url):
        if url == "https://example.com/docs":
            return "# Sample Documentation\n\nThis is sample documentation from a URL."
        return original_fetch_from_url(url)
    
    context.ingestion_manager.fetch_from_url = mock_fetch_from_url
...
def ingest_markdown_documentation(context):
    ...
    if markdown_dir:
        context.ingestion_manager.ingest_from_directory(
            directory_path=markdown_dir,
            file_extension=".md",
...
def ingest_url_documentation(context):
    ...
    if url:
        context.ingestion_manager.ingest_from_url(
            url=url,
            format_type="markdown"  # Assuming the URL returns Markdown content
        )
...
def verify_documentation_stored(context):
    ...
    docs = context.memory_manager.search({"type": MemoryType.DOCUMENTATION})
...
def verify_markdown_query(context):
    ...
    query_results = context.memory_manager.search({
...
def verify_different_query_methods(context):
    ...
    if hasattr(context.ingestion_manager, "semantic_search"):
        semantic_results = context.ingestion_manager.semantic_search("example documentation")
