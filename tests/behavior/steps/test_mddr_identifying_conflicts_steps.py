...
def team_analyzes_perspectives(context):
    ...
    context.conflicts_result = context.team.analyze_disciplinary_perspectives(
        context.perspectives
    )
        ...
        for discipline, assumption in conflict["assumptions"].items():
            ...
            if discipline == "security":
                assert any(
                    ...
                    for term in ["secure", "attack", "protect", "risk"]
                )
            elif discipline == "user_experience":
                assert any(
                    term in assumption.lower()
                    for term in ["user", "experience", "usability", "interface"]
                )
            elif discipline == "performance":
                assert any(
                    term in assumption.lower()
                    for term in ["performance", "speed", "response", "time"]
                )
            elif discipline == "accessibility":
                assert any(
                    term in assumption.lower()
                    for term in ["accessibility", "disability", "access", "user"]
                )
