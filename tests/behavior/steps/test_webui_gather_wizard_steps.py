...
scenarios("../features/webui_gather_wizard.feature")
...
def click_wizard_next(wizard_context):
    ...
    if validation_passes:
        ...
        wizard_context["ui"].display_result("Error: Please fill in all required fields", message_type="error")
...
def check_wizard_step(wizard_context, step):
    ...
    wizard_context["st"].write.assert_any_call(f"Step {step} of 3: {get_step_title(step)}")
...
def enter_resource_metadata(wizard_context):
    ...
    wizard_context["st"].text_input.side_effect = ["Test User", "1.0", "test,documentation"]
...
def check_validation_errors(wizard_context):
    ...
    wizard_context["ui"].display_result.assert_called()
    assert "error" in wizard_context["ui"].display_result.call_args[0][0].lower() or "validation" in wizard_context["ui"].display_result.call_args[0][0].lower()
...
def enter_custom_resource_information(wizard_context):
    ...
    wizard_context["st"].text_input.side_effect = ["/path/to/custom", "custom_value", "another_value"]
