...
def context():
    class Context:
        def __init__(self):
            ...
            self.cli_command_reference = self.user_guides_dir / 'cli_command_reference.md'
            self.webui_navigation_guide = self.user_guides_dir / 'webui_navigation_guide.md'
            ...
            self.configuration_reference = self.user_guides_dir / 'configuration_reference.md'
...
def docs_system_available(context):
    ...
    assert context.user_guides_dir.exists(), f"User guides directory {context.user_guides_dir} does not exist"
...
def each_command_lists_options(context):
    command_sections = re.findall(r'### ([a-zA-Z-]+).*?\*\*Options:\*\*\n\| Option \| Description \|', 
                                 context.current_doc_content, re.DOTALL)
...
def webui_guide_provides_navigation_instructions(context):
    ...
    navigation_sections = re.findall(r'Navigation:.*?', context.current_doc_content, re.DOTALL)
...
def configuration_reference_documents_all_options(context):
    ...
    expected_sections = [
        "LLM Configuration", "Environment Variables", "Project Configuration", 
        "Development Configuration", "Memory Configuration", "UXBridge Configuration", 
        "Feature Flags"
    ]
...
def configuration_reference_explains_config_files(context):
    ...
    assert "Configuration File" in context.current_doc_content, "No configuration file section found in configuration reference"
