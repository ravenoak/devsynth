    ...
    for perspective in context.result["disciplinary_perspectives"]:
        ...
        for source in perspective["knowledge_sources"]:
            ...
            key_points_reflected = any(
                any(term.lower() in perspective_text for term in key_point.lower().split())
                for key_point in source["key_points_used"]
            )
            assert key_points_reflected, f"Perspective does not reflect knowledge from {source['title']}"
...
def knowledge_properly_attributed_to_authoritative_sources(context):
    ...
    for perspective in context.result["disciplinary_perspectives"]:
        ...
        assert any(source.lower() in perspective_text for source in source_names), f"No sources mentioned in perspective: {perspective['perspective']}"
        
        # Verify that the disciplinary context mentions the sources
        context_text = perspective["disciplinary_context"].lower()
        assert any(source.lower() in context_text for source in source_names), f"No sources mentioned in disciplinary context: {perspective['disciplinary_context']}"
        ...
        for source in perspective["knowledge_sources"]:
            ...
            if perspective["discipline"] == "security":
                assert any(auth in source_name for auth in ["owasp", "nist", "iso", "cert", "sans"]), f"Not an authoritative security source: {source['source']}"
            elif perspective["discipline"] == "user_experience":
                assert any(auth in source_name for auth in ["nielsen", "norman", "baymard", "ux", "usability"]), f"Not an authoritative UX source: {source['source']}"
            elif perspective["discipline"] == "performance":
                assert any(auth in source_name for auth in ["web.dev", "google", "mdn", "mozilla", "performance"]), f"Not an authoritative performance source: {source['source']}"
    ...
    for discipline in disciplines:
        ...
        for item in integrated_knowledge[discipline]:
            ...
            if discipline == "security":
                assert any(std in item_lower for std in ["owasp", "nist", "iso", "standard", "best practice"]), f"Security item doesn't reference standards: {item}"
            elif discipline == "user_experience":
                assert any(std in item_lower for std in ["nielsen", "norman", "baymard", "research", "recommended"]), f"UX item doesn't reference standards: {item}"
            elif discipline == "performance":
                assert any(std in item_lower for std in ["web.dev", "mdn", "optimized", "recommended"]), f"Performance item doesn't reference standards: {item}"
...
def solution_demonstrates_awareness_of_cross_disciplinary_implications(context):
    ...
    for implication in implications:
        ...
        mentioned_disciplines = sum(1 for discipline in disciplines if discipline in implication_lower)
        assert mentioned_disciplines >= 2, f"Implication doesn't mention at least two disciplines: {implication}"
        
        # Verify that the implication describes how disciplines interact
        interaction_terms = ["with", "while", "maintain", "follow", "consider", "balance", "trade-off"]
        assert any(term in implication_lower for term in interaction_terms), f"Implication doesn't describe discipline interaction: {implication}"
        
        # Verify that the implication mentions specific standards or practices
        standards_terms = ["best practice", "standard", "guideline", "recommended", "compliance", "owasp", "nist", "wcag", "nielsen"]
        assert any(term in implication_lower for term in standards_terms), f"Implication doesn't mention specific standards: {implication}"