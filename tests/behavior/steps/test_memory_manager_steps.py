...
def perform_similarity_search(context, embedding):
    ...
    context.search_results = context.memory_manager.similarity_search(query_embedding, top_k=3)
...
@when(parsers.parse('I store a memory item with content "{content}", type "{memory_type}", and EDRR phase "{edrr_phase}"'))
...
@then(parsers.parse('the retrieved memory item should have metadata with "{key}" set to "{value}"'))
...
@when(parsers.parse('I query the graph for the evolution of item "{item_id}" across EDRR phases'))
def query_graph_for_evolution_across_edrr_phases(context, item_id):
    ...
    context.search_results = context.memory_manager.query_evolution_across_edrr_phases(item_id)
