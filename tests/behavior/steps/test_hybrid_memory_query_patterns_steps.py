...
@pytest.mark.medium
@when(    parsers.parse(        'I perform a direct query to the Graph store for relationships of "{item_id}"'    ))
...
@pytest.mark.medium
@when(    parsers.parse(        'I perform a cascading query starting with "{query}" in the Document store'    ))
...
@pytest.mark.medium
@then(    "then follow references to retrieve related code implementations from the Vector store")
