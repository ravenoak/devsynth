...
def evaluation_verifies_compliance_with_discipline_specific_standards(context):
    ...
    for discipline, assessment in context.evaluation["disciplinary_assessments"].items():
        ...
        if discipline == "security":
            ...
            assert any(any(std in standard for std in security_standards) for standard in standards)
        elif discipline == "user_experience":
            ux_standards = ["Nielsen", "Heuristic", "Design", "UX"]
            assert any(any(std in standard for std in ux_standards) for standard in standards)
        elif discipline == "performance":
            performance_standards = ["Performance", "Web", "Speed", "MDN"]
            assert any(any(std in standard for std in performance_standards) for standard in standards)
        elif discipline == "accessibility":
            accessibility_standards = ["WCAG", "WebAIM", "A11y", "Accessibility"]
            assert any(any(std in standard for std in accessibility_standards) for standard in standards)
    ...
    for discipline, assessment in context.evaluation["disciplinary_assessments"].items():
        ...
        if assessment["concerns"]:
            for concern in assessment["concerns"]:
                ...
                if discipline == "security":
                    assert any(term in concern.lower() for term in ["security", "secure", "protect", "risk", "attack"])
                elif discipline == "user_experience":
                    assert any(term in concern.lower() for term in ["user", "experience", "usability", "interface", "friction"])
                elif discipline == "performance":
                    assert any(term in concern.lower() for term in ["performance", "speed", "response", "time", "load"])
                elif discipline == "accessibility":
...
def evaluation_provides_overall_assessment_of_multi_disciplinary_quality(context):
    ...
    assert overall["rating"] in ["Excellent", "Good", "Satisfactory", "Needs Improvement", "Poor"]
    ...
    discipline_terms = ["security", "user", "experience", "performance", "accessibility"]
    summary_lower = overall["summary"].lower()
    assert sum(1 for term in discipline_terms if term in summary_lower) >= 3, "Summary should mention at least 3 disciplines"