    ...
    for expected_item in context.layer_contents["short-term"]:
        ...
        for retrieved_item in items:
            if retrieved_item.id == expected_item.id:
                found = True
                break
        assert found, f"Item with ID '{expected_item.id}' not retrieved from short-term memory layer"
    ...
    for expected_item in context.layer_contents["episodic"]:
        ...
        for retrieved_item in items:
            if retrieved_item.id == expected_item.id:
                found = True
                break
        assert found, f"Item with ID '{expected_item.id}' not retrieved from episodic memory layer"
    ...
    for expected_item in context.layer_contents["semantic"]:
        ...
        for retrieved_item in items:
            if retrieved_item.id == expected_item.id:
                found = True
                break
        assert found, f"Item with ID '{expected_item.id}' not retrieved from semantic memory layer"
...
def check_results_from_specific_layer(context, layer):
    ...
    for item in context.query_results:
        if layer == "short-term":
            assert item.memory_type == MemoryType.CONTEXT, f"Item with type {item.memory_type} found in short-term query results"
        elif layer == "episodic":
            assert item.memory_type == MemoryType.TASK_HISTORY, f"Item with type {item.memory_type} found in episodic query results"
        elif layer == "semantic":
            assert item.memory_type == MemoryType.KNOWLEDGE, f"Item with type {item.memory_type} found in semantic query results"
