    ...
    if "--path" in args:
        ...
        if path_index + 1 < len(args):
            ...
            if context.path == "./non_existent_dir":
                mock_inspect_code_cmd.side_effect = FileNotFoundError(f"Directory not found: {context.path}")
...
def report_contains_all_metrics(context):
    ...
    assert context.report_file is not None, "Report file was not specified"
    assert os.path.exists(context.report_path), f"Report file does not exist at {context.report_path}"
...
def path_not_exist_error_displayed(context):
    ...
    assert "Directory not found" in context.error_message, "Error message does not indicate directory not found"