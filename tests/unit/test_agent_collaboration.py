...
ReqID: N/A"""
        task = {'task_type': 'specification', 'requirements':
...
ReqID: N/A"""
        coordinator = AgentCoordinatorImpl({'features': {
            'wsde_collaboration': True, 'collaboration_notifications': True}})
        ...
        team.build_consensus = MagicMock(return_value={'consensus': 'final',
            'contributors': ['planner', 'worker'], 'method': 'consensus'})
        ...
        team.broadcast_message.assert_any_call('planner', 'task_assignment',
            subject='Team task started', content=task, metadata={'phase':
            'expand'})
        team.broadcast_message.assert_any_call('system', 'status_update',
            subject='Team task completed', content=team.build_consensus.
            return_value, metadata={'phase': 'expand'})
...
ReqID: N/A"""
        ...
        a1.name = 'a1'
        a1.config = AgentConfig(name='a1', agent_type=AgentType.
            ORCHESTRATOR, description='', capabilities=[], parameters={
            'expertise': ['code']})
        a1.process.return_value = {'result': 'sol1'}
        a2 = MagicMock(spec=UnifiedAgent)
        a2.name = 'a2'
        a2.config = AgentConfig(name='a2', agent_type=AgentType.
            ORCHESTRATOR, description='', capabilities=[], parameters={
            'expertise': ['test']})
        ...
        team.add_solution = MagicMock()
        team.build_consensus = MagicMock(return_value={'consensus': 'final',
            'contributors': ['a1', 'a2'], 'method': 'consensus',
            'reasoning': ''})
...
ReqID: N/A"""
        ...
        team.get_primus = MagicMock(return_value=a1)
        team.add_solution = MagicMock()
        team.build_consensus = MagicMock(return_value={'consensus': 'ok',
            'contributors': ['a1', 'a2'], 'method': 'consensus',
            'reasoning': ''})
        team.apply_enhanced_dialectical_reasoning_multi = MagicMock(
...
ReqID: N/A"""
        ...
        agent = MagicMock(spec=UnifiedAgent)
        agent.name = 'a'
        agent.config = AgentConfig(name='a', agent_type=AgentType.
            ORCHESTRATOR, description='', capabilities=[], parameters={})
        coordinator.add_agent(agent)
        coordinator.add_agent(agent)
        team = coordinator.get_team('team')
...
ReqID: N/A"""
        ...
        with patch.object(team, 'select_primus_by_expertise', side_effect=
            select_spy) as spy:
            ...
            team.get_primus = MagicMock(return_value=a1)
            team.add_solution = MagicMock()
            team.build_consensus = MagicMock(return_value={'consensus': 'x',
                'contributors': ['a1', 'a2'], 'method': 'consensus'})
            result = coordinator.delegate_task({'team_task': True, 'type':
                'code'})
