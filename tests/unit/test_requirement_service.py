...
from devsynth.domain.models.requirement import ChangeType, Requirement, RequirementChange, RequirementPriority, RequirementStatus, RequirementType
from devsynth.application.requirements.requirement_service import RequirementService
from devsynth.adapters.requirements.memory_repository import InMemoryChangeRepository, InMemoryRequirementRepository
...
ReqID: N/A"""
    ...
    def setUp(self):
        ...
        self.service = RequirementService(requirement_repository=self.
            requirement_repository, change_repository=self.
            change_repository, dialectical_reasoner=self.
            dialectical_reasoner, notification_service=self.
...
ReqID: N/A"""
        requirement = Requirement(title='Test Requirement', description=
            ...
            FUNCTIONAL)
        created_requirement = self.service.create_requirement(requirement,
            'test_user')
        self.assertEqual(created_requirement.title, 'Test Requirement')
        self.assertEqual(created_requirement.description,
            'This is a test requirement')
        self.assertEqual(created_requirement.status, RequirementStatus.DRAFT)
        self.assertEqual(created_requirement.priority, RequirementPriority.
            MEDIUM)
...
ReqID: N/A"""
        ...
        saved_requirement = self.requirement_repository.save_requirement(
            requirement)
        updates = {'title': 'Updated Test Requirement', 'description':
            'This is an updated test requirement', 'status':
            RequirementStatus.PROPOSED, 'priority': RequirementPriority.HIGH}
        updated_requirement = self.service.update_requirement(saved_requirement
            .id, updates, 'test_user', 'Testing update')
        self.assertEqual(updated_requirement.title, 'Updated Test Requirement')
        self.assertEqual(updated_requirement.description,
            'This is an updated test requirement')
...
ReqID: N/A"""
        ...
        saved_requirement = self.requirement_repository.save_requirement(
            requirement)
        deleted = self.service.delete_requirement(saved_requirement.id,
            'test_user', 'Testing deletion')
...
ReqID: N/A"""
        ...
        saved_requirement = self.requirement_repository.save_requirement(
            requirement)
        change = RequirementChange(requirement_id=saved_requirement.id,
            change_type=ChangeType.MODIFY, previous_state=saved_requirement,
            new_state=Requirement(id=saved_requirement.id, title=
            'Updated Test Requirement', description=
            ...
            ='test_user', reason='Testing approval')
        saved_change = self.change_repository.save_change(change)
        approved_change = self.service.approve_change(saved_change.id,
            'approver_user')
...
ReqID: N/A"""
        ...
        change = RequirementChange(requirement_id=saved_requirement.id,
            ...
            .PROPOSED, priority=RequirementPriority.HIGH, type=
            RequirementType.FUNCTIONAL, created_by='test_user'), created_by
            ='test_user', reason='Testing rejection')
        saved_change = self.change_repository.save_change(change)
        rejected_change = self.service.reject_change(saved_change.id,
            'rejector_user', 'This change is not necessary')
        self.assertFalse(rejected_change.approved)
        self.assertEqual(len(rejected_change.comments), 1)
        self.assertEqual(rejected_change.comments[0],
            'rejector_user: This change is not necessary')
