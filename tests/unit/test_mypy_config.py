...
ReqID: N/A"""
    ...
    try:
        result = subprocess.run(['mypy', temp_file_path], capture_output=
            True, text=True, check=False)
        assert result.returncode != 0, 'mypy should fail on type errors'
        assert 'Argument 1 to "add" has incompatible type' in result.stdout, f'mypy did not detect the type error. Output: {result.stdout}'
    finally:
...
ReqID: N/A"""
    ...
    try:
        ...
        result = subprocess.run(['mypy', '--config-file', config_file_path,
            temp_file_path], capture_output=True, text=True, check=False)
        ...
        assert 'Function is missing a type annotation' in result.stdout, f"mypy did not detect the untyped function definition error, which suggests it's not using the configuration correctly. Output: {result.stdout}"
    finally:
        os.unlink(temp_file_path)
