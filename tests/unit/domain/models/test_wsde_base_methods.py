...
class TestWSDEBaseMethods:
    ...
    def test_register_dialectical_hook(self):
        """Test registering a dialectical hook."""
        # Arrange
        hook_called = False
        
        def test_hook(thesis, antitheses):
        ...
        # Verify the hook is called when used
    ...
    def test_send_message(self):
        ...
        # Act
        message_id = self.team.send_message(
            sender="agent1",
    ...
    def test_broadcast_message(self):
        ...
        # Act
        message_id = self.team.broadcast_message(
            sender="agent1",
        ...
        message = self.team.messages[0]
        assert message["sender"] == "agent1"
        assert message["recipients"] == ["agent2", "agent3"]  # All agents except sender
    ...
    def test_get_messages(self):
        ...
        # Act - Get messages for a specific agent
        agent1_messages = self.team.get_messages(agent="agent1")
        
        # Assert
        assert len(agent1_messages) == 3  # agent1 is involved in all messages (as sender or recipient)
        
        # Act - Get messages with filters
        type1_messages = self.team.get_messages(filters={"message_type": "type1"})
    ...
    def test_conduct_peer_review(self):
        ...
        with patch.object(self.team, 'request_peer_review') as mock_request:
            ...
            mock_review.collect_reviews.return_value = {
                "agent2": {"feedback": "Looks good", "approved": True},
                "agent3": {"feedback": "Needs improvement", "approved": False}
            }
            mock_review.aggregate_feedback.return_value = {
                "overall_approved": False,
                "feedback": ["Looks good", "Needs improvement"]
            }
            mock_request.return_value = mock_review
            
            # Act
            result = self.team.conduct_peer_review(
                work_product=work_product,
                author=self.agent1,
                reviewer_agents=[self.agent2, self.agent3]
            )
