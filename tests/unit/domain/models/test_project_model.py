...
from devsynth.domain.models.project import ProjectModel, Artifact, ArtifactType, ProjectStructureType
...
ReqID: N/A"""
        artifact = Artifact('/test/path/file.py', ArtifactType.CODE, {'key':
            'value'})
        assert repr(artifact
...
ReqID: N/A"""
    ...
    def basic_manifest_data(self):
        ...
        return {'projectName': 'TestProject', 'version': '0.1.0',
            'structure': {'type': 'single_package', 'directories': {
            'source': ['src'], 'tests': ['tests'], 'docs': ['docs']}}}

    @pytest.fixture
    def monorepo_manifest_data(self):
        """Create monorepo manifest data for testing."""
        return {'projectName': 'TestMonorepo', 'version': '0.1.0',
            'structure': {'type': 'monorepo', 'directories': {'source': [
            'packages'], 'docs': ['docs']}, 'customLayouts': {'type':
            'monorepo', 'packagesRoot': 'packages', 'packages': [{'name':
            'package1', 'path': 'package1', 'source': 'src', 'tests':
            'tests'}, {'name': 'package2', 'path': 'package2', 'source':
            'src', 'tests': 'tests'}]}}}

    def test_project_model_initialization_succeeds(self, mock_project_root,
        basic_manifest_data):
...
ReqID: N/A"""
        model = ProjectModel(mock_project_root, {'structure': {'type':
            'single_package'}})
        assert model.structure_type == ProjectStructureType.STANDARD
        model = ProjectModel(mock_project_root, {'structure': {'type':
            'monorepo'}})
    ...
    def test_build_standard_model_succeeds(self, mock_glob, mock_is_dir,
        mock_exists, mock_project_root, basic_manifest_data):
...
ReqID: N/A"""
        ...
        assert model.artifacts[str(src_path)
            ].artifact_type == ArtifactType.CODE
        assert model.artifacts[str(tests_path)
            ].artifact_type == ArtifactType.TEST
        assert model.artifacts[str(docs_path)
            ].artifact_type == ArtifactType.DOCUMENTATION

    @patch('pathlib.Path.exists')
    @patch('pathlib.Path.is_dir')
    @patch('pathlib.Path.glob')
    def test_build_monorepo_model_succeeds(self, mock_glob, mock_is_dir,
        mock_exists, mock_project_root, monorepo_manifest_data):
...
ReqID: N/A"""
        ...
        assert model.artifacts[str(package1_src)
            ].artifact_type == ArtifactType.CODE
        assert model.artifacts[str(package1_tests)
            ].artifact_type == ArtifactType.TEST
        assert model.artifacts[str(package2_src)
            ].artifact_type == ArtifactType.CODE
        assert model.artifacts[str(package2_tests)
            ].artifact_type == ArtifactType.TEST
...
ReqID: N/A"""
    ...
    def test_get_artifacts_by_type_succeeds(self, mock_project_root,
        basic_manifest_data):
...
ReqID: N/A"""
    ...
    def test_get_related_artifacts_succeeds(self, mock_project_root,
        basic_manifest_data):
...
ReqID: N/A"""
    ...
    def test_determine_artifact_type_succeeds(self, mock_project_root,
        basic_manifest_data):
        """Test determining artifact type based on file extension and path.

ReqID: N/A"""
        model = ProjectModel(mock_project_root, basic_manifest_data)
        assert model._determine_artifact_type(Path('file.py'), ArtifactType
            .UNKNOWN) == ArtifactType.CODE
        assert model._determine_artifact_type(Path('file.js'), ArtifactType
            .UNKNOWN) == ArtifactType.CODE
        assert model._determine_artifact_type(Path('file.java'),
            ArtifactType.UNKNOWN) == ArtifactType.CODE
        assert model._determine_artifact_type(Path('file.md'), ArtifactType
            .UNKNOWN) == ArtifactType.DOCUMENTATION
        assert model._determine_artifact_type(Path('file.rst'),
            ArtifactType.UNKNOWN) == ArtifactType.DOCUMENTATION
        assert model._determine_artifact_type(Path('file.txt'),
            ArtifactType.UNKNOWN) == ArtifactType.DOCUMENTATION
        assert model._determine_artifact_type(Path('file.json'),
            ArtifactType.UNKNOWN) == ArtifactType.CONFIGURATION
        assert model._determine_artifact_type(Path('file.yaml'),
            ArtifactType.UNKNOWN) == ArtifactType.CONFIGURATION
        assert model._determine_artifact_type(Path('file.toml'),
            ArtifactType.UNKNOWN) == ArtifactType.CONFIGURATION
        assert model._determine_artifact_type(Path('file.sh'), ArtifactType
            .UNKNOWN) == ArtifactType.BUILD
        assert model._determine_artifact_type(Path('file.bat'),
            ArtifactType.UNKNOWN) == ArtifactType.BUILD
        assert model._determine_artifact_type(Path('test_file.py'),
            ArtifactType.UNKNOWN) == ArtifactType.TEST
        assert model._determine_artifact_type(Path('file_test.py'),
            ArtifactType.UNKNOWN) == ArtifactType.TEST
        assert model._determine_artifact_type(Path('file_spec.js'),
            ArtifactType.UNKNOWN) == ArtifactType.TEST
        assert model._determine_artifact_type(Path('unknown.xyz'),
...
ReqID: N/A"""
        ...
        model.artifacts[root_path] = Artifact(root_path, ArtifactType.
            UNKNOWN, {'is_root': True})
        model.artifacts[src_path] = Artifact(src_path, ArtifactType.CODE, {
            'role': 'source'})
        model.artifacts[file_path] = Artifact(file_path, ArtifactType.CODE,
            {'extension': '.py'})
        ...
        relationships = {(r['source'], r['target']): r['metadata'] for r in
