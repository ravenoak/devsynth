...
ReqID: N/A"""
        ...
        for role, method in [('Supervisor', self.team.get_supervisor), (
            'Designer', self.team.get_designer), ('Evaluator', self.team.
            get_evaluator)]:
            role_agent = None
            for agent in [self.agent2, self.agent3, self.agent4]:
...
ReqID: N/A"""
        ...
        self.team.primus_index = 0
        self.team.assign_roles()
        initial_roles = {self.agent1.name: self.agent1.current_role, self.
            agent2.name: self.agent2.current_role, self.agent3.name: self.
            agent3.current_role, self.agent4.name: self.agent4.current_role}
        self.team.rotate_primus()
        self.team.assign_roles()
        assert self.agent1.current_role != initial_roles[self.agent1.name]
        assert self.agent2.current_role == 'Primus'
        roles = [self.agent1.current_role, self.agent3.current_role, self.
            agent4.current_role]
        assert 'Worker' in roles
        assert 'Supervisor' in roles
        assert 'Designer' in roles or 'Evaluator' in roles
...
ReqID: N/A"""
    ...
    return username == 'admin' and password == 'password'"""
        ...
        mock_wsde_memory.query_knowledge_for_task.return_value = [{
            'concept': 'authentication', 'relevance': 0.9}, {'concept':
            'password', 'relevance': 0.8}, {'concept': 'security',
            'relevance': 0.7}]
