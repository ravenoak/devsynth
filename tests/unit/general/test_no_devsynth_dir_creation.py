...
ReqID: N/A"""
    ...
    def test_ensure_path_exists_respects_no_file_logging_succeeds(self,
        monkeypatch):
...
ReqID: N/A"""
    ...
    def test_settings_respects_no_file_logging_succeeds(self, monkeypatch,
        tmp_path):
...
ReqID: N/A"""
        ...
        assert str(test_project_dir
            ) in settings.memory_file_path, f'Memory path does not use project directory: {settings.memory_file_path}'
        assert '~/.devsynth' not in settings.log_dir, f'Log directory uses home directory: {settings.log_dir}'
        assert str(test_project_dir
            ) in settings.log_dir, f'Log directory does not use project directory: {settings.log_dir}'
