...
from test_first_metrics import get_commit_history, analyze_commit, calculate_metrics, generate_metrics_report, main
...
ReqID: N/A"""
    ...
    def setUp(self):
        ...
        self.metrics_file = os.path.join(self.temp_dir.name,
            'test_first_metrics.json')
        self.sample_commits = [{'hash': 'abc123', 'author': 'Test Author',
            'date': '2025-05-25', 'message': 'Add new feature', 'files': [{
            'path': 'src/devsynth/feature.py', 'status': 'A'}, {'path':
            'tests/unit/test_feature.py', 'status': 'A'}]}, {'hash':
            'def456', 'author': 'Test Author', 'date': '2025-05-26',
            'message': 'Fix bug', 'files': [{'path':
            'src/devsynth/module.py', 'status': 'M'}, {'path':
            'tests/unit/test_module.py', 'status': 'M'}]}, {'hash':
            'ghi789', 'author': 'Test Author', 'date': '2025-05-27',
            'message': 'Implement without test', 'files': [{'path':
            'src/devsynth/no_test.py', 'status': 'A'}]}]
        self.expected_metrics = {'total_commits': 3, 'test_first_commits': 
            2, 'implementation_first_commits': 1, 'test_first_percentage': 
            66.67, 'by_author': {'Test Author': {'total_commits': 3,
            'test_first_commits': 2, 'implementation_first_commits': 1,
            'test_first_percentage': 66.67}}, 'by_date': {'2025-05-25': {
            'total_commits': 1, 'test_first_commits': 1,
            'implementation_first_commits': 0, 'test_first_percentage': 
            100.0}, '2025-05-26': {'total_commits': 1, 'test_first_commits':
            1, 'implementation_first_commits': 0, 'test_first_percentage': 
            100.0}, '2025-05-27': {'total_commits': 1, 'test_first_commits':
            0, 'implementation_first_commits': 1, 'test_first_percentage': 
            0.0}}}
...
ReqID: N/A"""
        mock_process = MagicMock()
        mock_process.stdout = '\n'.join(['M\tsrc/devsynth/feature.py',
            'A\ttests/unit/test_feature.py'])
        mock_run.return_value = mock_process
        commit = {'hash': 'abc123', 'author': 'Test Author', 'date':
            '2025-05-25', 'message': 'Add new feature'}
...
ReqID: N/A"""
        ...
        self.assertAlmostEqual(metrics['test_first_percentage'], 66.67,
            places=2)
        self.assertEqual(metrics['by_author']['Test Author'][
            'total_commits'], 3)
        self.assertEqual(metrics['by_author']['Test Author'][
            'test_first_commits'], 2)
        self.assertEqual(metrics['by_author']['Test Author'][
            'implementation_first_commits'], 1)
        self.assertAlmostEqual(metrics['by_author']['Test Author'][
            'test_first_percentage'], 66.67, places=2)
        self.assertEqual(metrics['by_date']['2025-05-25']['total_commits'], 1)
        self.assertEqual(metrics['by_date']['2025-05-25'][
...
ReqID: N/A"""
    ...
    def test_main_succeeds(self, mock_print, mock_generate_report,
        mock_calculate_metrics, mock_analyze_commit, mock_get_history):
