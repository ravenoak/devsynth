...
from devsynth.adapters.requirements.memory_repository import InMemoryChatRepository, InMemoryChangeRepository, InMemoryDialecticalReasoningRepository, InMemoryImpactAssessmentRepository, InMemoryRequirementRepository
...
ReqID: N/A"""
    ...
    def setUp(self):
        ...
        self.reasoner = DialecticalReasonerService(requirement_repository=
            self.requirement_repository, reasoning_repository=self.
            reasoning_repository, impact_repository=self.impact_repository,
            chat_repository=self.chat_repository, notification_service=self
            .notification_service, llm_service=self.llm_service)
        self.requirement = Requirement(id=uuid4(), title='Test Requirement',
            description='This is a test requirement', status=
            RequirementStatus.DRAFT, priority=RequirementPriority.MEDIUM,
            type=RequirementType.FUNCTIONAL, created_by='test_user')
        self.requirement_repository.save_requirement(self.requirement)
        self.change = RequirementChange(id=uuid4(), requirement_id=self.
            requirement.id, change_type=ChangeType.MODIFY, previous_state=
            self.requirement, new_state=Requirement(id=self.requirement.id,
            title='Updated Test Requirement', description=
            'This is an updated test requirement', status=RequirementStatus
            .PROPOSED, priority=RequirementPriority.HIGH, type=
            RequirementType.FUNCTIONAL, created_by='test_user'), created_by
            ='test_user', reason='Testing purposes')
...
ReqID: N/A"""
        self.llm_service.query.side_effect = ['This is a thesis',
...
Recommendation: This is a recommendation"""
        ...
        reasoning_by_change = (self.reasoning_repository.
            get_reasoning_for_change(self.change.id))
...
ReqID: N/A"""
        self.llm_service.query.side_effect = ['This is an impact analysis',
            """- Recommendation 1
- Recommendation 2
- Recommendation 3"""]
        dependent_requirement = Requirement(id=uuid4(), title=
            'Dependent Requirement', description=
            'This requirement depends on the test requirement', status=
            RequirementStatus.DRAFT, priority=RequirementPriority.MEDIUM,
            type=RequirementType.FUNCTIONAL, created_by='test_user',
        ...
        impact_by_change = (self.impact_repository.
            get_impact_assessment_for_change(self.change.id))
...
ReqID: N/A"""
        ...
        reasoning = DialecticalReasoning(id=uuid4(), change_id=self.change.
            id, thesis='Test thesis', antithesis='Test antithesis',
            synthesis='Test synthesis', conclusion='Test conclusion',
            recommendation='Test recommendation', created_by='test_user')
        self.reasoning_repository.save_reasoning(reasoning)
        self.llm_service.query.return_value = (
            'Welcome to the dialectical reasoning system! We are discussing a change to the requirement.'
            )
        session_with_change = self.reasoner.create_session('test_user',
            self.change.id)
...
ReqID: N/A"""
        ...
        response = self.reasoner.process_message(session.id,
            'Hello, this is a test message.', 'test_user')
        ...
        self.assertEqual(updated_session.messages[1].content,
            'Hello, this is a test message.')
        self.assertEqual(updated_session.messages[2].sender, 'system')
        self.assertEqual(updated_session.messages[2].content,
            'This is a response to your message.')
