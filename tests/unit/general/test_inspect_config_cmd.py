...
@patch(
    'devsynth.application.cli.commands.inspect_config_cmd.analyze_project_structure'
    )
@patch(
    'devsynth.application.cli.commands.inspect_config_cmd.compare_with_manifest'
    )
@patch('devsynth.application.cli.commands.inspect_config_cmd.update_manifest')
@patch('yaml.dump')
def test_inspect_config_update_succeeds(mock_dump, mock_update,
    mock_compare, mock_analyze, mock_bridge, tmp_path):
...
ReqID: N/A"""
    ...
    mock_compare.return_value = [{'type': 'source', 'path': 'src', 'status':
        'missing in manifest'}]
    ...
    assert any('Configuration updated successfully' in str(call.args[0]) for
        call in mock_bridge.print.call_args_list)
...
@patch('yaml.dump')
def test_inspect_config_prune_succeeds(mock_dump, mock_prune, mock_compare,
    mock_analyze, mock_bridge, tmp_path):
...
ReqID: N/A"""
    ...
    yaml.safe_dump({'projectName': 'Test'}, manifest_path.open('w'))
    mock_analyze.return_value = {}
    mock_compare.return_value = [{'type': 'tests', 'path': 'tests',
    ...
    mock_dump.assert_called_once()
    assert any('Configuration pruned successfully' in str(call.args[0]) for
        call in mock_bridge.print.call_args_list)
