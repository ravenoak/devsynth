...
ReqID: N/A"""
        ...
        assert not parent_devsynth.exists(
...
ReqID: N/A"""
        ...
        with patch('os.path.expanduser') as mock_expanduser:
            ...
            assert not parent_config_dir.exists(
...
ReqID: N/A"""
        ...
        assert not original_memory_dir.exists(
            ), f'Memory directory was created in {original_memory_dir}'

    def test_no_file_logging_prevents_directory_creation_succeeds(self,
        monkeypatch):
...
ReqID: N/A"""
        ...
        try:
            ...
            def custom_ensure_path_exists(path, create=True):
                path_obj = Path(path)
                if path_obj.is_absolute() and not str(path_obj).startswith(str
                    (test_dir)):
                    if str(path_obj).startswith(str(Path.home())):
                        ...
                        new_path = os.path.join(test_dir, relative_path.
                            lstrip('/\\'))
                        print(
                            f'Redirecting home path {path} to test path {new_path}'
                            )
                        path = new_path
                    else:
                        relative_path = str(path_obj.relative_to(path_obj.
                            anchor))
                        new_path = os.path.join(test_dir, relative_path)
                        print(
                ...
                if path_obj.is_absolute() and not str(path_obj).startswith(str
                    ...
                    if str(path_obj).startswith(str(Path.home())):
                        ...
                        new_path = os.path.join(test_dir, relative_path.
                            lstrip('/\\'))
                        print(
                            f'Redirecting log path {log_dir} to test path {new_path}'
                            )
                        log_dir = new_path
                    else:
                        relative_path = str(path_obj.relative_to(path_obj.
            ...
            redirected_path = custom_ensure_path_exists(str(home_path),
                create=False)
            assert str(test_dir
                ) in redirected_path, f'Path was not redirected to test directory: {redirected_path}'
            assert str(home_path
                ) not in redirected_path, f'Path still contains home directory: {redirected_path}'
            home_log_path = Path.home() / '.devsynth' / 'logs'
            redirected_log_path = custom_ensure_log_dir_exists(str(
                home_log_path))
            assert str(test_dir
                ) in redirected_log_path, f'Log path was not redirected to test directory: {redirected_log_path}'
            assert str(home_log_path
                ) not in redirected_log_path, f'Log path still contains home directory: {redirected_log_path}'
            assert not home_path.exists(
                ), f'Directory was created in home directory: {home_path}'
            assert not home_log_path.exists(
...
ReqID: N/A"""
        ...
        try:
            ...
            memory_path = ensure_path_exists(str(Path.home() / '.devsynth' /
                'memory'), create=True)
            log_path = ensure_path_exists(str(Path.home() / '.devsynth' /
                'logs'), create=True)
            home_devsynth = Path.home() / '.devsynth'
            assert not home_devsynth.exists(
                ), f'.devsynth directory was created in home directory: {home_devsynth}'
            if not any(x in str(original_cwd).lower() for x in ['pytest',
                'tmp', 'temp']):
                cwd_devsynth = Path(original_cwd) / '.devsynth'
                assert not cwd_devsynth.exists(
                    ), f'.devsynth directory was created in original working directory: {cwd_devsynth}'
        finally:
            os.chdir(original_cwd)
            shutil.rmtree(test_dir, ignore_errors=True)
            for path in [Path.home() / '.devsynth', Path(original_cwd) /
                '.devsynth']:
                if path.exists():
                    print(
                        f'Warning: Cleaning up stray .devsynth directory: {path}'
                        )
