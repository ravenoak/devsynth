...
ReqID: N/A"""
        state = WorkflowState(workflow_id='test-id', command='init',
...
ReqID: N/A"""
        state = WorkflowState(workflow_id='test-id', command='init',
            project_root='/path/to/project', context={'key': 'value'},
            current_step='step-1')
...
ReqID: N/A"""
        state_dict = {'workflow_id': 'test-id', 'command': 'init',
            'project_root': '/path/to/project', 'context': {'key': 'value'},
            'current_step': 'step-1', 'status': WorkflowStatus.RUNNING.
            value, 'needs_human': True, 'human_message': 'Need input',
            'messages': [{'role': 'system', 'content': 'test'}], 'result':
            {'success': True}}
...
ReqID: N/A"""
    ...
    @patch('pickle.load')
    def test_get_checkpoint_exists_succeeds(self, mock_pickle_load,
        mock_file_open, mock_exists, checkpoint_saver):
        """Test getting an existing checkpoint.

ReqID: N/A"""
        mock_exists.return_value = True
        mock_pickle_load.return_value = {'state': 'test'}
        result = checkpoint_saver.get({'configurable': {'thread_id':
            'test-thread'}})
    ...
    @patch('os.path.exists')
    def test_get_checkpoint_not_exists_succeeds(self, mock_exists,
        checkpoint_saver):
...
ReqID: N/A"""
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('pickle.dump')
    def test_put_checkpoint_succeeds(self, mock_pickle_dump, mock_file_open,
        checkpoint_saver):
        """Test saving a checkpoint.

ReqID: N/A"""
        checkpoint = {'state': 'test'}
        checkpoint_saver.put({'configurable': {'thread_id': 'test-thread'}},
            checkpoint, {}, {})
        mock_file_open.assert_called_once()
...
ReqID: N/A"""
        workflow = workflow_engine.create_workflow('test-workflow',
            'Test workflow')
        step = WorkflowStep(id='step-1', name='Test Step', description=
            'A test step', agent_type='test_agent')
    ...
    def test_execute_workflow_succeeds(self, mock_pregel, mock_state_graph,
        workflow_engine):
...
ReqID: N/A"""
        ...
        mock_executor.invoke.return_value = WorkflowState(workflow_id=
            workflow.id, command='test', project_root='/path/to/project',
            status=WorkflowStatus.COMPLETED.value, result={'success': True}
            ).to_dict()
...
ReqID: N/A"""
        workflow = Workflow(id='test-id', name='Test Workflow', description
            ='A test workflow', status=WorkflowStatus.PENDING, steps=[])
        with patch('builtins.open', new_callable=mock_open) as mock_file:
            workflow_repo.save(workflow)
            mock_file.assert_called_once()
        with patch('os.path.exists', return_value=True), patch('builtins.open',
            new_callable=mock_open) as mock_file, patch('pickle.load',
...
ReqID: N/A"""
        ...
        workflow1 = Workflow(id='workflow1', name='Workflow 1', description
            ='', status=WorkflowStatus.COMPLETED, steps=[])
        workflow2 = Workflow(id='workflow2', name='Workflow 2', description
            ='', status=WorkflowStatus.RUNNING, steps=[])
        with patch.object(FileSystemWorkflowRepository, 'list',
            return_value=[workflow1, workflow2]):
            workflows = workflow_repo.list()
            assert len(workflows) == 2
            assert workflows[0].id == 'workflow1'
            assert workflows[1].id == 'workflow2'
        with patch.object(FileSystemWorkflowRepository, 'list',
            return_value=[workflow1]):
            completed_workflows = workflow_repo.list({'status':
