...
@pytest.fixture
def mock_bridge():
    with patch('devsynth.application.cli.commands.edrr_cycle_cmd.bridge'
        ) as mock:
        yield mock
...
@pytest.fixture
def mock_tqdm():
    with patch('devsynth.application.cli.commands.edrr_cycle_cmd.tqdm'
        ) as mock:
...
def mock_components():
    with patch(
        ...
        coordinator.generate_report.return_value = {'ok': True, 'insights':
            ['Insight 1', 'Insight 2'], 'next_steps': ['Step 1', 'Step 2']}
        coordinator.cycle_id = 'cid'
        coordinator.phase_metrics = MagicMock()
        coordinator.phase_metrics.get_all_metrics.return_value = {'EXPAND':
            {'quality': 0.8}, 'DIFFERENTIATE': {'quality': 0.85}, 'REFINE':
            {'quality': 0.9}, 'RETROSPECT': {'quality': 0.95}}
...
def mock_config():
    ...
    with patch('devsynth.config.unified_loader.UnifiedConfigLoader.load',
        return_value=cfg) as mock_loader:
        yield mock_loader
...
def test_edrr_cycle_cmd_manifest_missing_raises_error(tmp_path, mock_bridge,
    mock_config):
...
def test_edrr_cycle_cmd_manifest_success_succeeds(tmp_path, mock_bridge,
    mock_components, mock_config, mock_tqdm):
...
ReqID: N/A"""
    ...
    assert coord_cls.call_args.kwargs['config']['edrr']['phase_transition'][
        'auto'] is True
    coordinator.start_cycle_from_manifest.assert_called_once_with(manifest,
        is_file=True)
    assert coordinator.progress_to_phase.call_count == 4
    memory_manager.store_with_edrr_phase.assert_called_once_with(coordinator
        .generate_report.return_value, 'EDRR_CYCLE_RESULTS', Phase.
        RETROSPECT.value, {'cycle_id': coordinator.cycle_id})
    run_pipeline_mock.assert_called_once_with(report=coordinator.
        generate_report.return_value)
    assert mock_progress.update.call_count == 4
    assert any('Key Insights' in call.args[0] for call in mock_bridge.print
        .call_args_list)
    assert any('Recommended Next Steps' in call.args[0] for call in
        mock_bridge.print.call_args_list)


def test_edrr_cycle_cmd_prompt_success_succeeds(mock_bridge,
    mock_components, mock_config, mock_tqdm):
...
ReqID: N/A"""
    coordinator, memory_manager, coord_cls, run_pipeline_mock = mock_components
    _, mock_progress = mock_tqdm
    edrr_cycle_cmd(prompt='Improve error handling', context=
        'In the API endpoints', max_iterations=5)
    coord_cls.assert_called_once()
    assert coord_cls.call_args.kwargs['config']['edrr']['phase_transition'][
        'auto'] is True
...
def test_edrr_cycle_cmd_manual_succeeds(tmp_path, mock_bridge,
    mock_components, mock_config):
...
ReqID: N/A"""
    ...
    assert coord_cls.call_args.kwargs['config']['edrr']['phase_transition'][
        'auto'] is False
    expected = [Phase.EXPAND, Phase.DIFFERENTIATE, Phase.REFINE, Phase.
        RETROSPECT]
    assert [c.args[0] for c in coordinator.progress_to_phase.call_args_list
        ] == expected
...
def test_edrr_cycle_cmd_error_handling_raises_error(mock_bridge,
    mock_components, mock_config):
