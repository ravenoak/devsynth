...
from devsynth.application.promises import Promise, PromiseBroker, PromiseAgent, PromiseAgentMixin, CapabilityHandler, CapabilityNotFoundError, UnauthorizedAccessError
...
ReqID: N/A"""
        ...
        def add_numbers(a: int, b: int) ->int:
            return a + b
        handler = CapabilityHandler(agent_id='test_agent', capability_name=
            'add_numbers', handler_func=add_numbers, description=
            'Add two numbers together', parameters={'a': 'int', 'b': 'int'},
            tags=['math', 'addition'])
...
ReqID: N/A"""
        ...
        def multiply_numbers(a: int, b: int) ->int:
            return a * b
        handler = CapabilityHandler(agent_id='test_agent', capability_name=
            'multiply_numbers', handler_func=multiply_numbers, description=
            'Multiply two numbers together', parameters={'a': 'int', 'b':
            'int'})
        result = handler(3, 4)
        assert result == 12
...
ReqID: N/A"""
        ...
        def subtract_numbers(a: int, b: int) ->int:
            return a - b
        handler = CapabilityHandler(agent_id='test_agent', capability_name=
            'subtract_numbers', handler_func=subtract_numbers, description=
            'Subtract two numbers', parameters={'a': 'int', 'b': 'int'})
        promise = Promise[int]()
...
ReqID: N/A"""
        ...
        def divide_numbers(a: int, b: int) ->float:
            ...
            return a / b
        handler = CapabilityHandler(agent_id='test_agent', capability_name=
            'divide_numbers', handler_func=divide_numbers, description=
            'Divide two numbers', parameters={'a': 'int', 'b': 'int'})
...
ReqID: N/A"""
        ...
        def add_numbers(a: int, b: int) ->int:
            return a + b
        capability_id = agent.register_capability(name='add', handler_func=
            add_numbers, description='Add two numbers together', parameters
            ={'a': 'int', 'b': 'int'}, tags=['math', 'basic'])
...
ReqID: N/A"""
        ...
        def multiply_numbers(a: int, b: int) ->int:
            return a * b
        provider.register_capability(name='multiply', handler_func=
            multiply_numbers, description='Multiply two numbers together',
...
ReqID: N/A"""
        ...
        def slow_operation(delay: float) ->str:
            time.sleep(delay)
            return 'Completed'
        provider.register_capability(name='slow_operation', handler_func=
            slow_operation, description='A slow operation', parameters={
            'delay': 'float'})
        requester = PromiseAgent(agent_id='requester_agent', broker=broker)
        promise = requester.request_capability(name='slow_operation',
            timeout=0.5, delay=2)
        time.sleep(0.6)
        provider.handle_pending_capabilities()
        assert promise.is_rejected
...
ReqID: N/A"""
        ...
        provider.register_capability(name='greet', handler_func=greet,
            description='Greet a person', parameters={'person_name': 'str'})
        requester = PromiseAgent(agent_id='requester_agent', broker=broker)
        promise = requester.request_capability(name='greet', person_name=
            'World')
...
ReqID: N/A"""
        ...
        provider.register_capability(name='secret_operation', handler_func=
            secret_operation, description='A restricted operation',
            authorized_requesters={'authorized_agent'})
        unauthorized = PromiseAgent(agent_id='unauthorized_agent', broker=
            broker)
...
ReqID: N/A"""
        broker = PromiseBroker()
        provider = PromiseAgent(agent_id='provider_agent', broker=broker)
        provider.register_capability(name='capability1', handler_func=lambda :
            'result1', description='Capability 1')
        provider.register_capability(name='capability2', handler_func=lambda :
            'result2', description='Capability 2', tags=['tag1', 'tag2'])
        provider.register_capability(name='restricted', handler_func=lambda :
            'restricted', description='Restricted capability',
...
ReqID: N/A"""
        ...
        def example_capability(x: int) ->int:
            return x * 2
        capability_id = agent.promise_agent.register_capability(name=
            'example', handler_func=example_capability, description=
            'An example capability', parameters={'x': 'int'})
        assert capability_id is not None
