...
def test_parametrize_speed_marker_parity(tmp_path: Path):
    ...
    etp = _import_module_from_path(
        "enhanced_test_parser", Path(__file__).parents[3] / "scripts" / "enhanced_test_parser.py"
    )
    vtm = _import_module_from_path(
        "verify_test_markers", Path(__file__).parents[3] / "scripts" / "verify_test_markers.py"
    )

    # Parse using enhanced test parser
    parsed = etp.parse_test_file(str(file_path))
    # Find our test
    target = [t for t in parsed["tests"] if t["name"] == "test_derived_speed_from_params"]
    assert target, "Expected test to be discovered"
    test_entry = target[0]
    assert test_entry["markers"] == ["medium"], "Enhanced parser should derive 'medium' from pytest.param marks"
