...
class TestSyncManagerCrossStoreQuery:
    ...
    def _add_items(self, manager: MemoryManager):
        vec = MemoryVector(id='', content='apple', embedding=manager._embed_text('apple'), metadata={'memory_type': MemoryType.CODE.value})
        manager.adapters['vector'].store_vector(vec)
        item = MemoryItem(id='', content='apple', memory_type=MemoryType.CODE, metadata={})
    ...
    def test_query_results_cached_succeeds(self, manager):
        ...
        new_item = MemoryItem(id='', content='apple second', memory_type=MemoryType.CODE, metadata={})
...
class TestSyncManagerConcurrentUpdates:
    ...
    def manager(self):
        ...
        manager.sync_manager = memory_manager_module.SyncManager(manager, async_mode=True)
        return manager

    @pytest.mark.asyncio
    @pytest.mark.medium
    async def test_queue_updates_from_multiple_tasks_succeeds(self, manager):
        items = [MemoryItem(id=str(i), content=str(i), memory_type=MemoryType.CODE) for i in range(5)]
    ...
    async def test_conflict_resolution_with_concurrent_updates(self, manager):
        ...
        original = MemoryItem(id='x', content='orig', memory_type=MemoryType.CODE, created_at=base_time)
        manager.adapters['a'].store(original)
        newer = MemoryItem(id='x', content='newer', memory_type=MemoryType.CODE, created_at=base_time + timedelta(seconds=1))
        newest = MemoryItem(id='x', content='newest', memory_type=MemoryType.CODE, created_at=base_time + timedelta(seconds=2))
