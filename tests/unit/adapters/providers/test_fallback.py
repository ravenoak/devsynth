...
@pytest.fixture
def mock_function():
    mock = Mock(side_effect=[ValueError('Test error'), ValueError(
        'Test error'), 'success'])
...
ReqID: N/A"""
    decorated_func = retry_with_exponential_backoff(max_retries=3,
        initial_delay=0.1)(mock_function)
...
ReqID: N/A"""
    ...
    with patch('time.sleep', side_effect=mock_sleep):
        decorated_func = retry_with_exponential_backoff(max_retries=3,
            initial_delay=1.0, max_delay=10.0, jitter=True)(mock_func)
        with pytest.raises(ValueError):
            decorated_func()
...
ReqID: N/A"""
    ...
    with patch('time.sleep'):
        decorated_func = retry_with_exponential_backoff(max_retries=3,
            initial_delay=0.1, on_retry=on_retry_mock)(mock_func)
        with pytest.raises(ValueError):
            decorated_func()
...
ReqID: N/A"""
    mock_func = Mock(side_effect=[ValueError('Test error'), TypeError(
        'Type error'), ValueError('Test error'), 'success'])
    mock_func.__name__ = 'mock_func'
    with patch('time.sleep'):
        decorated_func = retry_with_exponential_backoff(max_retries=3,
            initial_delay=0.1, retryable_exceptions=(ValueError,))(mock_func)
...
ReqID: N/A"""
    ...
    with patch('time.sleep', side_effect=mock_sleep):
        decorated_func = retry_with_exponential_backoff(max_retries=3,
            initial_delay=1.0, exponential_base=2.0, max_delay=10.0, jitter
            =False)(mock_func)
        with pytest.raises(ValueError):
            decorated_func()
    assert len(sleep_times) == 3
