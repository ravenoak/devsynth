...
def test_fake_memory_store_store_retrieve_search_delete_and_txn():
    ...
    # Store two items
    id1 = store.store(MemoryItem(id="", memory_type=MemoryType.WORKING, content="Hello World"))
    id2 = store.store(MemoryItem(id="", memory_type=MemoryType.WORKING, content="Another note about World"))
    ...
    id3 = store.store(MemoryItem(id="", memory_type=MemoryType.WORKING, content="Temp"), transaction_id=tx)
...
def test_fake_vector_store_similarity_and_stats():
    ...
    v1 = MemoryVector(id="", content="x", embedding=[1.0, 0.0, 0.0], metadata={"name": "x"})
    v2 = MemoryVector(id="", content="y", embedding=[0.0, 1.0, 0.0], metadata={"name": "y"})
    v3 = MemoryVector(id="", content="x+y", embedding=[1.0, 1.0, 0.0], metadata={"name": "x+y"})
