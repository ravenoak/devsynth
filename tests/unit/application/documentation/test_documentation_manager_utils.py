...
ReqID: N/A"""
        mock_results = [{'content':
            'Documentation for function pandas.DataFrame.groupby', 'type':
            'function', 'library': 'pandas', 'version': '1.4.2',
            'relevance': 0.95}]
        self.documentation_manager.query_documentation.return_value = (
            mock_results)
        function_name = 'pandas.DataFrame.groupby'
        results = self.documentation_manager.get_function_documentation(
            function_name)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]['type'], 'function')
        self.assertEqual(results[0]['content'],
            'Documentation for function pandas.DataFrame.groupby')
...
ReqID: N/A"""
        mock_results = [{'content':
            ...
            'type': 'class', 'library': 'sklearn', 'version': '1.1.2',
            'relevance': 0.95}]
        self.documentation_manager.query_documentation.return_value = (
            mock_results)
...
ReqID: N/A"""
        mock_results = [{'content': 'Example 1 for numpy.array', 'type':
            'example', 'library': 'numpy', 'version': '1.22.4', 'relevance':
            0.95}, {'content': 'Example 2 for numpy.array', 'type':
            'example', 'library': 'numpy', 'version': '1.22.4', 'relevance':
            0.85}]
        self.documentation_manager.query_documentation.return_value = (
            mock_results)
...
ReqID: N/A"""
        ...
        (self.documentation_manager.repository.get_documentation.side_effect
            ) = (lambda lib, ver: {'pandas': {'1.3.0': {'parameters': [
            'param1', 'param2']}, '1.4.0': {'parameters': ['param1',
            'param2', 'param3'], 'changes': ['Added param3']}, '1.4.2': {
            'parameters': ['param1', 'param2', 'param3'], 'deprecated': [
            'param2 will be deprecated in 1.5.0']}}}.get(lib, {}).get(ver, {}))
        ...
        self.assertEqual(self.documentation_manager.repository.
            get_documentation.call_count, 3)
...
ReqID: N/A"""
        mock_results = [{'content':
            'Documentation for function scipy.stats.ttest_ind', 'type':
            'function', 'library': 'scipy', 'version': '1.8.1', 'relevance':
            0.95, 'related': ['scipy.stats.ttest_rel',
            'scipy.stats.ttest_1samp']}]
        self.documentation_manager.query_documentation.return_value = (
            mock_results)
        self.documentation_manager.repository.get_documentation = MagicMock()
        (self.documentation_manager.repository.get_documentation.side_effect
            ) = (lambda lib, ver, func=None: {'scipy.stats.ttest_rel': {
            'description':
            'Calculates the T-test for the means of two related samples of scores'
            }, 'scipy.stats.ttest_1samp': {'description':
            'Calculates the T-test for the mean of ONE group of scores'}}.
            get(func, {}) if func else {})
        function_name = 'scipy.stats.ttest_ind'
        results = self.documentation_manager.get_related_functions(
            function_name)
        self.assertEqual(results['function'], function_name)
        self.assertEqual(len(results['related_functions']), 2)
        self.assertEqual(results['related_functions'][0]['name'],
            'scipy.stats.ttest_rel')
        self.assertEqual(results['related_functions'][1]['name'],
            'scipy.stats.ttest_1samp')
        self.documentation_manager.query_documentation.assert_called_once()
        args, kwargs = self.documentation_manager.query_documentation.call_args
        self.assertEqual(args[0], f'function:{function_name}')
...
ReqID: N/A"""
        mock_results = [{'content':
            'Documentation for function sklearn.model_selection.train_test_split'
            , 'type': 'function', 'library': 'sklearn', 'version': '1.1.2',
            'relevance': 0.95, 'usage_patterns': [{'pattern': 'Basic usage',
            'code':
            'X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)'
            }, {'pattern': 'With stratification', 'code':
            'X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, stratify=y)'
            }], 'best_practices': [
            'Always set random_state for reproducibility',
            'Use stratify for imbalanced datasets'], 'common_params': {
            'test_size': '0.2 or 0.25 are common values', 'random_state':
            'Any integer for reproducibility', 'stratify':
            'Usually set to target variable y for classification'}}]
        self.documentation_manager.query_documentation.return_value = (
            mock_results)
        ...
        self.assertEqual(results['usage_patterns'][1]['pattern'],
            'With stratification')
...
ReqID: N/A"""
        ...
        result = self.documentation_manager.fetch_documentation('foo',
            '1.0', offline=True)
