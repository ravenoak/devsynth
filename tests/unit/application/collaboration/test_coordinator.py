...
ReqID: N/A"""
        consensus = {'consensus': 'final', 'contributors': ['python', 'javascript', 'docs'], 'method': 'consensus_synthesis'}
        with patch.object(self.coordinator.team, 'build_consensus', return_value=consensus):
            with patch.object(self.coordinator.team, 'select_primus_by_expertise', wraps=self.coordinator.team.select_primus_by_expertise) as spy:
...
ReqID: N/A"""
        consensus = {'consensus': 'final-result', 'contributors': ['python', 'javascript', 'docs'], 'method': 'consensus_synthesis'}
...
ReqID: N/A"""
        consensus = {'consensus': 'ok', 'contributors': ['python', 'javascript', 'docs'], 'method': 'consensus_synthesis'}
        ...
        with patch.object(self.coordinator.team, 'build_consensus', return_value=consensus):
            with patch.object(self.coordinator.team, 'assign_roles', side_effect=mock_assign_roles):
                self.coordinator.delegate_task({'team_task': True, 'language': 'python', 'type': 'coding'})
...
ReqID: N/A"""
        ...
        agent.current_role = None
        coordinator.add_agent(agent)
        task = {'team_task': True, 'type': 'critical_decision', 'is_critical': True, 'options': [{'id': 'a'}, {'id': 'b'}]}
        with patch.object(coordinator.team, 'vote_on_critical_decision', return_value={'result': {'winner': 'a'}}) as vote:
