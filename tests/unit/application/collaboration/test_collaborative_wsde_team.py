...
ReqID: N/A"""
        ...
        task = {'id': 'task1', 'type': 'decision_task', 'description': 'Choose a database technology', 'options': [{'id': 'option1', 'name': 'PostgreSQL'}, {'id': 'option2', 'name': 'MongoDB'}, {'id': 'option3', 'name': 'MySQL'}]}

        def mock_get_messages(agent=None, filters=None):
            if agent == 'Agent1' and filters.get('type') == 'opinion':
                return [{'sender': 'Agent1', 'type': 'opinion', 'content': {'opinion': 'I prefer PostgreSQL', 'rationale': "It's robust and reliable"}, 'timestamp': datetime.now().isoformat(), 'metadata': {'task_id': task['id']}}]
            elif agent == 'Agent2' and filters.get('type') == 'opinion':
                return [{'sender': 'Agent2', 'type': 'opinion', 'content': {'opinion': 'I prefer PostgreSQL', 'rationale': 'It has good performance'}, 'timestamp': datetime.now().isoformat(), 'metadata': {'task_id': task['id']}}]
            elif agent == 'Agent3' and filters.get('type') == 'opinion':
                return [{'sender': 'Agent3', 'type': 'opinion', 'content': {'opinion': 'I prefer PostgreSQL', 'rationale': "It's secure and stable"}, 'timestamp': datetime.now().isoformat(), 'metadata': {'task_id': task['id']}}]
            return []
...
ReqID: N/A"""
        ...
        def mock_get_messages(agent=None, filters=None):
            if agent == 'Agent1' and filters.get('type') == 'opinion':
                ...
                return [{'sender': 'Agent2', 'type': 'opinion', 'content': {'opinion': 'I prefer MongoDB', 'rationale': "It's flexible and scalable"}, 'timestamp': datetime.now().isoformat(), 'metadata': {'task_id': task['id']}}]
            elif agent == 'Agent3' and filters.get('type') == 'opinion':
                return [{'sender': 'Agent3', 'type': 'opinion', 'content': {'opinion': 'I prefer PostgreSQL', 'rationale': "It's secure and stable"}, 'timestamp': datetime.now().isoformat(), 'metadata': {'task_id': task['id']}}]
            return []
        conflicts = [{'agents': ['Agent1', 'Agent2'], 'opinions': ['I prefer PostgreSQL', 'I prefer MongoDB'], 'severity': 0.8, 'description': 'Conflict between PostgreSQL and MongoDB preferences'}]
        synthesis = {'text': 'After considering all perspectives, PostgreSQL is recommended for its robustness, security, and stability.', 'key_points': ['PostgreSQL is robust', 'PostgreSQL is secure', 'PostgreSQL is stable'], 'readability_score': {'flesch_reading_ease': 75.0}}
        with patch.object(team, 'get_messages', side_effect=mock_get_messages):
            with patch.object(team, '_identify_conflicts', return_value=conflicts):
                with patch.object(team, '_generate_conflict_resolution_synthesis', return_value=synthesis):
                    consensus_result = team.build_consensus(task)
                    assert 'task_id' in consensus_result
                    assert consensus_result['task_id'] == task['id']
                    assert 'method' in consensus_result
    ...
    @pytest.mark.medium
    def test_vote_on_critical_decision_with_expertise_weighting_succeeds(self, mock_agent_with_expertise):
...
ReqID: N/A"""
        ...
        task = {'id': 'db_decision', 'type': 'critical_decision', 'domain': 'databases', 'description': 'Choose a database technology', 'options': [{'id': 'postgres', 'name': 'PostgreSQL'}, {'id': 'mongodb', 'name': 'MongoDB'}]}
        ...
        expected_result = {'vote_weights': {'BackendExpert': 0.9, 'FrontendDev': 0.3, 'SecurityExpert': 0.5}, 'votes': {'BackendExpert': 'postgres', 'FrontendDev': 'mongodb', 'SecurityExpert': 'mongodb'}, 'result': {'method': 'weighted_vote', 'winner': 'postgres'}}
        with patch.object(team, 'vote_on_critical_decision', return_value=expected_result):
...
ReqID: N/A"""
        ...
        task = {'id': 'tie_decision', 'type': 'critical_decision', 'description': 'Choose a technology', 'options': [{'id': 'option1', 'name': 'Option 1'}, {'id': 'option2', 'name': 'Option 2'}]}
        ...
        expected_result = {'result_type': 'tie', 'votes': {'Agent1': 'option1', 'Agent2': 'option1', 'Agent3': 'option2', 'Agent4': 'option2'}, 'tie_resolution': {'strategies_applied': ['expertise_weighting', 'random_selection'], 'selected_option': 'option1'}, 'selected_option': {'id': 'option1', 'name': 'Option 1', 'tie_breaking_rationale': 'Selected based on expertise weighting and random selection'}}
        with patch.object(team, 'vote_on_critical_decision', return_value=expected_result):
            result = team.vote_on_critical_decision(task)
    ...
    @pytest.mark.medium
    def test_decision_tracking_and_explanation_succeeds(self, mock_agent_with_expertise):
...
ReqID: N/A"""
        ...
        team.add_agent(agent2)
        team.add_agent(agent3)
        task = {'id': 'tracked_decision', 'type': 'decision_task', 'description': 'Choose a technology stack', 'criticality': 'high', 'options': [{'id': 'stack1', 'name': 'Stack 1'}, {'id': 'stack2', 'name': 'Stack 2'}, {'id': 'stack3', 'name': 'Stack 3'}]}

        def mock_get_messages(agent=None, filters=None):
            if agent == 'Agent1' and filters.get('type') == 'opinion':
                return [{'sender': 'Agent1', 'type': 'opinion', 'content': {'opinion': 'I prefer Stack 1', 'rationale': "It's robust and reliable"}, 'timestamp': datetime.now().isoformat(), 'metadata': {'task_id': task['id']}}]
            elif agent == 'Agent2' and filters.get('type') == 'opinion':
                return [{'sender': 'Agent2', 'type': 'opinion', 'content': {'opinion': 'I prefer Stack 2', 'rationale': "It's flexible and scalable"}, 'timestamp': datetime.now().isoformat(), 'metadata': {'task_id': task['id']}}]
            elif agent == 'Agent3' and filters.get('type') == 'opinion':
                return [{'sender': 'Agent3', 'type': 'opinion', 'content': {'opinion': 'I prefer Stack 1', 'rationale': "It's secure and stable"}, 'timestamp': datetime.now().isoformat(), 'metadata': {'task_id': task['id']}}]
            return []
        with patch.object(team, 'get_messages', side_effect=mock_get_messages):
            with patch.object(team, '_identify_conflicts', return_value=[]):
                consensus_result = team.build_consensus(task)
                tracked_decision = {'id': task['id'], 'metadata': {'decision_date': datetime.now().isoformat(), 'decision_maker': 'TrackingTeam', 'criticality': 'high', 'implementation_status': 'pending', 'verification_status': 'pending'}, 'voting_results': {'method': 'majority_opinion', 'majority_opinion': 'I prefer Stack 1'}, 'rationale': {'expertise_references': ['python', 'backend', 'security'], 'considerations': ['robustness', 'security', 'stability']}, 'stakeholder_explanation': 'Stack 1 was chosen for its robustness, security, and stability.', 'readability_score': 75.0}
                with patch.object(team, 'get_tracked_decision', return_value=tracked_decision):
                    with patch.object(team, 'mark_decision_implemented', return_value=True):
                        team.mark_decision_implemented(task['id'])
                        with patch.object(team, 'add_decision_implementation_details', return_value=True):
                            implementation_details = {'implemented_by': 'development_team', 'implementation_date': '2025-07-10', 'implementation_status': 'completed', 'verification_status': 'verified'}
                            team.add_decision_implementation_details(task['id'], implementation_details)
                            tracked_decision['metadata']['implementation_status'] = 'completed'
                            tracked_decision['metadata']['verification_status'] = 'verified'
                            ...
                            with patch.object(team, 'query_decisions', side_effect=lambda **kwargs: high_criticality_decisions if kwargs.get('criticality') == 'high' else implemented_decisions):
                                high_criticality_result = team.query_decisions(criticality='high')
                                assert len(high_criticality_result) > 0
                                assert task['id'] in [decision['id'] for decision in high_criticality_result]
                                implemented_result = team.query_decisions(implementation_status='completed')
                                assert len(implemented_result) > 0
                                assert task['id'] in [decision['id'] for decision in implemented_result]