...
ReqID: N/A"""
        ...
        auto_tuner.prompt_variants[template_id].append(PromptVariant(
            'Variant 2'))
        auto_tuner.prompt_variants[template_id].append(PromptVariant(
            'Variant 3'))
        auto_tuner.prompt_variants[template_id][1].record_usage(success=
            True, feedback_score=1.0)
...
ReqID: N/A"""
        ...
        auto_tuner.record_feedback(template_id, variant.variant_id, success
            =True, feedback_score=0.9)
        assert variant.success_count == 1
        assert variant.feedback_scores == [0.9]
...
ReqID: N/A"""
        with pytest.raises(PromptAutoTuningError):
            auto_tuner.record_feedback('nonexistent-template',
                'nonexistent-variant')
    ...
    @patch(
        'devsynth.application.prompts.auto_tuning.PromptAutoTuner._mutate_variant'
        )
...
Please use it carefully."""
        ...
        assert mutated != template or '**' in mutated or '_' in mutated or any(
            word.isupper() for word in mutated.split())
...
ReqID: N/A"""
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            auto_tuner.record_feedback('test-template', variant.variant_id,
                success=True, feedback_score=0.9)
            ...
            assert new_auto_tuner.prompt_variants['test-template'][0
                ].template == 'Test template'
            assert new_auto_tuner.prompt_variants['test-template'][0
                ].success_count == 1
            assert new_auto_tuner.prompt_variants['test-template'][0
