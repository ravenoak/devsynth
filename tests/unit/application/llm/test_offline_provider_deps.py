...
def test_load_transformer_deps_imports_when_available(monkeypatch):
    _reload_provider()
    dummy_torch = types.SimpleNamespace(no_grad=lambda: contextmanager(lambda: (yield None))())
    dummy_trans = types.SimpleNamespace(AutoModelForCausalLM='Model', AutoTokenizer='Tok')
