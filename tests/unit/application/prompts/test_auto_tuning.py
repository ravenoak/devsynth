import pytest
from devsynth.application.prompts.auto_tuning import BasicPromptTuner, PromptAutoTuner, PromptVariant, iterative_prompt_adjustment, run_tuning_iteration
...
class _DummyLLMPort:
    ...
    def __init__(self):
        self.generate = (lambda prompt, parameters=None, provider_type=None:
            'ok')
...
ReqID: N/A"""
    agent = UnifiedAgent()
    config = AgentConfig(name='ua', agent_type=AgentType.ORCHESTRATOR,
...
ReqID: N/A"""
    ...
    with patch.object(PromptAutoTuner, '_generate_variants_if_needed',
        side_effect=generate_variant):
        tuner.exploration_rate = 0.0
        best = iterative_prompt_adjustment('base', evaluate, iterations=2,
            tuner=tuner)
