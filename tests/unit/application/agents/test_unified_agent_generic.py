...
def stub_agent_dependencies(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, 'devsynth.application.agents.agent_memory_integration', am_mod)
    wsde_mod = types.ModuleType('devsynth.application.agents.wsde_memory_integration')
    wsde_mod.WSDEMemoryIntegration = object
    monkeypatch.setitem(sys.modules, 'devsynth.application.agents.wsde_memory_integration', wsde_mod)
    yield
