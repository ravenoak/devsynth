...
class TestMultiLanguageCodeAgent:
    ...
    def agent(self, mock_llm_port):
        ...
        config = AgentConfig(name='MultiLangAgent', agent_type=AgentType.CODE, description='Test multi language agent', capabilities=[])
    ...
    def test_process_polyglot_succeeds(self, agent):
        result = agent.process({'language': 'polyglot', 'languages': ['python', 'javascript']})
