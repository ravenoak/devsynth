...
ReqID: N/A"""
    ...
    def slow(delay: float) ->str:
        time.sleep(delay)
        return 'done'
    provider.register_capability(name='slow', handler_func=slow,
        description='slow op', parameters={'delay': 'float'})
...
ReqID: N/A"""
    broker = PromiseBroker()
    provider = PromiseAgent(agent_id='provider', broker=broker)
    provider.register_capability(name='fast', handler_func=lambda : 'ok',
        description='fast')
    requester = PromiseAgent(agent_id='requester', broker=broker)
