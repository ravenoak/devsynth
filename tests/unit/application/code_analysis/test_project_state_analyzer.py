...
ReqID: N/A"""
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (project_dir / 'src' / 'models' / 'user.py').write_text('class User:\n    def __init__(self, name):\n        self.name = name')
        (project_dir / 'src' / 'controllers' / 'user_controller.py').write_text("from src.models.user import User\n\nclass UserController:\n    def get_user(self, user_id):\n        return User('Test')")
        (project_dir / 'src' / 'views' / 'user_view.py').write_text("class UserView:\n    def render_user(self, user):\n        return f'User: {user.name}'")
        (project_dir / 'docs' / 'requirements.md').write_text('# Requirements\n\n1. The system shall allow users to create accounts.\n2. The system shall allow users to log in.')
        (project_dir / 'docs' / 'specifications.md').write_text('# Specifications\n\n1. User Creation: The system will provide an API endpoint for user creation.\n2. User Authentication: The system will support username/password authentication.')
        (project_dir / 'tests' / 'test_user.py').write_text("def test_user_creation():\n    from src.models.user import User\n    user = User('Test')\n    assert user.name == 'Test'")
        ...
        with patch.object(analyzer, '_index_files') as mock_index:
            with patch.object(analyzer, '_detect_languages') as mock_detect:
                with patch.object(analyzer, '_infer_architecture') as mock_infer:
                    with patch.object(analyzer, '_analyze_requirements_spec_alignment', return_value={'matched_requirements': []}) as mock_req_spec:
                        with patch.object(analyzer, '_analyze_spec_code_alignment', return_value={'implemented_specifications': []}) as mock_spec_code:
                            with patch.object(analyzer, '_generate_health_report', return_value={'overall_health': 0.8}) as mock_health:
...
ReqID: N/A"""
        ...
        python_files = [f for f in analyzer.files.values() if f.get('language', '').lower() == 'python' or f.get('extension', '').lower() == '.py']
...
ReqID: N/A"""
        ...
        def mock_identify_components(architecture):
            ...
            if architecture.lower() == 'mvc':
                return [{'type': 'Model', 'path': 'src/models/user.py', 'name': 'user'}, {'type': 'View', 'path': 'src/views/user_view.py', 'name': 'user_view'}, {'type': 'Controller', 'path': 'src/controllers/user_controller.py', 'name': 'user_controller'}]
            else:
                return []
    ...
    @pytest.mark.medium
    def test_analyze_requirements_spec_alignment_succeeds(self, test_project_dir_succeeds):
...
ReqID: N/A"""
        ...
        with patch.object(analyzer, '_extract_requirements') as mock_extract_req:
            with patch.object(analyzer, '_extract_specifications') as mock_extract_spec:
                mock_extract_req.return_value = [{'text': 'The system shall allow users to create accounts.', 'section': 'Requirements', 'source_file': 'docs/requirements.md'}]
                mock_extract_spec.return_value = [{'text': 'User Creation: The system will provide an API endpoint for user creation.', 'section': 'Specifications', 'source_file': 'docs/specifications.md'}]
...
ReqID: N/A"""
        ...
        req_spec_alignment = {'total_requirements': 2, 'total_specifications': 2, 'matched_requirements': 2, 'unmatched_requirements': [], 'unmatched_specifications': [], 'alignment_score': 1.0}
        spec_code_alignment = {'total_specifications': 2, 'implemented_specifications': 2, 'unimplemented_specifications': [], 'implementation_score': 1.0}
