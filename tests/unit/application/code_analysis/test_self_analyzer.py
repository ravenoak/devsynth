...
def mock_code_analysis():
    ...
    file_paths = ['src/domain/models/user.py', 'src/domain/services/user_service.py', 'src/application/controllers/user_controller.py', 'src/adapters/repositories/user_repository.py', 'src/adapters/api/user_api.py', 'tests/unit/test_user.py', 'tests/integration/test_user_api.py']
    file_analyses = {}
    for path in file_paths:
        ...
        file_analysis.get_metrics.return_value = {'lines_of_code': 100, 'comment_lines': 20, 'complexity': 5}
        file_analyses[path] = file_analysis
    file_analyses['src/application/controllers/user_controller.py'].get_imports.return_value = [{'name': 'src.domain.services.user_service', 'alias': None}]
    file_analyses['src/domain/services/user_service.py'].get_imports.return_value = [{'name': 'src.domain.models.user', 'alias': None}, {'name': 'src.adapters.repositories.user_repository', 'alias': None}]
    ...
    code_analysis.get_metrics.return_value = {'total_files': len(file_paths), 'total_lines': 700, 'total_comment_lines': 140, 'average_complexity': 5}
...
ReqID: N/A"""
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (project_dir / 'src' / 'domain' / 'models' / 'user.py').write_text('class User:\n    def __init__(self, name):\n        self.name = name')
        (project_dir / 'src' / 'domain' / 'services' / 'user_service.py').write_text('from src.domain.models.user import User\nfrom src.adapters.repositories.user_repository import UserRepository\n\nclass UserService:\n    def __init__(self, repository):\n        self.repository = repository\n\n    def get_user(self, user_id):\n        return self.repository.find_by_id(user_id)')
        (project_dir / 'src' / 'application' / 'controllers' / 'user_controller.py').write_text('from src.domain.services.user_service import UserService\n\nclass UserController:\n    def __init__(self, service):\n        self.service = service\n\n    def get_user(self, user_id):\n        return self.service.get_user(user_id)')
        (project_dir / 'src' / 'adapters' / 'repositories' / 'user_repository.py').write_text("from src.domain.models.user import User\n\nclass UserRepository:\n    def find_by_id(self, user_id):\n        return User('Test')")
        (project_dir / 'src' / 'adapters' / 'api' / 'user_api.py').write_text('from src.application.controllers.user_controller import UserController\n\ndef get_user_endpoint(user_id):\n    controller = UserController(None)\n    return controller.get_user(user_id)')
        (project_dir / 'tests' / 'unit' / 'test_user.py').write_text("def test_user_creation():\n    from src.domain.models.user import User\n    user = User('Test')\n    assert user.name == 'Test'")
        (project_dir / 'tests' / 'integration' / 'test_user_api.py').write_text("def test_get_user_endpoint():\n    from src.adapters.api.user_api import get_user_endpoint\n    user = get_user_endpoint(1)\n    assert user.name == 'Test'")
...
ReqID: N/A"""
        analyzer = SelfAnalyzer()
        architecture_type, confidence = analyzer._detect_architecture_type(mock_code_analysis)
...
ReqID: N/A"""
        analyzer = SelfAnalyzer()
        layers = {'domain': ['src/domain/models/user.py', 'src/domain/services/user_service.py'], 'application': ['src/application/controllers/user_controller.py'], 'adapters': ['src/adapters/repositories/user_repository.py', 'src/adapters/api/user_api.py']}
...
ReqID: N/A"""
        analyzer = SelfAnalyzer()
        layer_dependencies = {'domain': set(), 'application': {'domain'}, 'adapters': {'domain', 'application'}}
        violations = analyzer._check_architecture_violations(layer_dependencies, 'Hexagonal')
        assert len(violations) == 0
        layer_dependencies = {'domain': {'adapters'}, 'application': {'domain'}, 'adapters': {'domain', 'application'}}
        violations = analyzer._check_architecture_violations(layer_dependencies, 'Hexagonal')
        assert len(violations) > 0
...
ReqID: N/A"""
        analyzer = SelfAnalyzer()
        architecture_insights = {'type': 'Hexagonal', 'layers': {'domain': ['src/domain/models/user.py'], 'application': ['src/application/controllers/user_controller.py'], 'adapters': ['src/adapters/repositories/user_repository.py']}, 'layer_dependencies': {'domain': set(), 'application': {'domain'}, 'adapters': {'domain', 'application'}}, 'architecture_violations': [{'source_layer': 'domain', 'target_layer': 'adapters', 'description': "Layer 'domain' should not depend on layer 'adapters' in Hexagonal architecture"}]}
        code_quality_insights = {'docstring_coverage': {'files': 0.5, 'classes': 0.4, 'functions': 0.3}, 'total_files': 10, 'total_classes': 5, 'total_functions': 20}
        test_coverage_insights = {'total_symbols': 100, 'tested_symbols': 30, 'coverage_percentage': 0.3}
        opportunities = analyzer._identify_improvement_opportunities(mock_code_analysis, architecture_insights, code_quality_insights, test_coverage_insights)
