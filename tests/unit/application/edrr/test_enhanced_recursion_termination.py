...
def memory_manager():
    ...
    mock.retrieve_historical_patterns.return_value = [{'task_type':
        'refactoring', 'recursion_effectiveness': 0.2}, {'task_type':
        'test', 'recursion_effectiveness': 0.3}, {'task_type': 'testing',
        'recursion_effectiveness': 0.8}, {'task_type': 'documentation',
        'recursion_effectiveness': 0.6}]
    return mock
...
ReqID: N/A"""
    ...
    task = {'description': 'Task at max depth', 'granularity_score': 0.8,
        'cost_score': 0.2, 'benefit_score': 0.9}
...
ReqID: N/A"""
    ...
    max_duration = coordinator.config.get('edrr', {}).get('recursion', {}).get(
        'max_duration', 3600)
    coordinator._cycle_start_time = datetime.now() - timedelta(seconds=
        max_duration + 60)
    task = {'description': 'Long-running task', 'granularity_score': 0.8,
        'cost_score': 0.2, 'benefit_score': 0.9}
    should_terminate, reason = coordinator.should_terminate_recursion(task)
    assert should_terminate is True
...
def test_should_terminate_recursion_historical_effectiveness_succeeds(
    coordinator):
...
def test_should_not_terminate_recursion_historical_effectiveness_succeeds(
    coordinator):
...
ReqID: N/A"""
    ...
    coordinator._cycle_start_time = datetime.now() - timedelta(seconds=
        max_duration - 60)
    task = {'description': 'Complex task', 'memory_usage': 0.85, 'type':
        'documentation', 'granularity_score': 0.15, 'quality_score': 0.8,
        'resource_usage': 0.7}
    should_terminate, reason = coordinator.should_terminate_recursion(task)
    assert should_terminate is True
    assert reason in ['granularity threshold', 'memory usage limit',
        'approaching memory limit', 'approaching maximum recursion depth',
