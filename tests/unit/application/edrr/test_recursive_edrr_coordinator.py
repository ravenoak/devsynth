...
ReqID: N/A"""
        ...
        for i in range(1, coordinator.max_recursion_depth + 1):
            ...
            current_cycle = current_cycle.create_micro_cycle(micro_task,
                Phase.EXPAND)
            assert current_cycle.recursion_depth == i
        with pytest.raises(EDRRCoordinatorError):
            current_cycle.create_micro_cycle({'description': 'Too Deep'},
                Phase.EXPAND)
...
ReqID: N/A"""
        ...
        with patch.object(coordinator, 'should_terminate_recursion',
            ...
            with pytest.raises(EDRRCoordinatorError) as excinfo:
                coordinator.create_micro_cycle({'description': 'Sub'},
                    Phase.EXPAND)
...
ReqID: N/A"""
        ...
        def mock_execute_expand(context=None):
            coordinator.create_micro_cycle({'description':
                'Brainstorm Ideas'}, Phase.EXPAND)
            coordinator.create_micro_cycle({'description':
                'Research Existing Solutions'}, Phase.EXPAND)
            coordinator.create_micro_cycle({'description':
                'Analyze Requirements'}, Phase.EXPAND)
        ...
        assert all(cycle.parent_cycle_id == coordinator.cycle_id for cycle in
            coordinator.child_cycles)
        assert all(cycle.recursion_depth == 1 for cycle in coordinator.
            child_cycles)
...
ReqID: N/A"""
        ...
        def mock_execute_differentiate(context=None):
            coordinator.create_micro_cycle({'description':
                'Compare Approaches'}, Phase.DIFFERENTIATE)
            coordinator.create_micro_cycle({'description':
                'Evaluate Trade-offs'}, Phase.DIFFERENTIATE)
            coordinator.create_micro_cycle({'description':
                'Select Best Approach'}, Phase.DIFFERENTIATE)
            return original_execute(context)
...
ReqID: N/A"""
        ...
        def mock_execute_refine(context=None):
            coordinator.create_micro_cycle({'description':
                'Implement Solution'}, Phase.REFINE)
            coordinator.create_micro_cycle({'description':
                'Optimize Performance'}, Phase.REFINE)
            coordinator.create_micro_cycle({'description': 'Ensure Quality'
                }, Phase.REFINE)
            return original_execute(context)
...
ReqID: N/A"""
        ...
        def mock_execute_retrospect(context=None):
            coordinator.create_micro_cycle({'description':
                'Extract Learnings'}, Phase.RETROSPECT)
            coordinator.create_micro_cycle({'description':
                'Identify Patterns'}, Phase.RETROSPECT)
            coordinator.create_micro_cycle({'description':
                'Generate Improvements'}, Phase.RETROSPECT)
            return original_execute(context)
...
ReqID: N/A"""
        coordinator.start_cycle({'description': 'Macro Task'})
        micro_task = {'description': 'Very small task', 'granularity_score':
            0.1}
...
ReqID: N/A"""
        coordinator.start_cycle({'description': 'Macro Task'})
        micro_task = {'description': 'High cost task', 'cost_score': 0.9,
            'benefit_score': 0.2}
        should_terminate, reason = coordinator.should_terminate_recursion(
            micro_task)
        assert should_terminate == True
        assert reason == 'cost-benefit analysis'
        micro_task = {'description': 'High benefit task', 'cost_score': 0.3,
            'benefit_score': 0.8}
        should_terminate, reason = coordinator.should_terminate_recursion(
            micro_task)
        assert should_terminate == False
    ...
    @pytest.mark.parametrize('micro_task', [{'description': 'Too granular',
        'granularity_score': 0.1}, {'description': 'Too costly',
        'cost_score': 0.9, 'benefit_score': 0.2}])
    def test_create_micro_cycle_termination_succeeds(self, coordinator,
        micro_task):
...
ReqID: N/A"""
        coordinator.start_cycle({'description': 'Macro Task'})
        micro_task = {'description': 'Resource intensive task',
            'resource_usage': 0.9}
        should_terminate, reason = coordinator.should_terminate_recursion(
            micro_task)
        assert should_terminate == True
...
ReqID: N/A"""
        coordinator.start_cycle({'description': 'Macro Task'})
        micro_task = {'description': 'Task with override', 'human_override':
            'terminate'}
        should_terminate, reason = coordinator.should_terminate_recursion(
            micro_task)
        assert should_terminate == True
...
ReqID: N/A"""
        ...
        micro_cycle = coordinator.create_micro_cycle({'description':
            'Micro Task'}, Phase.EXPAND)
        micro_cycle.execute_current_phase()
        assert f'EXPAND_{micro_cycle.cycle_id}' in micro_cycle._execution_traces
        assert micro_cycle._execution_traces[f'EXPAND_{micro_cycle.cycle_id}'][
            'parent_cycle_id'] == macro_cycle_id
        assert micro_cycle._execution_traces[f'EXPAND_{micro_cycle.cycle_id}'][
            'recursion_depth'] == 1
...
ReqID: N/A"""
        coordinator.start_cycle({'description': 'Macro'})
        context = {'micro_tasks': [{'description': 'Sub1',
            'granularity_score': 0.8}, {'description': 'Sub2',
...
ReqID: N/A"""
        ...
        for i in range(coordinator.max_recursion_depth):
            current = current.create_micro_cycle({'description':
                f'level {i}'}, Phase.EXPAND)
...
ReqID: N/A"""
    ...
    def test_should_terminate_recursion_all_factors_true_succeeds(self,
        coordinator):
        """If all delimiting principles trigger, recursion should terminate.

ReqID: N/A"""
        coordinator.start_cycle({'description': 'Macro'})
        task = {'description': 'child', 'human_override': 'terminate',
            'granularity_score': 0.1, 'cost_score': 0.9, 'benefit_score': 
            0.2, 'quality_score': 0.95, 'resource_usage': 0.9}
        should_terminate, reason = coordinator.should_terminate_recursion(task)
        assert should_terminate is True
        assert reason == 'human override'

    def test_should_terminate_recursion_all_factors_false_succeeds(self,
        coordinator):
        """If no delimiting principle triggers, recursion should continue.

ReqID: N/A"""
        coordinator.start_cycle({'description': 'Macro'})
        task = {'description': 'child', 'human_override': 'continue',
            'granularity_score': 0.8, 'cost_score': 0.2, 'benefit_score': 
            0.9, 'quality_score': 0.5, 'resource_usage': 0.3}
        should_terminate, reason = coordinator.should_terminate_recursion(task)
...
ReqID: N/A"""
        coordinator.performance_metrics = {Phase.EXPAND.name: {'duration': 
            1.0}, Phase.DIFFERENTIATE.name: {'duration': 2.0}, Phase.REFINE
            .name: {'duration': 3.0}, Phase.RETROSPECT.name: {'duration': 4.0}}
...
ReqID: N/A"""
        coordinator.start_cycle({'description': 'macro'})
        micro1 = coordinator.create_micro_cycle({'description': 'child1'},
            Phase.EXPAND)
        with patch.object(micro1, '_execute_differentiate_phase',
            return_value={'done': True}):
            micro1.progress_to_phase(Phase.DIFFERENTIATE)
        micro2 = coordinator.create_micro_cycle({'description': 'child2'},
            Phase.EXPAND)
        ...
        assert {k: v for k, v in entry.items() if k != 'task'
            } == micro1.results
...
ReqID: N/A"""
        ...
        micro = coordinator.create_micro_cycle({'description': 'child'},
            Phase.EXPAND)
        with patch.object(micro, '_execute_differentiate_phase',
    ...
    def test_human_continue_overrides_delimiting_principles_succeeds(self,
        coordinator):
        """A continue override should prevent termination even if other factors trigger.

ReqID: N/A"""
        coordinator.start_cycle({'description': 'macro'})
        micro_task = {'description': 'override task', 'human_override':
            'continue', 'granularity_score': 0.1, 'cost_score': 0.9,
            'benefit_score': 0.2, 'quality_score': 0.95, 'resource_usage': 0.9}
        should_terminate, reason = coordinator.should_terminate_recursion(
            micro_task)
        assert should_terminate is False
        assert reason is None
