...
def coordinator():
    ...
    return EDRRCoordinator(memory_manager=mm, wsde_team=wsde, code_analyzer
        =ca, ast_transformer=ast, prompt_manager=pm, documentation_manager=dm)
...
ReqID: N/A"""
    ...
    with patch.object(coordinator, '_execute_expand_phase', return_value={
        'phase_complete': True}) as ex, patch.object(coordinator,
        '_execute_differentiate_phase', return_value={'phase_complete': True}
        ) as diff, patch.object(coordinator, '_execute_refine_phase',
        return_value={'phase_complete': True}) as ref, patch.object(coordinator
        , '_execute_retrospect_phase', return_value={'phase_complete': True}
        ) as ret, patch.object(coordinator, '_decide_next_phase',
...
@pytest.mark.parametrize('task,expected,reason', [({'human_override':
    'terminate'}, True, 'human override'), ({'human_override': 'continue'},
    False, None)])
def test_should_terminate_recursion_human_override_succeeds(coordinator,
...
ReqID: N/A"""
    task = {'granularity_score': 0.8, 'cost_score': 0.1, 'benefit_score': 
        0.9, 'quality_score': 0.1, 'resource_usage': 0.1}
    result = coordinator.should_terminate_recursion(task)
...
ReqID: N/A"""
    task = {'granularity_score': coordinator.DEFAULT_GRANULARITY_THRESHOLD,
...
ReqID: N/A"""
    task = {'granularity_score': coordinator.DEFAULT_GRANULARITY_THRESHOLD /
        2, 'cost_score': 0.6, 'benefit_score': 0.2}
    result = coordinator.should_terminate_recursion(task)
    assert result[0] is True
    assert result[1] == 'granularity threshold'
