...
from devsynth.application.memory.fallback import FallbackError, StoreStatus, FallbackStore, with_fallback
...
def primary_store():
    ...
    store.retrieve.return_value = MemoryItem(id='test-item-1', content='This is a test item', memory_type=MemoryType.WORKING)
    store.search.return_value = [MemoryItem(id='test-item-1', content='This is a test item', memory_type=MemoryType.WORKING)]
    store.delete.return_value = True
    store.get_all_items.return_value = [MemoryItem(id='test-item-1', content='This is a test item', memory_type=MemoryType.WORKING)]
...
def fallback_store():
    ...
    store = MagicMock()
    store.store.return_value = 'test-item-1'
    store.retrieve.return_value = MemoryItem(id='test-item-1', content='This is a test item from fallback', memory_type=MemoryType.WORKING)
    store.search.return_value = [MemoryItem(id='test-item-1', content='This is a test item from fallback', memory_type=MemoryType.WORKING)]
    store.delete.return_value = True
    store.get_all_items.return_value = [MemoryItem(id='test-item-1', content='This is a test item from fallback', memory_type=MemoryType.WORKING)]
    store.begin_transaction.return_value = 'tx-1'
    store.commit_transaction.return_value = True
    store.rollback_transaction.return_value = True
    store.is_transaction_active.return_value = True
...
class TestFallbackStore:
    ...
    def test_initialization(self, primary_store, fallback_store):
        ...
        store = FallbackStore(primary_store=primary_store, fallback_stores=[fallback_store])
    ...
    def test_begin_transaction_all_failures(self, primary_store, fallback_store):
        ...
        primary_store.begin_transaction.side_effect = ValueError('Primary store failed')
        fallback_store.begin_transaction.side_effect = ValueError('Fallback store failed')
        store = FallbackStore(primary_store=primary_store, fallback_stores=[fallback_store])
        with pytest.raises(FallbackError) as excinfo:
    ...
    def test_commit_transaction_primary_failure(self, primary_store, fallback_store):
        ...
        primary_store.commit_transaction.side_effect = ValueError('Primary store failed')
        store = FallbackStore(primary_store=primary_store, fallback_stores=[fallback_store])
        result = store.commit_transaction('tx-1')
        assert result is True
        primary_store.commit_transaction.assert_called_once_with('tx-1')
    ...
    def test_commit_transaction_all_failures(self, primary_store, fallback_store):
        ...
        primary_store.commit_transaction.side_effect = ValueError('Primary store failed')
        fallback_store.commit_transaction.side_effect = ValueError('Fallback store failed')
        store = FallbackStore(primary_store=primary_store, fallback_stores=[fallback_store])
        result = store.commit_transaction('tx-1')
        assert result is False
        assert store.store_status[primary_store] == StoreStatus.DEGRADED
    ...
    @pytest.mark.medium
    def test_reconcile_pending_operations(self, primary_store, fallback_store, memory_item):
    ...
    @pytest.mark.medium
    def test_get_pending_operations_count(self, primary_store, fallback_store, memory_item):
