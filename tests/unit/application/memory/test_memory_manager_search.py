...
class TestMemoryManagerSearch:
    ...
    def _add_vector(self, manager: MemoryManager, text: str, memory_type: Union[MemoryType, str], **metadata):
        embedding = manager._embed_text(text)
        meta = {'memory_type': memory_type.value if hasattr(memory_type, 'value') else memory_type}
    ...
    def test_search_filters_by_metadata_succeeds(self, manager):
        ...
        results = manager.search_memory('apple', memory_type=MemoryType.CODE, metadata_filter={'category': 'fruit'}, limit=5)
