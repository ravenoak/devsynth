...
@patch(
    "devsynth.application.memory.chromadb_store.chromadb.EphemeralClient", autospec=True
)
@pytest.mark.medium
def test_init_logs_error_on_bad_fallback_raises_error(
    ...
    assert any(
        ("Failed to load fallback store" in rec.message for rec in caplog.records)
    )
...
@pytest.mark.medium
def test_save_fallback_logs_error_raises_error(
    mock_client_cls, tmp_path, caplog, monkeypatch
):
    ...
    assert any(
        ("Failed to save fallback store" in rec.message for rec in caplog.records)
    )
...
@patch(
    "devsynth.application.memory.chromadb_store.chromadb.PersistentClient",
    autospec=True,
)
@pytest.mark.medium
