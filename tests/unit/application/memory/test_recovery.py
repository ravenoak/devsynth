...
from devsynth.application.memory.recovery import RecoveryError, MemorySnapshot, OperationLog, RecoveryManager, get_recovery_manager, with_recovery
...
def memory_item():
    ...
    return MemoryItem(id='test-item-1', content='This is a test item', memory_type=MemoryType.WORKING, metadata={'test': 'value'}, created_at=datetime.now())
...
class TestMemorySnapshot:
    ...
    def test_initialization(self, memory_item):
        ...
        snapshot = MemorySnapshot(store_id='test-store', items=[memory_item], metadata={'version': '1.0'})
...
class TestOperationLog:
    ...
    def test_log_operation(self):
        ...
        log = OperationLog(store_id='test-store')
        log.log_operation(operation_type='store', operation_data={'item_id': 'test-item-1'})
        ...
        log.log_operation(operation_type='delete', operation_data={'item_id': 'test-item-2'}, timestamp=timestamp)
    ...
    def test_save_and_load(self, temp_dir):
        ...
        log = OperationLog(store_id='test-store')
        log.log_operation(operation_type='store', operation_data={'item_id': 'test-item-1'})
        log.log_operation(operation_type='delete', operation_data={'item_id': 'test-item-2'})
    ...
    def test_replay(self, memory_item):
        ...
        log = OperationLog(store_id='test-store')
        log.log_operation(operation_type='store', operation_data={'item': memory_item.to_dict()})
        log.log_operation(operation_type='delete', operation_data={'item_id': 'test-item-2'})
    ...
    def test_replay_with_time_range(self):
        ...
        log.log_operation(operation_type='store', operation_data={'item_id': 'test-item-1'}, timestamp=timestamp1)
        log.log_operation(operation_type='store', operation_data={'item_id': 'test-item-2'}, timestamp=timestamp2)
        log.log_operation(operation_type='store', operation_data={'item_id': 'test-item-3'}, timestamp=timestamp3)
        mock_store = MagicMock()
        start_time = datetime(2025, 1, 1, 18, 0, 0)
        end_time = datetime(2025, 1, 2, 18, 0, 0)
        replayed_count = log.replay(mock_store, start_time=start_time, end_time=end_time)
...
class TestRecoveryManager:
    ...
    def test_create_snapshot(self, memory_item):
        ...
        snapshot = manager.create_snapshot(store_id='test-store', store=mock_store, metadata={'version': '1.0'})
        assert snapshot.store_id == 'test-store'
        assert len(snapshot.items) == 1
        assert snapshot.items[0].id == 'test-item-1'
        assert snapshot.metadata == {'version': '1.0'}
    ...
    def test_log_operation(self):
        """Test logging an operation."""
        manager = RecoveryManager()
        manager.log_operation(store_id='test-store', operation_type='store', operation_data={'item_id': 'test-item-1'})
    ...
    def test_restore_from_snapshot_no_snapshot(self):
        ...
        manager = RecoveryManager()
        mock_store = MagicMock()
        result = manager.restore_from_snapshot(store_id='non-existent', store=mock_store)
        assert result is False
    ...
    def test_recover_store(self, memory_item):
        ...
        manager = RecoveryManager()
        snapshot = MemorySnapshot(store_id='test-store', items=[memory_item], created_at=datetime(2025, 1, 1, 12, 0, 0))
        manager.snapshots['test-store'] = snapshot
        log = OperationLog(store_id='test-store')
        log.log_operation(operation_type='delete', operation_data={'item_id': 'test-item-1'}, timestamp=datetime(2025, 1, 2, 12, 0, 0))
