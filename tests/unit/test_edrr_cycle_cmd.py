...
def mock_components():
    with patch(
        ...
        # Add phase_metrics attribute for EnhancedEDRRCoordinator
...
def test_edrr_cycle_cmd_no_input(mock_bridge, mock_config):
...
def test_edrr_cycle_cmd_manifest_success(tmp_path, mock_bridge, mock_components, mock_config, mock_tqdm):
    ...
    # Check that insights and next steps are printed
    assert any(
        "Key Insights" in call.args[0]
...
def test_edrr_cycle_cmd_prompt_success(mock_bridge, mock_components, mock_config, mock_tqdm):
    """Test successful execution with a prompt."""
    coordinator, memory_manager, coord_cls, run_pipeline_mock = mock_components
    _, mock_progress = mock_tqdm

    edrr_cycle_cmd(prompt="Improve error handling", context="In the API endpoints", max_iterations=5)
    ...
    # Check that start_cycle was called with the correct task
    ...
    # Check that progress_to_phase was called for each phase
    assert coordinator.progress_to_phase.call_count == 4

    # Check progress bar updates
    assert mock_progress.update.call_count == 4

    # Check that the final report was generated and stored
    memory_manager.store_with_edrr_phase.assert_called_once_with(
        coordinator.generate_report.return_value,
        "EDRR_CYCLE_RESULTS",
        Phase.RETROSPECT.value,
...
def test_edrr_cycle_cmd_custom_bridge(mock_components, mock_config):
    ...
    # Check that the custom bridge was used
    custom_bridge.print.assert_called()

    # Check that the coordinator was initialized and used
    coordinator.start_cycle.assert_called_once()
    coordinator.progress_to_phase.assert_called()


def test_edrr_cycle_cmd_error_handling(mock_bridge, mock_components, mock_config):
