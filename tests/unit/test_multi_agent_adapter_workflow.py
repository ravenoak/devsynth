...
ReqID: N/A"""
    ...
    def setup_method(self):
        ...
        self.agent2.config.parameters = {'expertise': ['javascript',
            'frontend']}
...
ReqID: N/A"""
        ...
        with patch.object(self.team, 'build_consensus', return_value={
            'consensus': 'done', 'contributors': ['PythonAgent', 'JSAgent',
            'DocAgent'], 'method': 'consensus_synthesis', 'reasoning': ''}
            ) as mock_consensus, patch.object(self.team,
            'select_primus_by_expertise', wraps=self.team.
            select_primus_by_expertise) as mock_select:
        ...
        assert set(result['contributors']) == {'PythonAgent', 'JSAgent',
            'DocAgent'}
