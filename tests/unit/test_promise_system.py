...
ReqID: N/A"""
        ...
        def stringify(x: int) ->str:
            return f'Result: {x}'
        final_promise = initial_promise.then(double).then(add_ten).then(
            stringify)
...
ReqID: N/A"""
        ...
        def handle_error(e: Exception) ->str:
            return f'Handled: {e}'
        final_promise = initial_promise.then(double).then(will_fail).then(
            add_ten).catch(handle_error)
        initial_promise.resolve(5)
        assert final_promise.is_fulfilled
...
ReqID: N/A"""
        promises: List[Promise[Any]] = [Promise[int](), Promise[str](),
            Promise[float]()]
