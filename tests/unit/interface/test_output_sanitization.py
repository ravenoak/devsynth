...
def test_webapp_cmd_error_sanitized(monkeypatch):
    ...
    monkeypatch.setattr(
        "devsynth.interface.ux_bridge.sanitize_output", lambda x: str(x)
    )
    ...
    monkeypatch.setattr(
        "devsynth.application.cli.cli_commands.os.makedirs", boom
    )
