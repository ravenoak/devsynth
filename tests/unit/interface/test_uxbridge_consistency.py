...
def _cli_bridge(monkeypatch):
    ...
    monkeypatch.setattr('devsynth.interface.cli.CLIProgressIndicator',
        DummyProgress)
...
def _web_bridge(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules,
        'devsynth.application.cli.setup_wizard', setup_module)
    ...
    monkeypatch.setitem(sys.modules,
        'devsynth.application.cli.commands.align_cmd', align_module)
    ...
    return WebUI(), {'write': st.write, 'markdown': st.markdown, 'progress':
        prog, 'sanitize': sanitizer}


def _api_bridge(monkeypatch):
    sanitizer = MagicMock(side_effect=lambda x: f'S:{x}')
    monkeypatch.setattr('devsynth.interface.agentapi.sanitize_output',
        sanitizer)
    return APIBridge(['foo', True]), {'sanitize': sanitizer}


@pytest.mark.parametrize('factory', [_cli_bridge, _web_bridge, _api_bridge])
def test_bridge_contract_succeeds(factory, monkeypatch):
