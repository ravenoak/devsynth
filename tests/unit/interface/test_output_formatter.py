...
ReqID: N/A"""
    ...
    def test_detect_message_type_succeeds(self, formatter):
        ...
        assert formatter.detect_message_type('CRITICAL: System failure'
            ) == 'error'
        assert formatter.detect_message_type('WARNING: Be careful'
            ) == 'warning'
        assert formatter.detect_message_type('SUCCESS: Task completed'
            ) == 'success'
        assert formatter.detect_message_type('COMPLETED: All done'
            ) == 'success'
        assert formatter.detect_message_type('Task completed successfully'
            ) == 'success'
        assert formatter.detect_message_type('INFO: Just FYI') == 'info'
        assert formatter.detect_message_type('NOTE: Take note of this'
            ) == 'info'
        assert formatter.detect_message_type('# Heading 1') == 'heading'
        assert formatter.detect_message_type('## Heading 2') == 'heading'
        assert formatter.detect_message_type('Just a normal message'
            ) == 'normal'
    ...
    def test_display_succeeds(self, formatter, console):
        ...
        console.print.assert_called_once()
        assert isinstance(console.print.call_args[0][0], Text)
        assert str(console.print.call_args[0][0]
