...
def _setup_cli(monkeypatch):
    ...
    original_sanitize = getattr(sys.modules["devsynth.interface.cli"], "sanitize_output", None)
    ...
    monkeypatch.setattr("rich.prompt.Prompt.ask", lambda *args, **kwargs: kwargs.get("default", "foo"))
...
def clean_state():
    ...
    original_sanitize_func = getattr(sys.modules.get(original_sanitize_module, {}), "sanitize_output", None)
    ...
    if "devsynth.interface.webui_bridge" in sys.modules:
        ...
        if hasattr(bridge_module, "WebUIBridge"):
            ...
            if hasattr(WebUIBridge, "messages") and isinstance(WebUIBridge.messages, list):
...
def test_prompt_and_result_consistency(monkeypatch, clean_state):
    ...
    original_sanitize = getattr(sys.modules.get(sanitize_output.__module__, {}), "sanitize_output", None)
    ...
    try:
        ...
        if original_sanitize is not None:
            setattr(sys.modules[sanitize_output.__module__], "sanitize_output", original_sanitize)