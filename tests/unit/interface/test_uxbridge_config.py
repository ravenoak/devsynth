...
class TestUXBridgeConfig:
    ...
    def test_apply_uxbridge_settings(self):
        ...
        # Apply settings to a mock bridge
        bridge = apply_uxbridge_settings(MockUXBridge, mock_config, test_arg="test")
        
        # Verify the bridge was created with the correct arguments
    ...
    def test_get_default_bridge_cli(self, mock_cli_bridge):
        """Test getting the default bridge when CLI is configured."""
        # Create a mock configuration with CLI as default
        mock_config = MagicMock(spec=ProjectUnifiedConfig)
        mock_config.config.uxbridge_settings = {"default_interface": "cli"}
        mock_config.config.features = {"uxbridge_webui": False, "uxbridge_agent_api": False}
        
        # Mock the CLIUXBridge class
        mock_cli_bridge.return_value = MockUXBridge()
        
        # Get the default bridge
        bridge = get_default_bridge(mock_config)
        
        # Verify the correct bridge was returned
        assert isinstance(bridge, MockUXBridge)
        mock_cli_bridge.assert_called_once()
    
    @patch("devsynth.interface.uxbridge_config.WebUI", create=True)
    def test_get_default_bridge_webui(self, mock_webui):
        """Test getting the default bridge when WebUI is configured."""
        # Create a mock configuration with WebUI as default
        mock_config = MagicMock(spec=ProjectUnifiedConfig)
        mock_config.config.uxbridge_settings = {"default_interface": "webui"}
        mock_config.config.features = {"uxbridge_webui": True, "uxbridge_agent_api": False}
        
        # Mock the WebUI class
    ...
    def test_get_default_bridge_api(self, mock_api_bridge):
        """Test getting the default bridge when API is configured."""
        # Create a mock configuration with API as default
        mock_config = MagicMock(spec=ProjectUnifiedConfig)
        mock_config.config.uxbridge_settings = {"default_interface": "api"}
        mock_config.config.features = {"uxbridge_webui": False, "uxbridge_agent_api": True}
        
        # Mock the APIBridge class
    ...
    @patch("devsynth.interface.uxbridge_config.CLIUXBridge")
    @patch("devsynth.interface.uxbridge_config.WebUI", create=True, side_effect=ImportError)
    def test_get_default_bridge_webui_fallback(self, mock_webui, mock_cli_bridge):
    ...
    @patch("devsynth.interface.uxbridge_config.CLIUXBridge")
    @patch("devsynth.interface.uxbridge_config.APIBridge", create=True, side_effect=ImportError)
    def test_get_default_bridge_api_fallback(self, mock_api_bridge, mock_cli_bridge):
