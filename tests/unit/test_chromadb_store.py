...
ReqID: N/A"""
        item = MemoryItem(id='test-item', content='This is a test item',
            memory_type=MemoryType.LONG_TERM, metadata={'test': 'metadata'})
...
ReqID: N/A"""
        items = [MemoryItem(id='item-1', content='This is item 1',
            memory_type=MemoryType.LONG_TERM, metadata={'category': 'test',
            'priority': 'high'}), MemoryItem(id='item-2', content=
            'This is item 2', memory_type=MemoryType.SHORT_TERM, metadata={
            'category': 'test', 'priority': 'medium'}), MemoryItem(id=
            'item-3', content='This is item 3', memory_type=MemoryType.
            LONG_TERM, metadata={'category': 'production', 'priority': 'high'})
            ]
        for item in items:
            self.store.store(item)
        results = self.store.search({'memory_type': MemoryType.LONG_TERM})
        no_file_logging = os.environ.get('DEVSYNTH_NO_FILE_LOGGING', '0'
            ).lower() in ('1', 'true', 'yes')
        if no_file_logging:
            print(
...
ReqID: N/A"""
        items = [MemoryItem(id='item-1', content=
            ...
            memory_type=MemoryType.LONG_TERM, metadata={'topic':
            'Python programming'}), MemoryItem(id='item-2', content=
            'Machine learning algorithms can learn from data', memory_type=
            MemoryType.LONG_TERM, metadata={'topic': 'Machine learning'}),
            MemoryItem(id='item-3', content=
            'Web development involves creating websites and web applications',
            memory_type=MemoryType.LONG_TERM, metadata={'topic':
            'Web development'})]
        for item in items:
            self.store.store(item)
        results = self.store.search({'semantic_query':
            'Programming languages and software development'})
        self.assertTrue(len(results) > 0)
        relevant_topics = ['Python programming', 'Web development']
        self.assertTrue(any(item.metadata['topic'] in relevant_topics for
            item in results[:2]))
...
ReqID: N/A"""
        items = [MemoryItem(id=f'item-{i}', content=
            f'This is test item {i}', memory_type=MemoryType.LONG_TERM,
            metadata={'test': 'metadata'}) for i in range(5)]
        for item in items:
            self.store.store(item)
