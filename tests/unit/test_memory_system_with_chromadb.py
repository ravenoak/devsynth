...
ReqID: N/A"""
    ...
    def memory_system(self, temp_dir):
        ...
        config = {'memory_store_type': 'chromadb', 'memory_file_path':
            temp_dir, 'vector_store_enabled': True,
...
ReqID: N/A"""
        config = {'memory_store_type': 'chromadb', 'memory_file_path':
            temp_dir, 'vector_store_enabled': False}
        memory_system = MemorySystemAdapter(config=config)
        assert memory_system.memory_store is not None
...
ReqID: N/A"""
        memory_item = MemoryItem(id='test-item-1', content=
            'This is a test item', memory_type=MemoryType.LONG_TERM,
...
ReqID: N/A"""
        ...
        context_manager.add_to_context('vector_data', {'embeddings': [0.1, 
            0.2, 0.3]})
