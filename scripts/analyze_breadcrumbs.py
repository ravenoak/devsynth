...
def analyze_documentation_breadcrumbs(docs_dir: Path) -> Dict[str, List[Tuple[int, str]]]:
...
def generate_report(breadcrumb_data: Dict[str, List[Tuple[int, str]]], validate: bool = False) -> None:
    """Generate analysis report of breadcrumb patterns."""
    total_files = len(breadcrumb_data)
    total_breadcrumbs = sum(len(breadcrumbs) for breadcrumbs in breadcrumb_data.values())
    duplicate_files = sum(1 for breadcrumbs in breadcrumb_data.values() if len(breadcrumbs) > 1)
    total_duplicates = sum(len(breadcrumbs) - 1 for breadcrumbs in breadcrumb_data.values() if len(breadcrumbs) > 1)
...
def main():
    parser = argparse.ArgumentParser(description="Analyze breadcrumb patterns in documentation")
    parser.add_argument("--validate", action="store_true", help="Validate that no duplicates exist")
