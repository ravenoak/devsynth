...
try:
    ...
    print("Error: common_test_collector.py not found or required functions not available.")
    sys.exit(1)
...
def analyze_apply_speed_markers(test_path: str, verbose: bool = False) -> Dict[str, Any]:
    """
...
def analyze_common_test_collector(test_path: str, verbose: bool = False) -> Dict[str, Any]:
    """
...
def compare_analyses(apply_analysis: Dict[str, Any], collector_analysis: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    collector_detection = collector_analysis.get("common_test_collector_detection", "none")
    ...
    return {
        ...
        "discrepancy_type": determine_discrepancy_type(apply_analysis, collector_analysis) if discrepancy else None
    }

def determine_discrepancy_type(apply_analysis: Dict[str, Any], collector_analysis: Dict[str, Any]) -> str:
    """
...
def main():
    ...
    print(f"Analyzing marker application and detection in {args.sample_size} test files...")
    ...
    for file_path, test_function in all_test_functions:
        ...
        if comparison.get("discrepancy", False):
            ...
            discrepancy_types[discrepancy_type] = discrepancy_types.get(discrepancy_type, 0) + 1
            
            if args.verbose:
                print(f"  Discrepancy found: {discrepancy_type}")
                print(f"    apply_speed_markers: {apply_analysis.get('existing_markers', [])}")
                print(f"    common_test_collector: {collector_analysis.get('marker_type')}")
    ...
    report = {
        ...
        "discrepancy_rate": discrepancy_count / len(all_test_functions) if all_test_functions else 0,
    ...
    print(f"Discrepancies found: {discrepancy_count} out of {len(all_test_functions)} ({report['discrepancy_rate']:.2%})")
    print("\nDiscrepancy types:")
