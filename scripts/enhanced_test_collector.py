...
def get_tests_with_markers(marker_types: List[str] = ["fast", "medium", "slow"], 
                          use_cache: bool = True) -> Dict[str, Dict[str, List[str]]]:
    """
    ...
    for category in TEST_CATEGORIES:
        ...
        # Use the enhanced parser for non-behavior tests
        if category != "behavior":
            category_markers = enhanced_test_parser.get_tests_with_markers(directory, marker_types, use_cache)
        else:
            # Use the original implementation for behavior tests
            category_markers = {}
            all_markers = common_test_collector.get_tests_with_markers(marker_types, use_cache)
...
def get_marker_counts(use_cache: bool = True) -> Dict[str, Dict[str, int]]:
    ...
    for category in TEST_CATEGORIES:
        ...
        # Use the enhanced parser for non-behavior tests
        if category != "behavior":
            category_counts = enhanced_test_parser.get_marker_counts(directory, use_cache)
        else:
            # Use the original implementation for behavior tests
            all_counts = common_test_collector.get_marker_counts(use_cache)
            category_counts = all_counts.get(category, {"fast": 0, "medium": 0, "slow": 0})
...
def check_test_has_marker(test_path: str, marker_types: List[str] = ["fast", "medium", "slow"]) -> Tuple[bool, Optional[str]]:
    """
...
def generate_test_isolation_report(directory: str = "tests") -> Dict[str, Any]:
    ...
    try:
        ...
        print("Warning: test_isolation_analyzer not found, falling back to basic analysis")
...
if __name__ == "__main__":
    ...
    if args.verify_counts:
        ...
        print(f"Total: {results['total']['enhanced_count']} tests (enhanced), {results['total']['pytest_count']} tests (pytest)")
        print(f"Total discrepancy: {results['total']['discrepancy']}")
        
        for category, cat_results in results["categories"].items():
            print(f"{category}: {cat_results['enhanced_count']} tests (enhanced), {cat_results['pytest_count']} tests (pytest), discrepancy: {cat_results['discrepancy']}")
        ...
        for marker in ["fast", "medium", "slow"]:
            print(f"{marker}: {results['markers'][marker]['enhanced_count']} tests (enhanced), {results['markers'][marker]['pytest_count']} tests (pytest), discrepancy: {results['markers'][marker]['discrepancy']}")
