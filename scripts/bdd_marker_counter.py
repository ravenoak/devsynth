...
def update_task_progress(bdd_counts: Dict[str, int]) -> None:
    ...
    for i, line in enumerate(lines):
        if ("tests now have markers" in line or "out of" in line and "tests" in line and "%" in line) or \
           ("Improved from" in line and "tests" in line and "%" in line):
            # Update the line with the latest counts
            lines[i] = f"  - Improved from 124 to {total_markers} out of {total_tests} tests ({percentage:.2f}%) with speed markers\n"
            updated = True
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Count markers in BDD step definition files')
    parser.add_argument('--update-progress', action='store_true', help='Update project status file with the latest counts')
    parser.add_argument('--dir', default=BDD_STEP_DIR, help='Directory containing BDD step definitions')
    ...
    print(f"  Total: {sum(bdd_counts[m] for m in ['fast', 'medium', 'slow', 'isolation'])}")
    print(f"  Files with markers: {bdd_counts['files_with_markers']} out of {bdd_counts['total_files']}")
    ...
    print(f"  Total: {sum(common_counts['total'][m] for m in ['fast', 'medium', 'slow']) + sum(bdd_counts[m] for m in ['fast', 'medium', 'slow'])}")
