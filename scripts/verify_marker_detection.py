...
try:
    from common_test_collector import get_marker_counts, get_tests_with_markers, clear_cache
except ImportError:
    print("Error: common_test_collector.py not found. This script requires common_test_collector.py.")
    sys.exit(1)
...
def parse_args():
    ...
    parser.add_argument(
        '--fix',
        action='store_true',
        help='Attempt to fix any discrepancies found'
    )
    parser.add_argument(
        '--report-file',
        type=str,
...
def load_apply_speed_markers_report() -> Dict[str, Any]:
    ...
    if not os.path.exists(report_path):
        print(f"Warning: {report_path} not found. Running apply_speed_markers.py to generate it...")
        try:
            subprocess.run(
...
def find_discrepancies(apply_report: Dict[str, Any], collector_counts: Dict[str, int]) -> Dict[str, Any]:
    """
    ...
    discrepancies["total_difference"] = discrepancies["total_markers"]["apply_speed_markers"] - discrepancies["total_markers"]["common_test_collector"]
    discrepancies["by_type"]["fast"]["difference"] = discrepancies["by_type"]["fast"]["apply_speed_markers"] - discrepancies["by_type"]["fast"]["common_test_collector"]
    discrepancies["by_type"]["medium"]["difference"] = discrepancies["by_type"]["medium"]["apply_speed_markers"] - discrepancies["by_type"]["medium"]["common_test_collector"]
    discrepancies["by_type"]["slow"]["difference"] = discrepancies["by_type"]["slow"]["apply_speed_markers"] - discrepancies["by_type"]["slow"]["common_test_collector"]
...
def main():
    ...
    print(f"Total markers (apply_speed_markers.py): {discrepancies['total_markers']['apply_speed_markers']}")
    print(f"Total markers (common_test_collector.py): {discrepancies['total_markers']['common_test_collector']}")
    print(f"Difference: {discrepancies['total_difference']}")
    
    print("\nBy marker type:")
    for marker_type in ["fast", "medium", "slow"]:
        print(f"  {marker_type.capitalize()} markers (apply_speed_markers.py): {discrepancies['by_type'][marker_type]['apply_speed_markers']}")
        print(f"  {marker_type.capitalize()} markers (common_test_collector.py): {discrepancies['by_type'][marker_type]['common_test_collector']}")
    ...
    if discrepancies["total_difference"] != 0:
        ...
        if problematic_files:
            ...
            print("No problematic files found. The discrepancy might be due to other issues.")
    else:
        print("\nNo discrepancies found! Marker detection is consistent.")
    
    # Save the report
    report = {
        "timestamp": str(Path(args.report_file).stat().st_mtime) if Path(args.report_file).exists() else None,
        "apply_speed_markers": apply_report,
        "common_test_collector": {
            "marker_counts": collector_counts
        },
        "discrepancies": discrepancies,
        "problematic_files": problematic_files if "problematic_files" in locals() else []
    }
