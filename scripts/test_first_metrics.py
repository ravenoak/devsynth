...
def calculate_metrics(commits: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for commit in commits:
        ...
        for file in commit["files"]:
            ...
            elif path.startswith("tests/") and (path.endswith(".py") or path.endswith(".feature")):
                has_test = True
    ...
    for author in metrics["by_author"]:
        if metrics["by_author"][author]["total_commits"] > 0:
            metrics["by_author"][author]["test_first_percentage"] = round(
                ...
                metrics["by_author"][author]["total_commits"] * 100, 2
            )
    ...
    for date in metrics["by_date"]:
        if metrics["by_date"][date]["total_commits"] > 0:
            metrics["by_date"][date]["test_first_percentage"] = round(
                ...
                metrics["by_date"][date]["total_commits"] * 100, 2
            )
...
def generate_metrics_report(metrics: Dict[str, Any]) -> str:
    ...
    report.append(f"Implementation-first commits: {metrics['implementation_first_commits']}")
    ...
    for author, author_metrics in metrics["by_author"].items():
        ...
        report.append(f"Implementation-first commits: {author_metrics['implementation_first_commits']}")
        report.append(f"Test-first percentage: {author_metrics['test_first_percentage']}%")
        report.append("")
    ...
    for date, date_metrics in sorted(metrics["by_date"].items()):
        ...
        report.append(f"Implementation-first commits: {date_metrics['implementation_first_commits']}")
        report.append(f"Test-first percentage: {date_metrics['test_first_percentage']}%")
        report.append("")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate test-first development metrics")
    parser.add_argument("--days", type=int, default=30, help="Number of days to look back in the commit history")
    parser.add_argument("--output", type=str, help="Path to the output file for the metrics JSON")
