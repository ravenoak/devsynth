...
def load_cache() -> Dict[str, Any]:
    ...
    if os.path.exists(TEST_CACHE_FILE):
        try:
            ...
            return {"metadata": {"last_updated": datetime.datetime.now().isoformat(), "file_timestamps": {}}}
    return {"metadata": {"last_updated": datetime.datetime.now().isoformat(), "file_timestamps": {}}}
...
def get_test_failure_rates(use_cache: bool = True) -> Dict[str, float]:
    ...
    for test_path, test_history in history["tests"].items():
        if test_history["total_runs"] > 0:
            failure_rates[test_path] = test_history["failures"] / test_history["total_runs"]
        else:
            failure_rates[test_path] = 0.0
    
    return failure_rates

def get_frequently_failing_tests(threshold: float = 0.1, min_runs: int = 3, use_cache: bool = True) -> List[Dict[str, Any]]:
    """
...
def calculate_test_complexity(test_path: str, use_cache: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if test_name:
            ...
            maintainability_metrics = ast_workflow._calculate_maintainability(file_analysis)
            
            metrics.update({
                "complexity": complexity_metrics,
                "readability": readability_metrics,
                "maintainability": maintainability_metrics,
                "risk_score": 1.0 - ((complexity_metrics + readability_metrics + maintainability_metrics) / 3.0)
            })
        else:
...
def collect_behavior_tests(directory: str, use_cache: bool = True) -> List[str]:
    ...
    try:
        ...
        print("Warning: enhanced_gherkin_parser not found, falling back to basic parser")
        ...
        for feature_file in feature_files:
            with open(feature_file, 'r') as f:
                ...
                scenario_matches = re.finditer(r'^\s*Scenario:(.+)$', content, re.MULTILINE)
                ...
                outline_matches = re.finditer(r'^\s*Scenario Outline:(.+)$', content, re.MULTILINE)
...
def get_tests_with_complexity(tests: List[str], use_cache: bool = True) -> List[Dict[str, Any]]:
    """
...
def get_high_risk_tests(tests: List[str], threshold: float = 0.5, use_cache: bool = True) -> List[Dict[str, Any]]:
    """
    ...
    high_risk_tests.sort(key=lambda x: x["complexity"].get("risk_score", 0), reverse=True)
    
    return high_risk_tests

def analyze_test_dependencies(test_paths: List[str], use_cache: bool = True) -> Dict[str, List[str]]:
    """
...
def get_dependent_tests(test_path: str, dependencies: Dict[str, List[str]]) -> List[str]:
    """
...
def generate_junit_xml_report(tests: List[str], test_results: Optional[Dict[str, bool]] = None, output_file: str = "test_report.xml") -> None:
    """
...
def generate_json_report(tests: List[str], test_results: Optional[Dict[str, bool]] = None, 
                         complexity_metrics: Optional[Dict[str, Dict[str, Any]]] = None,
                         dependencies: Optional[Dict[str, List[str]]] = None,
                         failure_rates: Optional[Dict[str, float]] = None,
                         output_file: str = "test_report.json") -> None:
    """
...
def check_test_has_marker(test_path: str, marker_types: List[str] = ["fast", "medium", "slow"], 
                      record_timestamp: bool = True) -> Tuple[bool, Optional[str]]:
    """
            ...
            if class_match:
                ...
                if test_match:
                    ...
                    for marker_type in marker_types:
                        ...
                        if marker_pattern.search(test_context_before) or marker_pattern.search(test_context_after):
...
def get_tests_with_markers(marker_types: List[str] = ["fast", "medium", "slow"], use_cache: bool = True) -> Dict[str, Dict[str, List[str]]]:
    """
    ...
    if use_cache and cache_key in cache:
        ...
        if last_updated:
            try:
                last_updated_time = datetime.datetime.fromisoformat(last_updated).timestamp()
                                ...
                                for file in files:
                                    if file.endswith(".py"):
                                        ...
                                        if os.path.exists(file_path):
                                            ...
                                            cached_mtime = cached_timestamps.get(file_path, 0)
                                            
                                            # If the file has been modified or is new, invalidate the cache
                                            if current_mtime > cached_mtime:
                                                print(f"File {file_path} has been modified, invalidating cache")
                                                cache_valid = False
                                                break
                                if not cache_valid:
                                    break
                ...
                cache_valid = False
                print(f"Invalid last_updated timestamp: {last_updated}, invalidating cache")
                ...
                for root, _, files in os.walk(test_dir):
                    for file in files:
                        if file.endswith(".py"):
                            file_path = os.path.join(root, file)
                            if os.path.exists(file_path):
                                cache["metadata"]["file_timestamps"][file_path] = os.path.getmtime(file_path)
...
def get_marker_counts(use_cache: bool = True) -> Dict[str, Dict[str, int]]:
    ...
    for category in tests_with_markers:
        ...
        for marker_type in tests_with_markers[category]:
            result[category][marker_type] = len(tests_with_markers[category][marker_type])
    
    # Add totals
    result["total"] = {}
    for marker_type in ["fast", "medium", "slow"]:
        result["total"][marker_type] = sum(result[category][marker_type] for category in TEST_CATEGORIES)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--category", choices=list(TEST_CATEGORIES.keys()) + ["all"], default="all", help="Test category to collect")
    parser.add_argument("--count", action="store_true", help="Show test counts")
    parser.add_argument("--markers", action="store_true", help="Show tests with markers")
    parser.add_argument("--marker-counts", action="store_true", help="Show counts of tests with markers")
    parser.add_argument("--list-marked-tests", action="store_true", help="List all tests with markers")
    parser.add_argument("--marker-type", choices=["fast", "medium", "slow"], default="medium", help="Marker type to list")
    ...
    complexity_group.add_argument("--complexity", action="store_true", help="Show complexity metrics for tests")
    complexity_group.add_argument("--high-risk", action="store_true", help="Show high-risk tests")
    complexity_group.add_argument("--risk-threshold", type=float, default=0.5, help="Risk score threshold (0-1, higher means more risky)")
    complexity_group.add_argument("--sort-by-risk", action="store_true", help="Sort tests by risk score (highest first)")
    complexity_group.add_argument("--limit", type=int, default=0, help="Limit the number of tests shown")
    
    # Add dependency-related arguments
    dependency_group = parser.add_argument_group("Dependency Analysis")
    dependency_group.add_argument("--dependencies", action="store_true", help="Show dependencies between tests")
    dependency_group.add_argument("--dependents", metavar="TEST_PATH", help="Show tests that depend on the specified test")
    dependency_group.add_argument("--sort-by-dependencies", action="store_true", help="Sort tests by number of dependents (highest first)")
    dependency_group.add_argument("--min-dependents", type=int, default=0, help="Show only tests with at least this many dependents")
    
    # Add failure history-related arguments
    failure_group = parser.add_argument_group("Failure History")
    failure_group.add_argument("--record-results", metavar="RESULTS_FILE", help="Record test results from a JSON file (format: {\"test_path\": true/false, ...})")
    failure_group.add_argument("--failure-rates", action="store_true", help="Show failure rates for tests")
    failure_group.add_argument("--frequently-failing", action="store_true", help="Show frequently failing tests")
    failure_group.add_argument("--failure-threshold", type=float, default=0.1, help="Failure rate threshold (0-1, higher means more failures)")
    failure_group.add_argument("--min-runs", type=int, default=3, help="Minimum number of runs required for failure analysis")
    failure_group.add_argument("--sort-by-failures", action="store_true", help="Sort tests by failure rate (highest first)")
    
    # Add filtering-related arguments
    filter_group = parser.add_argument_group("Test Filtering")
    filter_group.add_argument("--pattern", metavar="PATTERN", help="Filter tests by name pattern (supports * and ? wildcards)")
    filter_group.add_argument("--module", metavar="MODULE", help="Filter tests by module name")
    filter_group.add_argument("--exclude-pattern", metavar="PATTERN", help="Exclude tests matching the pattern")
    filter_group.add_argument("--exclude-module", metavar="MODULE", help="Exclude tests from the specified module")
    
    # Add reporting-related arguments
    report_group = parser.add_argument_group("Reporting")
    report_group.add_argument("--junit-xml", metavar="FILE", help="Generate a JUnit XML report")
    report_group.add_argument("--json-report", metavar="FILE", help="Generate a JSON report")
    report_group.add_argument("--include-complexity", action="store_true", help="Include complexity metrics in the JSON report")
    report_group.add_argument("--include-dependencies", action="store_true", help="Include dependencies in the JSON report")
    report_group.add_argument("--include-failure-rates", action="store_true", help="Include failure rates in the JSON report")
    report_group.add_argument("--use-test-results", metavar="RESULTS_FILE", help="Use test results from a JSON file for the report")
    ...
    if args.markers:
        ...
        for category in tests_with_markers:
            print(f"  {category}:")
            for marker_type in tests_with_markers[category]:
                print(f"    {marker_type}: {len(tests_with_markers[category][marker_type])}")
    ...
    if args.list_marked_tests:
        ...
        for category in tests_with_markers:
            if marker_type in tests_with_markers[category] and tests_with_markers[category][marker_type]:
                print(f"  {category}:")
                for test_path in tests_with_markers[category][marker_type]:
                    print(f"    {test_path}")
                    # Check if the file exists
                    if "::" in test_path:
                        ...
                        if os.path.exists(file_path):
                            ...
                            with open(file_path, 'r') as f:
                                ...
                                print(f"      First 5 lines: {first_lines.strip().replace('\n', ' | ')}")
        ...
        if os.path.exists(args.record_results):
            try:
                ...
                if not isinstance(test_results, dict):
                    print(f"Error: Results file must contain a JSON object mapping test paths to boolean results")
                    sys.exit(1)
                ...
                print(f"Failed tests: {sum(1 for result in test_results.values() if not result)}")
                print(f"Pass rate: {sum(1 for result in test_results.values() if result) / len(test_results) * 100:.1f}%")
    ...
    if args.junit_xml or args.json_report:
        ...
        if args.json_report:
            ...
            if args.include_dependencies:
                ...
                if not isinstance(dependencies_result, dict) or "error" not in dependencies_result:
    ...
    if args.complexity or args.high_risk or args.sort_by_risk or args.dependencies or args.dependents or args.sort_by_dependencies or args.failure_rates or args.frequently_failing or args.sort_by_failures:
        # Collect tests
        if args.category == "all":
            tests = collect_tests(use_cache=use_cache)
        else:
        ...
        if args.failure_rates or args.frequently_failing or args.sort_by_failures:
            ...
            if args.frequently_failing:
                ...
                print(f"Frequently failing tests (failure rate >= {args.failure_threshold * 100:.1f}%, min runs: {args.min_runs}):")
                ...
                for test in frequently_failing:
                    print(f"  {test['path']} (failure rate: {test['failure_rate'] * 100:.1f}%, {test['failures']}/{test['total_runs']} runs)")
                ...
                tests_with_runs = [
                    ...
                    if test in history["tests"] and history["tests"][test]["total_runs"] >= args.min_runs
                ]
                
                # Sort by failure rate if requested
                if args.sort_by_failures:
                    tests_with_runs.sort(key=lambda x: failure_rates.get(x, 0), reverse=True)
                ...
                for test_path in tests_with_runs:
                    ...
                    print(f"  {test_path} (failure rate: {failure_rate * 100:.1f}%, {test_history['failures']}/{test_history['total_runs']} runs)")
            ...
            # If only sorting by failures without showing them
                ...
                for test_path in tests_with_runs:
                    test_history = history["tests"].get(test_path, {"total_runs": 0, "failures": 0})
                    failure_rate = failure_rates.get(test_path, 0)
                    print(f"  {test_path} (failure rate: {failure_rate * 100:.1f}%, {test_history['failures']}/{test_history['total_runs']} runs)")
        ...
        elif args.dependencies or args.dependents or args.sort_by_dependencies or args.min_dependents > 0:
            ...
            if isinstance(dependencies, dict) and "error" in dependencies:
                ...
                if args.dependents:
                    ...
                    if args.sort_by_dependencies:
                        sorted_tests = sorted(tests, key=lambda x: dependent_counts.get(x, 0), reverse=True)
                    else:
                        sorted_tests = tests
                    
                    # Filter by minimum number of dependents
                    if args.min_dependents > 0:
                        sorted_tests = [t for t in sorted_tests if dependent_counts.get(t, 0) >= args.min_dependents]
                    ...
                    for test_path in sorted_tests:
                        ...
                        if len(dependents) > 0:
                            for dependent in dependents[:5]:  # Show only first 5 dependents
                                print(f"    {dependent}")
                            if len(dependents) > 5:
                ...
                # If only sorting by dependencies without showing them
                    ...
                    for test_path in sorted_tests:
                        print(f"  {test_path} ({dependent_counts.get(test_path, 0)} dependents)")
        ...
        # Handle high-risk tests
            ...
            for test in high_risk_tests:
                ...
                if args.complexity:
                    ...
                    print(f"    Maintainability: {complexity.get('maintainability', 0):.2f}")
        ...
        # Handle complexity metrics
            ...
            if args.sort_by_risk:
                tests_with_complexity.sort(
                    key=lambda x: x["complexity"].get("risk_score", 0) 
                    if "error" not in x["complexity"] else 0, 
                    reverse=True
                )
            ...
            for test in tests_with_complexity:
                if "error" in test["complexity"]:
                    ...
                    risk_score = test["complexity"].get("risk_score", 0)
                    print(f"  {test['path']} (risk score: {risk_score:.2f})")
                    print(f"    Complexity: {test['complexity'].get('complexity', 0):.2f}")
                    print(f"    Readability: {test['complexity'].get('readability', 0):.2f}")
                    print(f"    Maintainability: {test['complexity'].get('maintainability', 0):.2f}")
            
            print(f"\nTotal: {len(tests_with_complexity)} tests")
    
    elif not args.count and not args.markers and not args.marker_counts and not args.list_marked_tests and not args.clear_cache:
        if args.category == "all":
            tests = collect_tests(use_cache=use_cache)
        else:
            tests = collect_tests_by_category(args.category, use_cache=use_cache)
