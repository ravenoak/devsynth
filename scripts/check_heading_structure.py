...
def check_heading_structure(file_path):
    ...
    if len(h1_headings) == 0:
        ...
        issues.append(f"Multiple H1 headings found: {', '.join([h[1] for h in h1_headings])}")
    ...
    if metadata and len(h1_headings) == 1:
        ...
        if title and h1_title != title:
            issues.append(f"H1 heading '{h1_title}' does not match metadata title '{title}'")
    
    # Check if headings are properly nested
    current_level = 0
    for level, title in headings:
        if level > current_level + 1:
            issues.append(f"Heading level skipped: H{current_level} to H{level} ('{title}')")
        current_level = level
