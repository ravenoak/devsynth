...
def main() -> int:
    ...
    parser.add_argument("--all", action="store_true", help="Run fast + segmented medium/slow lanes")
    parser.add_argument("--fast-only", action="store_true", help="Run only fast unit/integration/behavior lanes")
    parser.add_argument("--smoke", action="store_true", help="Run behavior fast lane in smoke mode as per docs")
    parser.add_argument("--report", action="store_true", help="Include --report to generate HTML reports where supported")
    ...
    lanes.append((
        ...
        [
            ...
            "--target", "unit-tests", "--speed", "fast", "--no-parallel", "--maxfail=1",
        ],
    ))
    lanes.append((
        "integration_fast",
    ...
    behavior_cmd = [
        "poetry", "run", "devsynth", "run-tests",
        "--target", "behavior-tests", "--speed", "fast", "--no-parallel", "--maxfail=1",
    ]
    if args.smoke:
    ...
    if args.all:
        ...
        lanes.append((
            ...
            [
                "poetry", "run", "devsynth", "run-tests",
                "--target", "unit-tests", "--speed", "medium", "--segment", "--segment-size", "50", "--no-parallel",
        ...
        lanes.append((
            ...
            [
                "poetry", "run", "devsynth", "run-tests",
                "--target", "unit-tests", "--speed", "slow", "--segment", "--segment-size", "50", "--no-parallel",
            ] + (["--report"] if args.report else []),
        ))
