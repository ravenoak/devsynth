#!/usr/bin/env python3
"""
Enhanced script to fix syntax errors in interface module test files (Version 3).

This script builds on fix_interface_syntax_v2.py with additional capabilities to handle:
1. Duplicate fixture definitions
2. Complex indentation issues in multi-line function calls
3. Better handling of commented-out closing parentheses
4. Proper handling of unmatched brackets/parentheses
5. Better handling of placeholder variables
6. Fixing unreachable code
7. Removing excessive blank lines

Usage:
    python scripts/fix_interface_syntax_v3.py [--dry-run] [--verbose] [--file FILE]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Fix syntax errors in interface module test files (Version 3)."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="interface_syntax_report_v3.json",
        help="Output file for the report (default: interface_syntax_report_v3.json)",
    )
        ...
        if stripped.startswith("@pytest.fixture"):
            ...
            if i + 1 < len(lines) and "def " in lines[i + 1]:
                ...
                if fixture_name in fixture_names:
                    ...
                    if verbose:
                        print(f"  Line {i+1}: Removing duplicate fixture {fixture_name}")
                    ...
                    while i < len(lines):
                        if not lines[i].strip() or len(lines[i]) - len(lines[i].lstrip()) > indent:
                            i += 1
                        else:
                            break
...
def fix_test_function_indentation(content: str, verbose: bool = False) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if in_test_function:
            ...
            if current_indent <= test_indent and stripped and not stripped.startswith("#"):
    ...
    for i, line in enumerate(lines):
        ...
        if stripped.startswith("return ") or stripped == "return":
            ...
            if i + 1 < len(lines):
                ...
                if next_stripped and len(next_line) - len(next_line.lstrip()) == len(line) - len(line.lstrip()):
...
def fix_unclosed_parentheses(content: str, verbose: bool = False) -> str:
    ...
    try:
        ...
        if "unexpected EOF while parsing" in str(e) or "unclosed" in str(e) or "was never closed" in str(e):
...
def fix_unmatched_brackets(content: str, verbose: bool = False) -> str:
    ...
    if open_curly > close_curly:
        ...
        if verbose:
            print(f"  Adding {missing} closing curly brackets")
        fixed_content += "\n" + "}" * missing
    elif close_curly > open_curly:
        # Remove extra closing curly brackets
        if verbose:
            print(f"  Removing {close_curly - open_curly} extra closing curly brackets")
        # This is a more complex operation, we'll use a simple approach for now
        fixed_content = fixed_content.replace("}", "", close_curly - open_curly)
    ...
    if open_square > close_square:
        ...
        if verbose:
            print(f"  Adding {missing} closing square brackets")
        fixed_content += "\n" + "]" * missing
    elif close_square > open_square:
        # Remove extra closing square brackets
        if verbose:
            print(f"  Removing {close_square - open_square} extra closing square brackets")
        # This is a more complex operation, we'll use a simple approach for now
        fixed_content = fixed_content.replace("]", "", close_square - open_square)
    ...
    for i, line in enumerate(lines):
        ...
        for var, replacement in replacements.items():
            ...
            if re.search(pattern, line):
                if verbose:
                    print(f"  Line {i+1}: Replacing undefined variable {var} with {replacement}")
...
def fix_missing_comma(content: str, verbose: bool = False) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if re.search(r'\[\s*\w+\s+\w+', stripped) or re.search(r'\w+\s+\w+\s*\]', stripped):
...
def fix_multi_line_indentation(content: str, verbose: bool = False) -> str:
    """Fix indentation in multi-line function calls and statements.
    ...
    while i < len(lines):
        ...
        if "(" in stripped and ")" not in stripped and not stripped.endswith(","):
            # Function call might span multiple lines
            ...
            while i < len(lines) and ")" not in lines[i].strip():
                ...
                # Check if the continuation line has proper indentation
                cont_indent = len(continuation) - len(cont_stripped)
                if cont_indent <= indent:
                    # Continuation line needs proper indentation
                    if verbose:
                        print(f"  Line {i+1}: Fixing indentation in multi-line function call")
                    fixed_lines.append(" " * (indent + 4) + cont_stripped)
                else:
            ...
            if i < len(lines):
                closing = lines[i]
                close_stripped = closing.strip()
                
                # Check if the closing line has proper indentation
                close_indent = len(closing) - len(close_stripped)
                if close_indent <= indent and ")" in close_stripped:
                    # Closing line needs proper indentation
                    if verbose:
                        print(f"  Line {i+1}: Fixing indentation in multi-line function call closing")
                    fixed_lines.append(" " * indent + close_stripped)
                else:
...
def fix_file(file_path: str, dry_run: bool = True, verbose: bool = False) -> Dict[str, Any]:
    ...
    # Fix unmatched brackets
    content = fix_unmatched_brackets(content, verbose)
    
    # Fix multi-line indentation
    content = fix_multi_line_indentation(content, verbose)
...
def generate_report(results: Dict[str, Dict[str, Any]], output_file: str = "interface_syntax_report_v3.json") -> None:
    ...
    print(f"  Files with unfixed errors: {report['summary']['files_with_unfixed_errors']}")
    print(f"  Files with processing errors: {report['summary']['files_with_processing_errors']}")
