...
try:
    ...
    print("Warning: enhanced_test_parser.py not found, some functionality may be limited")
...
class IsolationVisitor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node):
        ...
        if is_test_class:
            ...
            has_setup = any(child.name == 'setUp' or child.name == 'setup_method' 
                           for child in node.body if isinstance(child, ast.FunctionDef))
            has_teardown = any(child.name == 'tearDown' or child.name == 'teardown_method' 
                              for child in node.body if isinstance(child, ast.FunctionDef))
    ...
    def visit_FunctionDef(self, node):
        ...
        is_fixture = any(decorator.id == 'fixture' if isinstance(decorator, ast.Name) else False
        ...
        if is_fixture:
            ...
            for child in ast.walk(node):
                if isinstance(child, ast.Yield):
                    ...
                    last_line = max(stmt.lineno for stmt in ast.walk(node) if hasattr(stmt, 'lineno'))
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name) and node.func.id == 'open':
            if len(node.args) >= 2:
                ...
                if isinstance(node.args[1], ast.Str) and ('w' in node.args[1].s or 'a' in node.args[1].s):
                    # Check if tmpdir is not used
                    tmpdir_used = False
                    if isinstance(node.args[0], ast.Name) and node.args[0].id == 'tmpdir':
                        tmpdir_used = True
                    elif isinstance(node.args[0], ast.Attribute) and isinstance(node.args[0].value, ast.Name) and node.args[0].value.id == 'tmpdir':
                        tmpdir_used = True
...
def generate_recommendations(visitor: IsolationVisitor) -> List[str]:
    ...
    for fixture, info in visitor.fixture_defs.items():
        if info['has_yield'] and not info['has_teardown']:
            recommendations.append(f"Add teardown code after yield in fixture '{fixture}'")
        elif not info['has_yield'] and not info['has_return']:
            recommendations.append(f"Consider adding a return or yield in fixture '{fixture}'")
    
    # Check for setup/teardown
    if not visitor.has_setup_teardown and visitor.current_class:
        recommendations.append("Consider adding setUp and tearDown methods to test classes")
...
def analyze_test_isolation(directory: str = "tests") -> Dict[str, Any]:
    ...
    if results["issues_by_type"].get("global_state", 0) > 0:
        results["recommendations"].append("Replace global variables with function-scoped fixtures")
    
    if results["issues_by_type"].get("shared_resource", 0) > 0:
        results["recommendations"].append("Use function-scoped fixtures for shared resources")
    ...
    if results["issues_by_type"].get("file_operation", 0) > 0:
        results["recommendations"].append("Always use tmpdir fixture for file operations")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Analyze test isolation issues."
    )
    parser.add_argument(
        "--directory",
        default="tests/unit",
