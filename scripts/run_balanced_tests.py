...
def distribute_tests(tests: List[str], test_times: Dict[str, float], num_processes: int) -> List[List[str]]:
    """
...
def run_tests_in_parallel(test_bins: List[List[str]], args) -> int:
    ...
    for i, bin_tests in enumerate(test_bins):
        ...
        if args.report:
            ...
            html_report = os.path.join(test_utils.TEST_CACHE_DIR, f"html_report_process_{i+1}.html")
