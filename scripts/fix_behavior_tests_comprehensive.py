        ...
        if circular_imports:
            ...
            for import_name in circular_imports:
                ...
                if target_file in step_files:
                    ...
                    new_content = new_content.replace(import_line, f"# Content from {target_file} inlined here\n{target_content}")
...
def fix_feature_paths(test_files: List[Path], feature_files: Dict[str, Path], dry_run: bool) -> int:
    """
...
def fix_step_file_feature_paths(step_files: Dict[str, Path], feature_files: Dict[str, Path], dry_run: bool) -> int:
    """
            ...
            for feature_path in scenarios_calls:
                ...
                if feature_filename in feature_files:
                    ...
                    if not found:
                        ...
                        new_content = new_content.replace(old_call, new_call)
                        modified = True
                        print(f"Commented out non-existent feature file reference in {path}: {feature_path}")
...
def main():
    ...
    updated_step_paths = fix_step_file_feature_paths(step_files, feature_files, args.dry_run)
