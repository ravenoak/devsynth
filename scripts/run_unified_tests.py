...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Unified test execution workflow."
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--pattern",
        help="Only run tests matching the specified pattern"
    )
    parser.add_argument(
        "--prioritize",
        action="store_true",
...
def print_test_statistics():
    ...
    if not EXTENDED_UTILS_AVAILABLE:
        print("Warning: test_utils_extended.py not found. Cannot display test statistics.")
        return
    ...
    if total_tests > 0:
        ...
        total_with_markers = marker_counts["total"]["fast"] + marker_counts["total"]["medium"] + marker_counts["total"]["slow"]
        ...
        print(f"  - Medium tests: {marker_counts['total']['medium']} ({medium_percent:.2f}%)")
    ...
    if hasattr(common_test_collector, 'get_test_failure_rates'):
        ...
        if frequently_failing:
            ...
            for test, rate in sorted(frequently_failing.items(), key=lambda x: x[1], reverse=True)[:5]:
