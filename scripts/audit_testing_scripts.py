...
class ScriptAnalyzer:
    ...
    def __init__(self) -> None:
        ...
        self.categories = {
            'test_execution': {
                'keywords': ['run_tests', 'execute_test', 'pytest', 'test_runner', 'run_all_tests'],
    ...
    def analyze_script(self, script_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for category, info in self.categories.items():
                if any(keyword in script_name_lower or keyword in content_lower 
            ...
            testing_patterns = [
                r'pytest', r'coverage', r'unittest', r'test_', r'@pytest',
                r'--cov', r'-m\s+\w+', r'pytest.main', r'subprocess.*pytest'
            ]
            ...
            if 'argparse' in content or 'ArgumentParser' in content:
                ...
                arg_patterns = re.findall(r'add_argument\([\'"]([^\'\"]+)[\'"]', content)
...
class ScriptAuditor:
    ...
    def find_testing_scripts(self) -> List[Path]:
        ...
        testing_patterns = [
            ...
            'validate*', 'check*', 'run_*', '*_test', 'audit*'
        ]
            ...
            if script_path.is_file():
                try:
                    ...
                    if any(keyword in content.lower() for keyword in 
                           ['pytest', 'test_', 'coverage', 'unittest', '@pytest.mark']):
                        found_scripts.add(script_path)
                except:
                    pass
    ...
    def get_git_usage_frequency(self, script_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            ], capture_output=True, text=True, cwd=self.scripts_dir.parent)
            
            commit_count = len(result.stdout.strip().split('\n')) if result.stdout.strip() else 0
            
            # Get last modification date
            result = subprocess.run([
                'git', 'log', '-1', '--format=%cd', '--date=iso', '--', str(script_path)
            ], capture_output=True, text=True, cwd=self.scripts_dir.parent)
            ...
            # Get creation date
            result = subprocess.run([
                'git', 'log', '--reverse', '--format=%cd', '--date=iso', '--', str(script_path)
            ], capture_output=True, text=True, cwd=self.scripts_dir.parent)
    ...
    def analyze_overlaps(self) -> Dict[str, Any]:
        ...
        duplicate_functions = {
            func: scripts for func, scripts in function_overlaps.items()
            if len(scripts) > 1
        }
        ...
        for category, scripts in category_groups.items():
            if len(scripts) > 2:  # More than 2 scripts in same category
                recommendations['consolidation_groups'].append({
                    'category': category,
                    'description': self.analyzer.categories[category]['description'],
                    'scripts': [
                        {
                            'name': s['name'],
                            'path': s['path'],
                            ...
                            'git_frequency': s.get('git_stats', {}).get('frequency_score', 0)
                        }
                        for s in scripts
                    ],
                    'consolidation_benefit': 'high' if len(scripts) > 4 else 'medium'
                })
        ...
        for script in self.scripts:
            for category in script.get('categories', []):
                if category in cli_commands:
                    recommendations['cli_migration_mapping'][script['name']] = cli_commands[category]
        
        # Identify deprecation candidates (low usage, simple functionality)
        for script in self.scripts:
            git_stats = script.get('git_stats', {})
            if (git_stats.get('frequency_score', 0) < 3 and 
                script.get('lines', 0) < 100 and
                len(script.get('functions', [])) < 3):
    ...
    def audit(self, include_git_history: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            'summary': {
                ...
                'average_script_size': total_lines // total_scripts if total_scripts > 0 else 0
            },
...
def generate_markdown_report(audit_data: Dict[str, Any]) -> str:
    ...
    for category, count in sorted(category_counts.items()):
        description = audit_data['category_definitions'].get(category, {}).get('description', '')
    ...
    if recs['consolidation_groups']:
        for group in recs['consolidation_groups']:
            ...
            for script in group['scripts']:
                md_lines.append(f"- `{script['name']}` ({script['size']} bytes, {script['functions']} functions)")
            md_lines.append("")
