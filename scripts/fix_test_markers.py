        ...
        if decorator_match:
            ...
            for j in range(i+1, min(i+10, len(lines))):
                ...
                if func_match:
                    ...
                    if current_class:
                        if next_func in structure['classes'][current_class]['functions']:
                            structure['classes'][current_class]['functions'][next_func]['decorators'].append(i)
                    else:
...
def identify_marker_issues(file_path: Path, structure: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    for class_name, class_info in structure['classes'].items():
        for func_name, func_info in class_info['functions'].items():
            for decorator_line in func_info['decorators']:
                marker_match = MARKER_PATTERN.search(lines[decorator_line])
                if marker_match:
                    function_markers[f"{class_name}.{func_name}"] = marker_match.group(1)
    ...
    for group_name, funcs in function_groups.items():
        if len(funcs) > 1:
            markers = set(function_markers.get(func, None) for func in funcs if func in function_markers)
...
def fix_misaligned_markers(file_path: Path, structure: Dict[str, Any], issues: Dict[str, Any], dry_run: bool = False, verbose: bool = False) -> Tuple[int, int, int]:
    """
    ...
    for issue in issues['misaligned_markers']:
        ...
        if next_func_line is not None:
            ...
            if has_speed_marker:
                ...
                if verbose:
                    print(f"Removing misaligned marker at {file_path}:{line_num+1} - {issue['text']}")
                ...
                if verbose:
                    print(f"Moving marker from {file_path}:{line_num+1} to {file_path}:{next_func_line}")
            ...
            if verbose:
                print(f"Removing orphaned marker at {file_path}:{line_num+1} - {issue['text']}")
...
def fix_all_marker_issues(file_path: Path, structure: Dict[str, Any], issues: Dict[str, Any], dry_run: bool = False, verbose: bool = False) -> Tuple[int, int, int, int]:
    """
    ...
    fixed_misaligned, removed, unchanged = fix_misaligned_markers(file_path, structure, issues, dry_run, verbose)
    ...
    for issue in issues['duplicate_markers']:
        ...
        if verbose:
            print(f"Fixing duplicate markers for {func_name} at {file_path}:{line_num+1} - {markers}")
        ...
        if '.' in func_name:
            ...
            if class_name in structure['classes'] and method_name in structure['classes'][class_name]['functions']:
                decorator_lines = structure['classes'][class_name]['functions'][method_name]['decorators']
        else:
            # Module function
        ...
        for i, decorator_line in enumerate(decorator_lines):
            ...
            if marker_match:
                if kept_marker is None:
                    ...
                    if verbose:
                        print(f"Removing duplicate marker at {file_path}:{decorator_line+1} - {lines[decorator_line].strip()}")
...
def generate_report(issues_by_file: Dict[str, Dict[str, Any]], output_file: str = "test_marker_issues_report.json"):
    """
    ...
    report = {
        'summary': {
            ...
            'files_with_issues': sum(1 for issues in issues_by_file.values() if any(len(v) > 0 for v in issues.values())),
            'total_issues': sum(sum(len(v) for v in issues.values()) for issues in issues_by_file.values()),
            'misaligned_markers': sum(len(issues['misaligned_markers']) for issues in issues_by_file.values()),
            'missing_markers': sum(len(issues['missing_markers']) for issues in issues_by_file.values()),
            'duplicate_markers': sum(len(issues['duplicate_markers']) for issues in issues_by_file.values()),
            'inconsistent_markers': sum(len(issues['inconsistent_markers']) for issues in issues_by_file.values())
        },
        'issues_by_file': {str(file_path): issues for file_path, issues in issues_by_file.items() if any(len(v) > 0 for v in issues.values())}
    }
...
def main():
    ...
    if args.fix_all:
        ...
        for file_path, issues in issues_by_file.items():
            structure = analyze_test_file_structure(file_path)
            fixed_misaligned, fixed_duplicates, fixed_inconsistent, added_missing = fix_all_marker_issues(
