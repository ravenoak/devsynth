...
def analyze_documentation_files(docs_dir: Path) -> List[Dict]:
    ...
    for md_file in docs_dir.rglob("*.md"):
        ...
        if any(skip in relative_str for skip in ['archived/', 'tmp_', '.git/', '__pycache__']):
            continue
        ...
        if frontmatter:
            ...
            release_phase = frontmatter.get('release_phase', 
                           frontmatter.get('tags', ['unknown'])[0] if frontmatter.get('tags') else 'unknown')
...
def main():
    ...
    parser.add_argument("--generate", action="store_true", help="Generate and write the index file")
    parser.add_argument("--docs-dir", default="docs", help="Documentation directory to analyze")
    parser.add_argument("--output", default="docs/documentation_index.md", help="Output file path")
