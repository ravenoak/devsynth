...
def parse_args():
    ...
    parser.add_argument(
        '--module',
        help='Specific module to update (e.g., tests/unit/interface)'
    )
...
def update_test_file(
    ...
    for line_num, test_name in sorted(test_line_numbers.items()):
        ...
        if test_path in categorized_tests:
            ...
            matching_paths = [
                path for path in categorized_tests.keys()
                if isinstance(path, str) and test_name in path and str(file_path) in path
            ]
...
def main():
    ...
    for i, file_path in enumerate(test_files):
        ...
        if args.verbose or added > 0 or updated > 0:
            print(f"Results for {file_path}: added={added}, updated={updated}, unchanged={unchanged}")
