...
class GherkinParser:
    ...
    def parse_feature_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Regular scenarios
            scenario_matches = re.finditer(r'Scenario:(.+?)(?=\n\s*(?:Scenario:|Scenario Outline:|$))', content, re.DOTALL)
                ...
                if scenario['type'] == 'scenario':
                    ...
                    if examples:
                        ...
                        for i, example in enumerate(examples):
                            ...
                            example_str = "_".join(f"{k}_{v}" for k, v in example.items())
                            test_path = f"{feature_path}::{scenario_name}::{example_str}"
                            test_paths.append(test_path)
                    else:
...
def parse_feature_directory(directory: str, verbose: bool = False) -> List[Dict[str, Any]]:
    """
    Parse all feature files in a directory.
