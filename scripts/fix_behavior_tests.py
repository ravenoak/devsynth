...
# Regex patterns
SCENARIOS_IMPORT_PATTERN = re.compile(r'from pytest_bdd import (.*?)scenarios(.*?)$', re.MULTILINE)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Fix behavior test collection issues.'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
...
def standardize_feature_files(feature_files: Dict[str, List[Path]], dry_run: bool) -> Dict[str, Path]:
    """
...
def update_step_files(step_files: Dict[str, Path], standardized_feature_paths: Dict[str, Path], dry_run: bool) -> int:
    """
        ...
        if not scenarios_calls:
            ...
            if feature_name in standardized_feature_paths:
                ...
                if 'scenarios(' not in content:
                    ...
                    new_content = content.replace(import_line, import_line + scenarios_call)
...
def update_test_files(test_files: List[Path], standardized_feature_paths: Dict[str, Path], dry_run: bool) -> int:
    """
                ...
                if var_match:
                    ...
                    if feature_path_match:
                        ...
                        if feature_name in standardized_feature_paths:
                            ...
                            if os.path.basename(relative_path) != os.path.basename(feature_path):
