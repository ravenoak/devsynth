...
def extract_terms_from_content(content: str) -> Set[str]:
    ...
    devsynth_terms = {
        'EDRR', 'WSDE', 'DevSynth', 'dialectical reasoning', 'hexagonal architecture',
        'memory store', 'provider system', 'agent system', 'BDD', 'TDD',
        'requirements traceability', 'consensus building', 'peer review',
        'MVUU', 'UXBridge', 'CLI', 'WebUI', 'API', 'specification',
        'implementation', 'validation', 'harmonization'
    }
...
def analyze_terminology_usage(docs_dir: Path, glossary_path: Path) -> Dict:
    ...
    glossary_terms_lower = {term.lower(): term for term in results['glossary_terms'].keys()}
    ...
    for md_file in docs_dir.rglob("*.md"):
        ...
        for term in file_terms:
            ...
            results['files_by_term'][term].append(str(md_file.relative_to(docs_dir.parent)))
    ...
    results['coverage_analysis'] = {
        ...
        'coverage_percentage': (defined_terms / total_terms * 100) if total_terms > 0 else 0
    }
...
"""
    ...
    if results['terms_found']:
        ...
        for term, count in results['terms_found'].most_common(10):
            status = "✅" if term.lower() in [t.lower() for t in results['glossary_terms'].keys()] else "❌"
    ...
    high_usage_undefined = [
        (term, count) for term, count in results['terms_found'].most_common()
...
def main():
    parser = argparse.ArgumentParser(description="Analyze terminology usage in documentation")
    parser.add_argument("--docs-dir", default="docs", help="Documentation directory to analyze")
    parser.add_argument("--glossary", default="docs/glossary.md", help="Glossary file path")
