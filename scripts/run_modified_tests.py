            ...
            for line in f:
                ...
                if match:
                    ...
                    if module.startswith('.'):
                        ...
                        package_parts = file_path.replace('/', '.').replace('\\', '.').split('.')
...
def build_dependency_map(src_dir: str, test_dir: str) -> Dict[str, Set[str]]:
    ...
    if os.path.exists(cache_file):
        ...
        src_mtime = max(os.path.getmtime(path) for path, _, _ in os.walk(src_dir) for path in Path(path).glob('*.py'))
        test_mtime = max(os.path.getmtime(path) for path, _, _ in os.walk(test_dir) for path in Path(path).glob('*.py'))
