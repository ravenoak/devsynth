...
def fix_syntax_errors(directory: str, verbose: bool = False) -> Dict[str, Any]:
    ...
    for file_path in files_with_errors:
        ...
        for line in lines:
            ...
            if in_function and (line.strip().startswith('import ') or line.strip().startswith('from ')):
...
def fix_misaligned_markers(directory: str, dry_run: bool = False, verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    command = [
        sys.executable, 'scripts/fix_test_markers.py',
        '--directory', directory,
        '--fix-all'
    ]
    ...
    for line in stdout.split('\n'):
        ...
        if 'Fix ' in line and 'inconsistent markers' in line:
            try:
                fixed_inconsistent = int(line.split('Fix ')[1].split(' inconsistent')[0])
            except (IndexError, ValueError):
                pass
...
def add_missing_markers(directory: str, batch_size: int = 20, max_tests: int = 100, dry_run: bool = False, verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    command = [
        sys.executable, 'scripts/incremental_test_categorization.py',
        '--directory', directory,
...
def verify_marker_detection(verbose: bool = False) -> Dict[str, Any]:
    ...
    command = [
        sys.executable, 'scripts/common_test_collector.py',
        '--marker-counts',
        '--no-cache'
    ]
    ...
    for line in stdout.split('\n'):
        ...
        if line.endswith(':'):
            ...
            if current_category in marker_counts and marker_type in marker_counts[current_category]:
...
def main():
    ...
    categorization_results = add_missing_markers(directory, args.batch_size, args.max_tests, args.dry_run, args.verbose)
