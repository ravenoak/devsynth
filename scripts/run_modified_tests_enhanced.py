...
def get_modified_files(base_commit: str) -> List[str]:
    ...
    try:
        ...
        for file_path in result.stdout.strip().split('\n'):
            if file_path and (file_path.endswith('.py') or file_path.endswith('.feature')):
...
def build_dependency_map(src_dir: str, test_dir: str, use_cache: bool = True) -> Dict[str, Set[str]]:
    """
    ...
    if hasattr(common_test_collector, 'analyze_test_dependencies'):
        ...
        dependencies = common_test_collector.analyze_test_dependencies(all_tests, use_cache=True)
    ...
    for test_file in test_files:
        ...
        for src_file in src_files:
            ...
            src_module = src_file.replace('/', '.').replace('\\', '.').replace('.py', '')
...
def find_affected_tests(modified_files: List[str], dependency_map: Dict[str, Set[str]], all_tests: bool = False) -> Set[str]:
    """
            ...
            for test in test_list:
                ...
                if category:
                    ...
                    for marker in markers:
                        if marker in tests_with_markers.get(category, {}) and test in tests_with_markers[category][marker]:
                            filtered_tests.append(test)
                            break
...
def run_tests(affected_tests: List[str], args) -> int:
    ...
    if args.balanced and args.parallel and hasattr(test_utils, 'distribute_tests'):
        ...
        test_bins = test_utils.distribute_tests(affected_tests, test_times, num_processes)
