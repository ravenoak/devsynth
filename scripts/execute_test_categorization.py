...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Execute the test categorization plan"
    )
    parser.add_argument(
        "--schedule",
            ...
            if "::" in test and len(test.split("::")) > 2:
                ...
                if class_match:
                    ...
                    if method_match:
                        ...
                        if "import pytest" not in content and "from pytest import" not in content:
...
def print_summary(results):
    ...
    avg_time = sum(result["execution_time"] for result in results) / len(results) if results else 0
...
def main():
    ...
    for i, test in enumerate(uncategorized_tests):
        ...
        has_marker, existing_marker = check_test_has_marker(test, ["fast", "medium", "slow"])
        ...
        if not args.dry_run:
            ...
            if success:
                # Update progress
                progress = update_progress(progress, test, speed_category, result["execution_time"])
