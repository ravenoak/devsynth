...
def generate_standard_metadata(file_path: Path, existing_metadata: Optional[Dict] = None) -> Dict:
    ...
    if existing_metadata:
        ...
        if 'status' in existing_metadata and existing_metadata['status'] in STANDARD_METADATA_SCHEMA['valid_statuses']:
...
def validate_all_documentation(docs_dir: Path) -> Dict:
    ...
    for md_file in docs_dir.rglob("*.md"):
        ...
        if metadata is None:
            results['files_without_metadata'].append(str(md_file.relative_to(docs_dir.parent)))
...
def generate_validation_report(results: Dict) -> str:
    ...
    compliance_rate = ((files_processed - files_with_issues) / files_processed * 100) if files_processed > 0 else 0
...
"""
    ...
    if results['files_without_metadata']:
        report += f"Files Without Metadata ({len(results['files_without_metadata'])}):\n"
        for file_path in results['files_without_metadata'][:10]:  # Show first 10
            report += f"- {file_path}\n"
        if len(results['files_without_metadata']) > 10:
            report += f"... and {len(results['files_without_metadata']) - 10} more files\n"
        report += "\n"
    
    # Show detailed issues for files with problems
    files_with_detailed_issues = [f for f in results['validation_details'] if f['issues']]
...
def main():
    ...
    parser.add_argument("--docs-dir", default="docs", help="Documentation directory to validate")
    ...
    if args.report or args.strict:
        ...
        print(f"Compliance rate: {((results['files_processed'] - results['files_with_issues']) / results['files_processed'] * 100):.1f}%")
