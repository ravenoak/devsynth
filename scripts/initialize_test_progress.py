...
def parse_args():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Overwrite existing file if it exists"
    )
    parser.add_argument(
...
def initialize_progress_file(progress_file, force=False, verbose=False):
    ...
    if os.path.exists(progress_file) and not force:
        print(
            f"Error: Progress file {progress_file} already exists. Use --force to overwrite."
        )
        return False
...
def main():
    ...
    success = initialize_progress_file(
        args.progress, force=args.force, verbose=args.verbose
    )
