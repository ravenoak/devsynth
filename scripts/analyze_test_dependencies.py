...
class TestDependencyAnalyzer(ast.NodeVisitor):
    ...
    def __init__(self) -> None:
        ...
        self.fs_patterns = {
            'open', 'file', 'Path', 'pathlib', 'os.path', 'shutil', 'tempfile',
            'glob', 'makedirs', 'rmdir', 'remove', 'rename', 'copyfile',
            'tmp_path', 'tmpdir', 'tmpdir_factory', 'temp_log_dir', 'tmp_project_dir'
        }
        
        self.network_patterns = {
            'requests', 'urllib', 'http', 'socket', 'aiohttp', 'httpx',
            'websocket', 'ftp', 'smtp', 'telnet', 'paramiko', 'ssh'
        }
        
        self.global_state_patterns = {
            'os.environ', 'sys.path', 'sys.modules', 'globals()', 'locals()',
            'setattr', 'delattr', 'monkeypatch', 'patch', 'mock'
        }
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if call_name:
            ...
            if any(pattern in call_name.lower() for pattern in self.global_state_patterns):
...
class TestFileAnalyzer:
        ...
        try:
            ...
            result = {
                ...
                'dependencies': {
                    ...
                    'global_state_modifications': sorted(analyzer.global_state_modifications),
                ...
                'safe_for_parallel': self._is_safe_for_parallel(analyzer, has_isolation_marker)
            }
    ...
    def _is_safe_for_parallel(self, analyzer: TestDependencyAnalyzer, has_marker: bool) -> bool:
        ...
        return (
            len(analyzer.file_operations) == 0 and
            len(analyzer.network_calls) == 0 and
            len(analyzer.global_state_modifications) <= 1  # Allow minimal mocking
        )
...
def generate_recommendations(analysis_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    files_with_isolation = sum(1 for r in analysis_results if r.get('has_isolation_marker', False))
    safe_for_removal = sum(1 for r in analysis_results if r.get('safe_for_parallel', False) and r.get('has_isolation_marker', False))
    
    # Categorize by risk level
    low_risk = [r for r in analysis_results if r.get('risk_score', 0) <= 2 and r.get('has_isolation_marker', False)]
    medium_risk = [r for r in analysis_results if 3 <= r.get('risk_score', 0) <= 8 and r.get('has_isolation_marker', False)]
    high_risk = [r for r in analysis_results if r.get('risk_score', 0) > 8 and r.get('has_isolation_marker', False)]
    ...
    return {
        'summary': {
            ...
            'removal_percentage': round((safe_for_removal / files_with_isolation * 100) if files_with_isolation > 0 else 0, 1)
        },
        'risk_categories': {
            'low_risk': {
                'count': len(low_risk),
...
def apply_recommendations(recommendations: Dict[str, Any], dry_run: bool = True) -> Dict[str, Any]:
...
def main() -> int:
    ...
    parser.add_argument(
        '--apply',
        action='store_true',
        help='Apply safe isolation marker removals'
    )
    ...
    print(f"  Files with isolation markers: {recommendations['summary']['files_with_isolation_markers']}")
    print(f"  Safe for removal: {recommendations['summary']['safe_for_removal']} ({recommendations['summary']['removal_percentage']}%)")
    
    print(f"\nRisk breakdown:")
    print(f"  Low risk (immediate removal): {recommendations['risk_categories']['low_risk']['count']}")
    print(f"  Medium risk (careful review): {recommendations['risk_categories']['medium_risk']['count']}")
    print(f"  High risk (keep isolation): {recommendations['risk_categories']['high_risk']['count']}")
    
    # Apply changes if requested
    if args.apply:
        print(f"\nApplying recommendations...")
        application_result = apply_recommendations(recommendations, dry_run=args.dry_run)
