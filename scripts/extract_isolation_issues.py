...
def extract_issues(report_path: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for file_path in files_with_issues:
        try:
            ...
            import subprocess
            cmd = ["python", "scripts/test_isolation_analyzer.py", "--file", file_path, "--verbose"]
    ...
    if not result["shared_resource"] and not result["missing_teardown"]:
        ...
        for file_data in report.get("files", []):
            ...
            if issues_count > 0:
                ...
                has_shared_resource = any("shared resource" in r.lower() for r in recommendations)
                has_missing_teardown = any("teardown" in r.lower() for r in recommendations)
