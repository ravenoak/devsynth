...
def ensure_union_import(content):
    ...
    if re.search(r'from\s+typing\s+import\s+[^(]*\bUnion\b', content) or \
       re.search(r'from\s+typing\s+import\s+\([^)]*\bUnion\b[^)]*\)', content) or \
       re.search(r'import\s+typing', content):
        return content
    ...
    import_matches = list(re.finditer(r'^import\s+.*$|^from\s+.*\s+import', content, re.MULTILINE))
    if import_matches:
        last_import = import_matches[-1]
        last_import_end = last_import.end()
        return content[:last_import_end] + "\nfrom typing import Union" + content[last_import_end:]
    
    # If there are no imports, add it at the beginning after any module docstring
    docstring_match = re.search(r'^""".*?"""', content, re.DOTALL)
    if docstring_match:
        docstring_end = docstring_match.end()
        return content[:docstring_end] + "\n\nfrom typing import Union" + content[docstring_end:]
...
def process_file(file_path, dry_run=False):
    ...
    if '|' not in content or not re.search(r'\w+\s*:\s*[A-Za-z0-9_"\'\.]+\s*\|', content):
        return False
...
def main():
    parser = argparse.ArgumentParser(description='Fix type annotation issues in Python files')
    parser.add_argument('--directory', '-d', default='.', help='Directory to search for Python files')
    parser.add_argument('--dry-run', '-n', action='store_true', help='Dry run (no changes will be made)')
