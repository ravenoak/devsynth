...
def main() -> int:
    ...
    parser.add_argument(
        'target_path',
        help='Path to source code to mutate'
    )
    
    parser.add_argument(
        'test_path', 
        help='Path to tests that should catch mutations'
    )
    ...
    parser.add_argument(
        '--module-filter',
        help='Only mutate files containing this string in their path'
    )
    ...
    try:
        ...
        for file_path, stats in report.summary['file_breakdown'].items():
            score = (stats['killed'] / stats['total']) if stats['total'] > 0 else 0
            if score < args.threshold and stats['total'] >= 3:  # Only show files with multiple mutations
                print(f"  {file_path}: {stats['killed']}/{stats['total']} ({score:.2%})")
