...
def determine_speed_category(execution_time: float, fast_threshold: float, medium_threshold: float) -> str:
    """
...
def add_marker_to_file(file_path: str, test_name: str, marker: str, dry_run: bool = True, verbose: bool = False) -> Tuple[bool, bool]:
    """
    ...
    try:
        ...
        has_pytest_import = re.search(r"import\s+pytest", content) or re.search(r"from\s+pytest\s+import", content)
        ...
        if "::" in test_name:
            ...
            if not method_match:
                print(f"Error: Method {base_method_name} not found in class {class_name} in {file_path}")
                return False, False
            ...
            if f"@pytest.mark.{marker}" in marker_line:
                if verbose:
                    print(f"Test {class_name}::{base_method_name} already has {marker} marker")
        ...
        if verbose:
            if "::" in test_name:
                ...
                if "[" in method_name:
                    ...
                    base_method_name = method_name
                print(f"Adding {marker} marker to {class_name}::{base_method_name} in {file_path}")
            else:
                if "[" in test_name:
                    base_test_name = test_name.split("[")[0]
                else:
...
def main():
    ...
    if args.skip_tests > 0:
        if args.skip_tests < len(unmarked_tests):
            ...
            print(f"Warning: Skip count ({args.skip_tests}) exceeds number of unmarked tests ({len(unmarked_tests)})")
            unmarked_tests = []
    ...
    for i, batch in enumerate(batches):
        ...
        for test_path, execution_time in batch_results.items():
            ...
            if file_path in processed_functions and base_test_name in processed_functions[file_path]:
                if args.verbose:
                    print(f"Skipping {base_test_name} in {file_path} (already processed)")
                continue
