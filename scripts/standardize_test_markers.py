...
def parse_args():
    ...
    parser.add_argument(
        "--fix",
        action="store_true",
        help="Fix marker placement issues"
    )
    parser.add_argument(
        "--report",
        action="store_true",
...
def analyze_file(file_path: str, patterns: Dict[str, Dict[str, Any]], verbose: bool = False) -> Dict[str, List[Dict[str, Any]]]:
    """
    ...
    try:
        ...
        for issue in issues["function"]:
            if issue["type"] == "incorrect_1":
                ...
                if verbose:
                    print(f"  Fixing marker after function definition at lines {issue['line_start']}-{issue['line_end']}")
                
                # Extract the marker and function definition
                match = re.search(r"(def\s+test_[a-zA-Z0-9_]+\s*\([^)]*\):)\s*\n\s*(@pytest\.mark\.[a-z_]+)", issue["match"])
                if match:
                ...
                if verbose:
                    print(f"  Fixing marker with blank line at lines {issue['line_start']}-{issue['line_end']}")
                
                # Replace multiple blank lines with a single newline
                fixed = re.sub(r"(@pytest\.mark\.[a-z_]+)\s*\n\s*\n+\s*(def\s+test_[a-zA-Z0-9_]+\s*\()", r"\1\n\2", issue["match"])
                ...
                if verbose:
                    print(f"  Fixing multiple markers at lines {issue['line_start']}-{issue['line_end']}")
                
                # Extract the markers and function definition
                match = re.search(r"(@pytest\.mark\.[a-z_]+)\s*\n\s*(@pytest\.mark\.[a-z_]+)\s*\n\s*(def\s+test_[a-zA-Z0-9_]+\s*\()", issue["match"])
                if match:
        ...
        for issue in issues["class_method"]:
            if issue["type"] == "incorrect_1":
                ...
                if verbose:
                    print(f"  Fixing marker after method definition at lines {issue['line_start']}-{issue['line_end']}")
...
def generate_report(results: Dict[str, Dict[str, List[Dict[str, Any]]]], output_file: str = "marker_placement_report.json"):
    """
...
def main():
    ...
    if args.report:
        ...
        if args.verify:
            ...
            total_tests = sum(result["total_tests"] for result in verification_results.values())
            tests_with_markers = sum(result["tests_with_markers"] for result in verification_results.values())
            tests_without_markers = sum(result["tests_without_markers"] for result in verification_results.values())
            
            print("\nVerification Summary:")
            print(f"  Total tests: {total_tests}")
            print(f"  Tests with markers: {tests_with_markers} ({tests_with_markers / total_tests * 100:.2f}%)")
            print(f"  Tests without markers: {tests_without_markers} ({tests_without_markers / total_tests * 100:.2f}%)")
    
    # Print summary
    print("\nSummary:")
    print(f"  Total files processed: {len(test_files)}")
    print(f"  Files with issues: {len([f for f in results if sum(len(issue_list) for issue_list in results[f].values()) > 0])}")
