...
def parse_args():
    ...
    parser.add_argument(
        "--weeks", type=int, default=4, help="Number of weeks to schedule (default: 4)"
    )
    parser.add_argument(
        "--tests-per-day",
        type=int,
...
def create_schedule(tests, weeks, tests_per_day, priority_modules):
    ...
    if total_tests > total_days * tests_per_day:
        ...
        print(
            f"Warning: Adjusted tests per day to {tests_per_day} to fit all tests in {total_days} days"
        )
...
def main():
    ...
    schedule = create_schedule(
        unmarked_tests, args.weeks, args.tests_per_day, args.priority_modules
    )
