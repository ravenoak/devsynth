...
def fix_heading_structure(file_path):
    ...
    # If the first heading is not H1, convert it to H1
        ...
        changes.append(f"Converted H{first_heading_level} heading '{first_heading_text}' to H1")
    ...
    while i < len(lines):
        if re.match(r'^#+\s+', lines[i]):
            ...
            if i > 0 and lines[i-1].strip() != '':
                lines.insert(i, '')
                changes.append(f"Added blank line before heading '{lines[i+1].strip()}'")
                i += 1
        ...
        if match:
            ...
            if i > first_heading_index:
                ...
                if level > current_level + 1:
                    ...
                    changes.append(f"Adjusted heading level from H{level} to H{new_level} for '{text}'")
                    level = new_level
