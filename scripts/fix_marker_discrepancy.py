...
def run_standardize_markers(dry_run: bool = False, verbose: bool = False) -> Tuple[bool, Dict[str, Any]]:
    """
...
def run_verify_markers(fix: bool = False, verbose: bool = False, report_file: str = 'marker_detection_report.json') -> Tuple[bool, Dict[str, Any]]:
    """
    ...
    cmd = [sys.executable, 'scripts/verify_marker_detection.py', f'--report-file={report_file}']
    if fix:
        cmd.append('--fix')
    if verbose:
        cmd.append('--verbose')
...
def run_apply_speed_markers(dry_run: bool = True, verbose: bool = False) -> Tuple[bool, Dict[str, Any]]:
    """
...
def main():
    ...
    standardize_success, standardize_result = run_standardize_markers(args.dry_run, args.verbose)
    ...
    verify_success, verify_result = run_verify_markers(False, args.verbose, 'marker_detection_report.json')
    ...
    if verify_result.get("discrepancies_resolved", False):
        ...
        while iteration <= args.max_iterations:
            ...
            if not args.dry_run:
                verify_success, verify_result = run_verify_markers(True, args.verbose, f'marker_detection_report_{iteration}.json')
            else:
                print("Skipping fix attempt in dry run mode")
                verify_success, verify_result = run_verify_markers(False, args.verbose, f'marker_detection_report_{iteration}.json')
            
            if not verify_success:
                print(f"Error: Failed to run verify_marker_detection.py in iteration {iteration}. Aborting.")
                report["error"] = f"Failed to run verify_marker_detection.py in iteration {iteration}"
                break
            ...
            if not standardize_success:
                print(f"Error: Failed to run standardize_marker_placement.py in iteration {iteration}. Aborting.")
                report["error"] = f"Failed to run standardize_marker_placement.py in iteration {iteration}"
                break
            
            # Run verification again to check if discrepancies are resolved
            verify_success, verify_result = run_verify_markers(False, args.verbose, f'marker_detection_report_{iteration}_after.json')
            
            if not verify_success:
                print(f"Error: Failed to run verify_marker_detection.py after fix in iteration {iteration}. Aborting.")
                report["error"] = f"Failed to run verify_marker_detection.py after fix in iteration {iteration}"
                break
        ...
        if iteration > args.max_iterations and not verify_result.get("discrepancies_resolved", False):
    ...
    if report.get("status") == "resolved":
        ...
        if "apply_result" in report and "marker_report" in report["apply_result"]:
            ...
            print(f"  Total: {marker_report.get('fast_tests', 0) + marker_report.get('medium_tests', 0) + marker_report.get('slow_tests', 0)}")
    else:
