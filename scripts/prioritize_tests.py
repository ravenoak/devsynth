...
def parse_args():
    ...
    parser.add_argument(
        '--reset',
        action='store_true',
        help='Reset the failure history'
    )
    parser.add_argument(
        '--category',
...
def collect_tests(test_dir: str, category: str = 'all', speed: str = 'all') -> List[str]:
    """
    ...
    if 'last_run' in test_history and 'last_failure' in test_history:
        ...
        if last_failure == datetime.min:
            ...
            if time_since_last_run == 0:
                ...
                recency_factor = max(0.0, 1.0 - (time_since_last_failure / time_since_last_run))
...
def prioritize_tests(tests: List[str], history: Dict[str, Dict[str, Any]]) -> List[Tuple[str, float]]:
    """
...
def run_tests(prioritized_tests: List[Tuple[str, float]], args) -> Tuple[int, Dict[str, bool]]:
    """
...
def update_test_history(history: Dict[str, Dict[str, Any]], test_results: Dict[str, bool]) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for test_path, passed in test_results.items():
        # Get the test's history, or create a new entry if it doesn't exist
        test_history = history.get(test_path, {
            'runs': 0,
            'failures': 0,
            'last_run': datetime.min.isoformat(),
            'last_failure': datetime.min.isoformat()
