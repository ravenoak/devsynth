...
def fix_indentation_line_by_line(content: str, verbose: bool = False) -> str:
    ...
    while i < len(lines):
        ...
        if re.match(r'def\s+\w+_state', stripped) or (i > 0 and lines[i-1].strip().startswith('@pytest.fixture')):
            if in_class:
        ...
        if stripped.startswith('"""') and stripped.endswith('"""') and len(stripped) > 3:
            ...
            if in_method or in_fixture:
                ...
                if not line.startswith(' ' * docstring_indent):
                    if verbose:
                        print(f"    Line {i+1}: Fixing single-line docstring indentation")
        ...
        if stripped.startswith('"""') and not stripped.endswith('"""') or (stripped.endswith('"""') and not stripped.startswith('"""')):
            ...
            if in_method or in_fixture:
                ...
                if not line.startswith(' ' * docstring_indent):
                    if verbose:
                        print(f"    Line {i+1}: Fixing multi-line docstring indentation")
                    fixed_lines.append(' ' * docstring_indent + stripped)
                else:
                    fixed_lines.append(line)
            else:
        ...
        if in_docstring:
            if (in_method or in_fixture) and not line.startswith(' ' * docstring_indent):
                if verbose:
        ...
        if in_method or in_fixture:
            ...
            if not line.startswith(' ' * body_indent) and not stripped.startswith('@'):
                if verbose:
                    print(f"    Line {i+1}: Fixing {'method' if in_method else 'fixture'} body indentation")
                fixed_lines.append(' ' * body_indent + stripped)
            else:
                fixed_lines.append(line)
            i += 1
...
def fix_try_except_blocks(content: str, verbose: bool = False) -> str:
    ...
    while i < len(lines):
        ...
        if in_try_block and (stripped == 'except:' or stripped.startswith('except ') or stripped == 'finally:'):
            # These should be at the same level as the try statement
            if not line.startswith(' ' * try_level):
                if verbose:
                    print(f"    Line {i+1}: Fixing {stripped} indentation to match try level")
        ...
        if in_try_block and not (stripped == 'try:' or stripped == 'except:' or stripped.startswith('except ') or stripped == 'finally:'):
            # These should be indented one level from the try/except/finally statement
            if not line.startswith(' ' * block_level):
                if verbose:
                    print(f"    Line {i+1}: Fixing try/except/finally block body indentation")
...
def remove_duplicate_fixtures(content: str, verbose: bool = False) -> str:
    ...
    for fixture_name, occurrences in fixtures.items():
        if len(occurrences) > 1:
            if verbose:
                print(f"    Found {len(occurrences)} occurrences of fixture '{fixture_name}'")
            ...
            for start, end, _ in to_remove:
                if verbose:
                    print(f"    Removing duplicate fixture '{fixture_name}' at position {start}")
...
def fix_test_function_indentation(content: str, verbose: bool = False) -> str:
    ...
    for match in matches:
        ...
        replacement = f"{' ' * indent}def test_with_clean_state(clean_state):\n{' ' * (indent + 4)}\"\"\"" + docstring + f"\"\"\"\n"
        
        if verbose:
            print(f"    Replacing generic test_function with properly indented test_with_clean_state")
...
def fix_test_function_structure(content: str, verbose: bool = False) -> str:
    ...
    for match in matches:
        ...
        replacement = f'def test_with_clean_state(clean_state):\n    """{docstring}"""\n'
...
def fix_file(file_path: str, dry_run: bool = True, verbose: bool = False) -> Tuple[bool, Dict[str, Any]]:
    """
...
def generate_report(results: Dict[str, Dict[str, Any]], output_file: str = "interface_syntax_report.json"):
    """
    ...
    report = {
        "summary": {
            ...
            "files_with_unfixed_errors": status_counts["fixes_failed"] + status_counts["no_changes"],
