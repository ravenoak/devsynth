...
try:
    ...
    print("Make sure enhanced_test_parser.py, enhanced_test_collector.py, and test_isolation_analyzer.py are in the same directory as this script.")
    sys.exit(1)
...
def generate_test_count_report() -> Dict[str, Any]:
    ...
    for category in TEST_CATEGORIES:
        ...
        pytest_tests = enhanced_test_collector.common_test_collector.collect_tests_with_pytest(directory)
        pytest_count = len(pytest_tests)
        
        # Collect tests using the enhanced collector
        enhanced_tests = enhanced_test_collector.collect_tests_by_category(category, use_cache=False)
        enhanced_count = len(enhanced_tests)
        
        # Calculate discrepancy
        discrepancy = abs(pytest_count - enhanced_count)
        discrepancy_percentage = (discrepancy / max(pytest_count, 1)) * 100 if pytest_count > 0 else 0
    ...
    report["total"]["discrepancy_percentage"] = (report["total"]["discrepancy"] / max(report["total"]["pytest_count"], 1)) * 100 if report["total"]["pytest_count"] > 0 else 0
...
def generate_marker_detection_report() -> Dict[str, Any]:
    ...
    for marker in ["fast", "medium", "slow"]:
        ...
        report["markers"][marker]["discrepancy_percentage"] = (discrepancy / max(pytest_count, 1)) * 100 if pytest_count > 0 else 0
    ...
    total_marked_tests = sum(report["markers"][marker]["enhanced_count"] for marker in ["fast", "medium", "slow"])
    
    report["total_tests"] = total_tests
    report["total_marked_tests"] = total_marked_tests
    report["marker_coverage_percentage"] = (total_marked_tests / max(total_tests, 1)) * 100 if total_tests > 0 else 0
...
def generate_test_isolation_report() -> Dict[str, Any]:
    ...
    for module in HIGH_PRIORITY_MODULES:
        ...
        for issue_type, count in module_results["issues_by_type"].items():
            report["overall"]["issues_by_type"][issue_type] = report["overall"]["issues_by_type"].get(issue_type, 0) + count
    
    # Calculate percentages
    report["overall"]["files_with_issues_percentage"] = (report["overall"]["files_with_issues"] / max(report["overall"]["total_files"], 1)) * 100 if report["overall"]["total_files"] > 0 else 0
...
def print_report_summary(report_type: str, report: Dict[str, Any]):
    ...
    if report_type == "Test Count":
        ...
        print(f"Total discrepancy: {report['total']['discrepancy']} ({report['total']['discrepancy_percentage']:.2f}%)")
        
        print("\nBy category:")
        for category, cat_report in report["categories"].items():
            print(f"  {category}: {cat_report['pytest_count']} (pytest), {cat_report['enhanced_count']} (enhanced), discrepancy: {cat_report['discrepancy']} ({cat_report['discrepancy_percentage']:.2f}%)")
        ...
        for marker in ["fast", "medium", "slow"]:
            ...
            print(f"  {marker}: {marker_report['enhanced_count']} (enhanced), {marker_report['pytest_count']} (pytest), discrepancy: {marker_report['discrepancy']} ({marker_report['discrepancy_percentage']:.2f}%)")
    
    elif report_type == "Test Isolation":
        print(f"Total files analyzed: {report['overall']['total_files']}")
        print(f"Files with issues: {report['overall']['files_with_issues']} ({report['overall']['files_with_issues_percentage']:.2f}%)")
...
def main():
    ...
    parser.add_argument(
        "--test-counts",
        action="store_true",
        help="Generate test count report"
    )
    parser.add_argument(
        "--marker-detection",
        action="store_true",
    ...
    parser.add_argument(
        "--test-isolation",
        action="store_true",
        help="Generate test isolation report"
    )
    parser.add_argument(
        "--output-dir",
        default=".",
    ...
    if args.all or args.test_counts:
        ...
        save_report(test_count_report, os.path.join(args.output_dir, "test_count_report.json"))
    ...
    if args.all or args.marker_detection:
        ...
        save_report(marker_detection_report, os.path.join(args.output_dir, "marker_detection_report.json"))
    ...
    if args.all or args.test_isolation:
        ...
        save_report(test_isolation_report, os.path.join(args.output_dir, "test_isolation_report.json"))
