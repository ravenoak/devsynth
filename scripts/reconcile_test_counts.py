...
try:
    from common_test_collector import collect_tests, collect_tests_by_category, get_test_counts, get_marker_counts
except ImportError:
...
def get_doc_test_counts() -> Dict[str, Dict[str, Any]]:
    ...
    if TEST_SUITE_OVERVIEW.exists():
        ...
        total_match = re.search(r'DevSynth test suite consists of \*\*([0-9,\*]+) total tests\*\*', content)
        total_count = int(clean_count_string(total_match.group(1))) if total_match else None
        
        # Extract category counts
        category_counts = {}
        category_table = re.search(r'\| Category \| Count \| Description \|\n\|.*\|\n((?:\|.*\|\n)+)', content)
        if category_table:
        ...
        speed_counts = {}
        speed_table = re.search(r'\| Speed \| Count \| Description \|\n\|.*\|\n((?:\|.*\|\n)+)', content)
        if speed_table:
        ...
        status_counts = {}
        status_table = re.search(r'\| Status \| Count \| Percentage \|\n\|.*\|\n((?:\|.*\|\n)+)', content)
        if status_table:
    ...
    if TEST_CATEGORIZATION_GUIDE.exists():
        ...
        # Extract total test count
        total_match = re.search(r'DevSynth has a large test suite with ([0-9,\*]+) tests', content)
        total_count = int(clean_count_string(total_match.group(1))) if total_match else None
        
        # Extract speed counts
        speed_counts = {}
        speed_table = re.search(r'\| Speed \| Count \| Percentage \| Target \|\n\|.*\|\n((?:\|.*\|\n)+)', content)
        if speed_table:
            for line in speed_table.group(1).strip().split('\n'):
                parts = line.split('|')
                if len(parts) >= 4:
    ...
    if PROJECT_STATUS.exists():
        ...
        # Extract test counts
        test_count_match = re.search(r'Resolving test failures \(approximately ([0-9,\*]+) failing tests out of ([0-9,\*]+) total tests', content)
        if test_count_match:
...
def analyze_discrepancies(
    ...
    actual_category_counts = {k: v for k, v in actual_counts["test_counts"].items() if k != "total"}
    actual_speed_counts = {
    ...
    if "test_suite_overview" in doc_counts:
        ...
        for category, actual_count in actual_category_counts.items():
            if category in doc_category_counts and doc_category_counts[category] != actual_count:
    ...
    if "schedule_file" in tracking_counts:
        ...
        actual_categorized = sum(actual_speed_counts[s] for s in ["fast", "medium", "slow"])
    ...
    if TEST_SUITE_OVERVIEW.exists():
        ...
        if category_table:
            ...
            for category, count in sorted(actual_category_counts.items()):
                if category != "total":
                    rows.append(f"| {category.capitalize()} | {count:,} | Tests for {category} components |")
    ...
    if TEST_CATEGORIZATION_GUIDE.exists():
        ...
        if speed_table:
            ...
            for speed, count in sorted(actual_speed_counts.items()):
                ...
                rows.append(f"| {speed.capitalize()} | {count:,} | {percentage:.1f}% | {target} |")
        ...
        categorized_count = sum(actual_speed_counts[s] for s in ["fast", "medium", "slow"])
        categorized_percentage = categorized_count / actual_total * 100 if actual_total > 0 else 0
...
def update_tracking_files(
    ...
    if SCHEDULE_FILE.exists():
        try:
            ...
            schedule_data["categorized_tests"] = sum(actual_speed_counts[s] for s in ["fast", "medium", "slow"])
...
def print_discrepancies(discrepancies: Dict[str, Any], verbose: bool = False):
    ...
    if discrepancies["total_counts"]:
        ...
        for disc in discrepancies["total_counts"]:
            print(f"  {disc['source']}: {disc['documented']:,} (Actual: {disc['actual']:,}, Difference: {disc['difference']:+,})")
    ...
    if discrepancies["category_counts"]:
        ...
        for disc in discrepancies["category_counts"]:
            print(f"  {disc['source']} - {disc['category']}: {disc['documented']:,} (Actual: {disc['actual']:,}, Difference: {disc['difference']:+,})")
    ...
    if discrepancies["speed_counts"]:
        ...
        for disc in discrepancies["speed_counts"]:
            print(f"  {disc['source']} - {disc['speed']}: {disc['documented']:,} (Actual: {disc['actual']:,}, Difference: {disc['difference']:+,})")
    ...
    if discrepancies["tracking_counts"]:
        ...
        for disc in discrepancies["tracking_counts"]:
            if "type" in disc:
                print(f"  {disc['source']} - {disc['type']}: {disc['tracked']:,} (Actual: {disc['actual']:,}, Difference: {disc['difference']:+,})")
            else:
                print(f"  {disc['source']} - {disc['speed']}: {disc['tracked']:,} (Actual: {disc['actual']:,}, Difference: {disc['difference']:+,})")
