...
def run_utils_coverage():
    ...
    try:
        ...
        try:
            ...
            utils_files = {k: v for k, v in coverage_data['files'].items() 
                          if k.endswith('/utils/__init__.py') or 
                             k.endswith('/utils/logging.py') or 
                             k.endswith('/utils/serialization.py')}
            ...
            print(f"Total: {total_covered}/{total_statements} = {overall_coverage:.1f}%")
