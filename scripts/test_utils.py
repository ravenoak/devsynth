...
def collect_tests(test_dir: str, speed: str = 'all', use_cache: bool = True) -> List[str]:
    """
    ...
    if category is None:
        if test_dir == 'tests':
            ...
            print(f"Warning: Using fallback test collection for non-standard directory: {test_dir}")
            return _collect_tests_fallback(test_dir, speed, use_cache)
    else:
        # Collecting tests for a specific category
        tests = common_test_collector.collect_tests_by_category(category, use_cache=use_cache)
    
    # Filter by speed if specified
    if speed != 'all':
        # Get tests with the specified speed marker
        tests_with_markers = common_test_collector.get_tests_with_markers([speed], use_cache=use_cache)
        ...
        if category == 'behavior' and not filtered_tests:
            print(f"No behavior tests found with {speed} marker. Returning all behavior tests...")
            return tests
...
def _collect_tests_fallback(test_dir: str, speed: str = 'all', use_cache: bool = True) -> List[str]:
    """
    ...
    if use_cache and os.path.exists(cache_file):
        try:
            ...
            if (datetime.now() - cache_time).total_seconds() < 3600:  # 1 hour
                print(f"Using cached test collection for {test_dir} (less than 1 hour old)")
...
def run_tests(tests: List[str], args, xml_report: Optional[str] = None) -> Tuple[int, Dict[str, bool]]:
    """
...
def run_tests_in_segments(tests: List[str], args, segment_size: int = 50, xml_report: Optional[str] = None) -> Tuple[int, Dict[str, bool]]:
    """
...
def clear_all_caches(collection: bool = True, timing: bool = True, history: bool = True, 
                  common_collector: bool = True, verbose: bool = False) -> None:
    """
    ...
    if collection:
        try:
            if os.path.exists(COLLECTION_CACHE_DIR):
                ...
                for cache_file in glob.glob(os.path.join(COLLECTION_CACHE_DIR, "*.json")):
...
def measure_test_time(test_path: str, timeout: int = 30, use_cache: bool = True) -> Tuple[float, bool, bool]:
    """
