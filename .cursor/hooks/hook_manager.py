...
class PatternDetector:
    ...
    def detect_error_patterns(
        self, error_message: str, context: HookContext
    ) -> list[dict[str, Any]]:
...
class RuleEvolutionManager:
    ...
    def suggest_rule_improvements(
        self, rule_name: str, feedback: dict[str, Any]
    ) -> list[str]:
        ...
        if len(content) > 2000:
            improvements.append(
                f"Consider breaking down large rule {rule_name} into smaller, focused rules"
            )
...
class HookManager:
    ...
    def __init__(self, cursor_dir: Path):
        ...
        self.evolution_manager = RuleEvolutionManager(
            self.cursor_dir / "rules", self.learning_dir
        )
    ...
    def _save_improvements_history(self):
        ...
        data = {
            "improvements": [
                improvement.to_dict() for improvement in self.improvements_history
            ],
    ...
    def _code_generation_hook(
        ...
        patterns = self.pattern_detector.detect_repetitive_patterns(
...
def on_code_generated(
    code_snippet: str, file_path: str = None
) -> list[ImprovementSuggestion]:
    """Trigger hook when code is generated."""
    return trigger_hook(
        HookEvent.CODE_GENERATED, code_snippet=code_snippet, file_path=file_path
    )


def on_error_occurred(
    error_message: str, context: str = None
) -> list[ImprovementSuggestion]:
...
def on_session_end(session_data: dict[str, Any]) -> list[ImprovementSuggestion]:
    ...
    return trigger_hook(
        HookEvent.SESSION_ENDED, metadata={"session_data": session_data}
    )
