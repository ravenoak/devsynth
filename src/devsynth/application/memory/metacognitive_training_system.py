...
class MetacognitiveTrainingSystem:
    ...
    def start_think_aloud_session(self, task_description: str) -> str:
        ...
        logger.info(
            f"Started think-aloud session {session_id} for task: {task_description}"
        )
        return session_id

    def record_verbalization(
        self, session_id: str, verbalization: str, context: dict[str, Any] = None
    ) -> None:
    ...
    def end_think_aloud_session(self, session_id: str) -> dict[str, Any]:
        ...
        summary = {
            ...
            "strategy_improvements": [
                i.description
                for i in insights
                if i.insight_type == "strategy_improvement"
            ],
        ...
        logger.info(
            f"Ended think-aloud session {session_id} with {len(insights)} insights"
        )
        return summary

    def _analyze_verbalization_for_insights(
        self, verbalization: str, context: dict[str, Any]
    ) -> None:
        ...
        if any(
            indicator.lower() in verbalization_lower
        ...
        # Check for error patterns
            ...
            indicator.lower() in verbalization_lower for indicator in error_indicators
        ):
            insight = MetacognitiveInsight(
    ...
    def _extract_insights_from_session(
        self, session: ThinkAloudSession
        ...
        if session.verbalizations:
            # Strategy analysis
            strategy_count = sum(
                1 for v in session.verbalizations if "strategy" in str(v).lower()
            )
            if strategy_count > 0:
    ...
    def _update_metacognitive_state_from_session(
        self, session: ThinkAloudSession, insights: list[MetacognitiveInsight]
    ) -> None:
        ...
        strategy_insights = [
            i for i in insights if i.insight_type == "strategy_improvement"
        ]
        if strategy_insights:
            strategy_avg_confidence = sum(
                i.confidence for i in strategy_insights
            ) / len(strategy_insights)
            self.current_state.strategy_evaluation["strategy_awareness"] = (
        ...
        if session.verbalizations:
            confidence_scores = [
                v.get("cognitive_state", {}).get("confidence", 0.5)
                for v in session.verbalizations
            ]
        ...
        # Record reflection
        self.current_state.last_reflection = (
            f"Session {session.session_id} completed with {len(insights)} insights"
        )
    ...
    def get_metacognitive_insights(
        self, insight_type: str | None = None
    ) -> list[MetacognitiveInsight]:
        """Get metacognitive insights, optionally filtered by type."""
        if insight_type:
            return [
                i for i in self.metacognitive_insights if i.insight_type == insight_type
            ]
    ...
    def apply_metacognitive_improvements(
        ...
        for insight in insights:
            if insight.applicability_score > 0.7:  # High applicability threshold
                ...
                improvements["implementation_suggestions"].extend(
                    insight.implementation_suggestions
                )

        logger.info(
            f"Applied {improvements['applied_insights']} metacognitive improvements"
        )
        return improvements
    ...
    def _apply_insight_improvement(self, insight: MetacognitiveInsight) -> None:
        ...
        if insight.insight_type == "strategy_improvement":
            # Update strategy evaluation
            self.current_state.strategy_evaluation[insight.description[:50]] = (
                insight.confidence
            )
    ...
    def generate_self_monitoring_report(self) -> dict[str, Any]:
        ...
        avg_duration = (
            sum(s.session_duration for s in self.training_sessions) / total_sessions
        )
        avg_verbalizations = (
            sum(len(s.verbalizations) for s in self.training_sessions) / total_sessions
        )
    ...
    def _calculate_improvement_trends(self) -> dict[str, Any]:
        ...
        older_insights = (
            self.metacognitive_insights[:-10]
        ...
        if older_insights and recent_insights:
            ...
            older_confidence = sum(i.confidence for i in older_insights) / len(
                older_insights
            )
            recent_confidence = sum(i.confidence for i in recent_insights) / len(
                recent_insights
            )
            ...
            older_applicability = sum(
                i.applicability_score for i in older_insights
            ) / len(older_insights)
            recent_applicability = sum(
                i.applicability_score for i in recent_insights
            ) / len(recent_insights)
    ...
    def _generate_self_improvement_recommendations(self) -> list[str]:
        ...
        if self.current_state.awareness_level < 0.6:
            recommendations.append(
                "Increase metacognitive awareness through more frequent think-aloud sessions"
            )

        # Check cognitive load
        if self.current_state.cognitive_load > 0.8:
            recommendations.append(
                "Reduce cognitive load through task decomposition and strategy optimization"
            )

        # Check strategy evaluation
        if not self.current_state.strategy_evaluation:
            recommendations.append(
                "Develop strategy evaluation framework through targeted training"
            )
        ...
        if improvement_areas:
            recommendations.append(
                f"Focus on improving: {', '.join(improvement_areas[:3])}"
            )
    ...
    def import_metacognitive_state(self, state_data: dict[str, Any]) -> None:
        ...
        self.current_state.self_monitoring_active = state_info.get(
            "self_monitoring_active", False
        )
        self.current_state.strategy_evaluation = state_info.get(
            "strategy_evaluation", {}
        )
        self.current_state.cognitive_load = state_info.get("cognitive_load", 0.0)
        self.current_state.confidence_calibration = state_info.get(
            "confidence_calibration", 0.5
        )
        ...
        self.cognitive_patterns = defaultdict(
            dict, state_data.get("cognitive_patterns", {})
        )

        logger.info(
            f"Imported metacognitive state: {len(self.training_sessions)} sessions, {len(self.metacognitive_insights)} insights"
        )

    def simulate_think_aloud_exercise(
        self, task_description: str, duration_minutes: int = 5
    ) -> dict[str, Any]:
        ...
        for i in range(verbalization_count):
            ...
            self.record_verbalization(
                session_id, verbalization, {"simulated": True, "step": i}
            )
