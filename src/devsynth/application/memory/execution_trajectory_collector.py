...
class ExecutionTrajectoryCollector:
    ...
    def __init__(self, sandbox_enabled: bool = True, max_execution_time: float = 30.0):
        ...
        logger.info(f"Execution trajectory collector initialized (sandbox: {sandbox_enabled}, timeout: {max_execution_time}s)")
    ...
    def _execute_in_sandbox(self, code: str) -> Optional[ExecutionTrace]:
    ...
    def _analyze_code_structure(self, tree: ast.AST) -> List[ExecutionStep]:
        ...
        return analyzer.steps if analyzer.steps else [
            ExecutionStep(
                step_number=1,
                line_number=1,
                code_line="Code analysis complete"
            )
        ]

    def extract_execution_patterns(self, traces: List[ExecutionTrace]) -> Dict[str, Any]:
        ...
        for trace in traces:
            ...
            for step in trace.execution_steps:
                for func_call in step.function_calls:
                    ...
                    patterns["function_call_patterns"][func_call]["contexts"].append({
                        ...
                        "code_context": step.code_line
                    })

                    if trace.execution_outcome:
                        patterns["function_call_patterns"][func_call]["outcomes"].append(
            ...
            for var_name, changes in trace.variable_changes.items():
                ...
                patterns["variable_usage_patterns"][var_name]["change_count"] += len(changes)
    ...
    def create_memetic_units_from_trajectories(self, traces: List[ExecutionTrace]) -> List[MemeticUnit]:
        ...
        logger.info(f"Created {len(units)} Memetic Units from {len(traces)} execution traces")
        return units
    ...
    def batch_collect_trajectories(self, code_batch: List[str], batch_size: int = 5) -> List[ExecutionTrace]:
    ...
    def get_execution_insights(self, traces: List[ExecutionTrace]) -> Dict[str, Any]:
        ...
        insights = {
            ...
            "average_execution_time": sum(t.execution_time for t in traces) / len(traces),
        ...
        for category in perf_categories:
            count = sum(1 for t in traces if self._categorize_performance(t.execution_time) == category)
        ...
        for trace in traces:
            if trace.execution_error:
                error_type = self._classify_error_type(trace.execution_error)
                insights["error_distribution"][error_type] = insights["error_distribution"].get(error_type, 0) + 1
