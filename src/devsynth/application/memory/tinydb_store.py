...
class TinyDBStore(MemoryStore):
    ...
    def __init__(self, base_path: str):
        ...
        self.db = TinyDB(self.db_file, storage=CachingMiddleware(JSONStorage))
    ...
    def search(self, query: Dict[str, Any]) -> List[MemoryItem]:
        ...
        try:
            ...
            for item_data in all_items:
                ...
                for key, value in query.items():
                    if key == "memory_type" and isinstance(value, MemoryType):
                        ...
                        field = key.split(".", 1)[1]
                        if "metadata" not in item_data or field not in item_data["metadata"] or item_data["metadata"][field] != value:
                            match = False
                            break
                    else:
            ...
            # Update token count
            if filtered_items:
                token_count = sum(self._count_tokens(str(item)) for item in filtered_items)
                self.token_count += token_count
