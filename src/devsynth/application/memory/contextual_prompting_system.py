...
class EnvironmentalConstraint:
    ...
    constraint_id: str
    constraint_type: (
        str  # "resource", "security", "performance", "quality", "compliance"
    )
    description: str
...
class ContextualPromptingSystem:
    ...
    def _initialize_default_framework(self) -> None:
        ...
        self.environmental_constraints["performance_response_time"] = (
    ...
    def create_contextual_prompt(
        ...
        validation_criteria = self._define_validation_criteria(
            relevant_directives, relevant_constraints
        )
    ...
    def _select_relevant_directives(
        self, context: dict[str, Any]
    ) -> list[BehavioralDirective]:
    ...
    def _select_relevant_constraints(
        self, context: dict[str, Any]
    ) -> list[EnvironmentalConstraint]:
    ...
    def _define_validation_criteria(
        self,
        directives: list[BehavioralDirective],
        constraints: list[EnvironmentalConstraint],
    ) -> list[str]:
        ...
        for directive in directives:
            criteria.append(
                f"Response must follow {directive.category} directive: {directive.description}"
            )

        # Constraint-based criteria
        for constraint in constraints:
            criteria.append(
                f"Response must comply with {constraint.constraint_type} constraint: {constraint.description}"
            )
    ...
    def engineer_contextual_prompt(
        self, contextual_prompt: ContextualPrompt
        ...
        if contextual_prompt.behavioral_directives:
            ...
            for directive in contextual_prompt.behavioral_directives:
                prompt_parts.append(
                    f"- {directive.category.upper()}: {directive.description}"
                )
        ...
        if contextual_prompt.environmental_constraints:
            ...
            for constraint in contextual_prompt.environmental_constraints:
                prompt_parts.append(
                    f"- {constraint.constraint_type.upper()}: {constraint.description}"
                )
        ...
        directive_coverage = (
            len(contextual_prompt.behavioral_directives)
            / len(self.behavioral_directives)
            if self.behavioral_directives
            else 0.0
        )
        constraint_compliance = (
            len(contextual_prompt.environmental_constraints)
            / len(self.environmental_constraints)
            if self.environmental_constraints
            else 0.0
        )
        ...
        estimated_effectiveness = (
            directive_coverage * 0.3
            + constraint_compliance * 0.3
            + context_relevance * 0.4
        )

        # Generate suggestions
        suggestions = self._generate_engineering_suggestions(
            contextual_prompt, estimated_effectiveness
        )
    ...
    def _generate_engineering_suggestions(
        self, contextual_prompt: ContextualPrompt, effectiveness: float
    ) -> list[str]:
        ...
        if len(contextual_prompt.environmental_constraints) == 0:
            suggestions.append(
                "Add environmental constraints to ensure operational compliance"
            )
    ...
    def get_prompt_performance_analytics(self) -> dict[str, Any]:
        ...
        avg_directive_coverage = (
            sum(p["directive_coverage"] for p in self.prompt_performance_history)
            / total_prompts
        )
        avg_constraint_compliance = (
            sum(p["constraint_compliance"] for p in self.prompt_performance_history)
            / total_prompts
        )
        avg_context_relevance = (
            sum(p["context_relevance"] for p in self.prompt_performance_history)
            / total_prompts
        )
        avg_effectiveness = (
            sum(p["estimated_effectiveness"] for p in self.prompt_performance_history)
            / total_prompts
        )

        # Identify trends
        recent_performance = (
            self.prompt_performance_history[-10:]
            if len(self.prompt_performance_history) >= 10
            else self.prompt_performance_history
        )
        recent_effectiveness = sum(
            p["estimated_effectiveness"] for p in recent_performance
        ) / len(recent_performance)

        trend = (
            "improving"
            if recent_effectiveness > avg_effectiveness
            else "declining" if recent_effectiveness < avg_effectiveness else "stable"
        )
    ...
    def _generate_system_improvements(self) -> list[str]:
        ...
        low_performers = [
            ...
            if p["estimated_effectiveness"] < 0.6
        ]
        ...
        missing_categories = (
            {"reasoning", "communication", "decision_making", "error_handling"}
            - used_categories
        )

        if missing_categories:
            suggestions.append(
                f"Consider adding directives for missing categories: {', '.join(missing_categories)}"
            )
    ...
    def validate_prompt_effectiveness(
        self, engineered_prompt: str, expected_outcomes: list[str]
    ) -> dict[str, Any]:
        ...
        present_components = sum(
            1 for component in expected_components if component in engineered_prompt
        )
    ...
    def import_prompt_framework(self, framework_data: dict[str, Any]) -> None:
        ...
        logger.info(
            f"Imported framework: {len(directives_data)} directives, {len(constraints_data)} constraints"
        )
