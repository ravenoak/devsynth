    ...
    try:
        ...
        for line in output.split("\n"):
            if line == "--END--":
                ...
                current_commit["date"] = datetime.fromtimestamp(int(line)).strftime("%Y-%m-%d %H:%M:%S")
...
def analyze_commit(commit: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for file in commit["files"]:
        path = file["path"]
        if path.endswith((".py", ".js", ".ts", ".java", ".rb", ".go", ".rs", ".cpp", ".c", ".h", ".hpp")):
            if "test" in path.lower() or "spec" in path.lower() or path.startswith("tests/"):
    ...
    if has_tests and has_code:
        ...
        if any(term in subject_lower for term in ["test first", "tdd", "bdd", "test-driven", "behavior-driven"]):
...
def calculate_metrics(commits: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for commit in analyzed_commits:
        ...
        if commit["is_test_first"]:
            metrics["test_first_by_date"][date] = metrics["test_first_by_date"].get(date, 0) + 1
        elif commit["is_code_first"]:
            metrics["code_first_by_date"][date] = metrics["code_first_by_date"].get(date, 0) + 1
        elif commit["is_mixed"]:
            metrics["mixed_by_date"][date] = metrics["mixed_by_date"].get(date, 0) + 1
    
    # Calculate ratios
    if metrics["total_commits"] > 0:
        metrics["test_first_ratio"] = metrics["test_first_commits"] / metrics["total_commits"]
        metrics["code_first_ratio"] = metrics["code_first_commits"] / metrics["total_commits"]
        metrics["mixed_ratio"] = metrics["mixed_commits"] / metrics["total_commits"]
    
    if metrics["code_files_count"] > 0:
        metrics["test_to_code_ratio"] = metrics["test_files_count"] / metrics["code_files_count"]
    
    # Get recent commits of each type (up to 5)
    metrics["recent_test_first_commits"] = [
        commit for commit in analyzed_commits
        if commit["is_test_first"]
    ][:5]
    
    metrics["recent_code_first_commits"] = [
        commit for commit in analyzed_commits
        if commit["is_code_first"]
    ][:5]
...
def generate_metrics_report(metrics: Dict[str, Any]) -> str:
    ...
    if test_first_ratio >= 0.8:
        ...
        report.append("The project generally follows test-first development, but there's room for improvement.")
    elif test_first_ratio >= 0.4:
        report.append("⚠️ **Moderate test-first development practices.**")
        report.append("The project sometimes follows test-first development, but should improve consistency.")
    else:
        report.append("❌ **Poor test-first development practices.**")
        report.append("The project rarely follows test-first development. Consider adopting TDD/BDD practices.")
    ...
    if test_first_ratio < 0.8:
        ...
        report.append("2. **Use the `devsynth test` command to generate test cases before writing code.**")
        report.append("3. **Consider using BDD with Gherkin feature files to define behavior before implementation.**")
        report.append("4. **Review the TDD/BDD approach documentation in `docs/developer_guides/tdd_bdd_approach.md`.**")
    
    report.extend([
        "",
        "## Recent Test-First Commits",
        ""
    ])
    
    if metrics["recent_test_first_commits"]:
        for commit in metrics["recent_test_first_commits"]:
            report.append(f"- **{commit['date']}**: {commit['subject']} ({commit['hash'][:7]})")
    else:
        report.append("No test-first commits found in the analyzed period.")
    
    report.extend([
        "",
        "## Recent Code-First Commits",
        ""
    ])
