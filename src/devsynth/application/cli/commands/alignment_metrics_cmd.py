...
from devsynth.application.cli.commands.align_cmd import (
    ...
    REQUIREMENT_FILES, SPECIFICATION_FILES, TEST_FILES, CODE_FILES
)
...
def calculate_alignment_coverage(files: List[str]) -> Dict:
    ...
    metrics["total_code_files"] = len([f for f in files if is_file_of_type(f, CODE_FILES) and f.endswith('.py')])
    ...
    if metrics["total_requirements"] > 0:
        metrics["requirements_coverage"] = (metrics["requirements_with_specs"] / metrics["total_requirements"]) * 100
    else:
        metrics["requirements_coverage"] = 0
    
    if metrics["total_specs"] > 0:
        metrics["specs_coverage"] = (metrics["specs_with_tests"] / metrics["total_specs"]) * 100
    else:
        metrics["specs_coverage"] = 0
    
    if metrics["total_tests"] > 0:
        metrics["tests_coverage"] = (metrics["tests_with_code"] / metrics["total_tests"]) * 100
    else:
        metrics["tests_coverage"] = 0
    
    if metrics["total_code_files"] > 0:
        metrics["code_coverage"] = (metrics["code_with_tests"] / metrics["total_code_files"]) * 100
    else:
...
def display_metrics(metrics: Dict, historical_metrics: List[Dict]):
    ...
    if len(historical_metrics) > 1:
        ...
        # Calculate changes
        coverage_change = metrics["overall_coverage"] - previous_metrics["overall_coverage"]
...
def generate_metrics_report(metrics: Dict, historical_metrics: List[Dict], output_file: str):
    ...
    try:
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write("|--------|-------|----------|\n")
            f.write(f"| Requirements with Specifications | {metrics['requirements_with_specs']} / {metrics['total_requirements']} | {metrics['requirements_coverage']:.1f}% |\n")
            f.write(f"| Specifications with Tests | {metrics['specs_with_tests']} / {metrics['total_specs']} | {metrics['specs_coverage']:.1f}% |\n")
            f.write(f"| Tests with Code | {metrics['tests_with_code']} / {metrics['total_tests']} | {metrics['tests_coverage']:.1f}% |\n")
            f.write(f"| Code with Tests | {metrics['code_with_tests']} / {metrics['total_code_files']} | {metrics['code_coverage']:.1f}% |\n")
