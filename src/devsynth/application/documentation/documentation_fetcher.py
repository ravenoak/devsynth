...
class PyPIDocumentationSource(DocumentationSource):
    ...
    def fetch_documentation(self, library: str, version: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Fetching documentation for {library} {version} from PyPI/ReadTheDocs")
    ...
    def _fetch_from_readthedocs(self, library: str, version: str) -> List[Dict[str, Any]]:
        ...
        for url in urls:
            try:
                ...
                if response.status_code == 200:
                    return self._parse_html_documentation(response.text, url, library, version)
            except Exception as e:
    ...
    def _fetch_from_pypi(self, library: str, version: str) -> List[Dict[str, Any]]:
        ...
        try:
            response = requests.get(f"https://pypi.org/pypi/{library}/{version}/json", timeout=10)
            if response.status_code == 200:
                data = response.json()
                ...
                if doc_url:
                    try:
                        ...
                        if doc_response.status_code == 200:
                            return self._parse_html_documentation(doc_response.text, doc_url, library, version)
                    except Exception as e:
                        logger.debug(f"Error fetching from documentation URL {doc_url}: {str(e)}")
            ...
            logger.warning(f"Error fetching from PyPI for {library} {version}: {str(e)}")
            return []
    ...
    def _extract_docstrings(self, library: str, version: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            script_path = os.path.join(self.cache_dir, f"extract_docstrings_{library}_{version}.py")
            ...
            if result.returncode == 0 and result.stdout:
                try:
                    ...
                    return self._convert_docstrings_to_chunks(docstrings, library, version)
                except json.JSONDecodeError:
                    logger.error(f"Error parsing docstring output for {library} {version}")
            
            return []
        except Exception as e:
            logger.error(f"Error extracting docstrings for {library} {version}: {str(e)}")
            return []
        finally:
            # Clean up
            if os.path.exists(venv_dir):
    ...
    def _parse_html_documentation(self, html: str, base_url: str, library: str, version: str) -> List[Dict[str, Any]]:
    ...
    def _parse_markdown_documentation(self, markdown: str, library: str, version: str) -> List[Dict[str, Any]]:
    ...
    def _convert_docstrings_to_chunks(self, docstrings: Dict[str, Any], library: str, version: str) -> List[Dict[str, Any]]:
    ...
    def fetch_documentation(self, library: str, version: str) -> List[Dict[str, Any]]:
        ...
        for source in self.sources:
            if source.supports_library(library):
                ...
                if chunks:
                    logger.info(f"Fetched {len(chunks)} documentation chunks for {library} {version}")
                    return chunks
