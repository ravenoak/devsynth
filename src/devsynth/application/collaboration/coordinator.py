...
from ...domain.models.wsde import WSDETeam, WSDE
    ...
    def _handle_team_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for agent in self.agents:
            try:
                ...
                logger.info(f"Agent {agent.name} ({agent.current_role}) proposed a solution")
            except Exception as e:
                logger.warning(f"Agent {agent.name} failed to propose a solution: {str(e)}")
        ...
        if not solutions:
            try:
                ...
                logger.error(f"Primus {primus.name} failed to create a solution: {str(e)}")
                raise AgentExecutionError(
        ...
        critic = next((agent for agent in self.agents if agent.agent_type == "critic"), None)
        if critic and solutions:
            try:
                # Apply dialectical reasoning to the solutions
                dialectical_result = self.team.apply_enhanced_dialectical_reasoning(task, critic)
        ...
        design_agent = next((agent for agent in self.agents if agent.agent_type == "planner"), None)
        work_agent = next((agent for agent in self.agents if agent.agent_type == "code"), None)
        supervision_agent = next((agent for agent in self.agents if agent.agent_type == "test"), None)
        evaluation_agent = next((agent for agent in self.agents if agent.agent_type == "validation"), None)
        ...
        return {
            "team_result": {
                ...
                "supervision": supervision_agent.process.return_value if supervision_agent else {},
                "evaluation": evaluation_agent.process.return_value if evaluation_agent else {},
