...
class SecurityAuditReport:
    ...
    timestamp: datetime
    project_path: str
    bandit_report: Optional[BanditReport]
    safety_report: Optional[SafetyReport]
    custom_report: Optional[CustomSecurityReport]
    overall_score: float
    risk_level: str
    recommendations: List[str]
...
class SecurityAuditSystem:
    ...
    def run_bandit_analysis(self, target_path: str = ".") -> BanditReport:
        ...
        try:
            ...
            # Calculate metrics
            metrics = {
                "total_files": bandit_data.get("metrics", {}).get("_totals", {}).get("files", 0),
                "total_lines": bandit_data.get("metrics", {}).get("_totals", {}).get("loc", 0),
                "total_nosec": bandit_data.get("metrics", {}).get("_totals", {}).get("nosec", 0),
                "skipped_tests": bandit_data.get("metrics", {}).get("_totals", {}).get("skipped_tests", 0)
            }
    ...
    def run_safety_scan(self, target_path: str = ".") -> SafetyReport:
        ...
        try:
            ...
            for vuln in vulnerabilities:
                severity = vuln.get("severity", "unknown")
                vulnerabilities_by_severity[severity] = vulnerabilities_by_severity.get(severity, 0) + 1
                affected_packages.add(vuln.get("package", ""))

            # Generate remediation advice
            remediation_advice = []
            if vulnerabilities_by_severity.get("high", 0) > 0:
                remediation_advice.append("Immediately update packages with high-severity vulnerabilities")

            if len(affected_packages) > 5:
                remediation_advice.append("Consider reviewing dependency management strategy")
    ...
    def run_custom_security_checks(self, target_path: str = ".") -> CustomSecurityReport:
        """
    ...
    def _determine_risk_level(
        ...
        if safety_report:
            ...
            high_severity_issues += safety_report.vulnerabilities_by_severity.get("high", 0)
    ...
    def _generate_security_recommendations(self, check_results: Dict[str, Any]) -> List[str]:
        ...
        for check_name, result in check_results.items():
            ...
            if check_name == "_check_hardcoded_secrets" and issues:
                recommendations.append("Remove hardcoded secrets and use environment variables or secure vaults")

            if check_name == "_check_insecure_random" and issues:
                recommendations.append("Use cryptographically secure random number generation (secrets module)")

            if check_name == "_check_debug_endpoints" and issues:
                recommendations.append("Disable debug mode and remove debug endpoints in production")
            ...
            if check_name == "_check_xss_patterns" and issues:
                recommendations.append("Sanitize user input and use secure templating to prevent XSS")

            if check_name == "_check_insecure_file_operations" and issues:
                recommendations.append("Validate file paths and use secure file operations")
    ...
    def _generate_comprehensive_recommendations(
        ...
        if bandit_report and bandit_report.total_issues > 0:
            if bandit_report.issues_by_severity.get("high", 0) > 0:
                recommendations.append("Immediately address high-severity security issues found by Bandit")

            if bandit_report.total_issues > 10:
                recommendations.append("Consider implementing a security-focused code review process")

        # Safety recommendations
        if safety_report and safety_report.total_vulnerabilities > 0:
            if safety_report.vulnerabilities_by_severity.get("high", 0) > 0:
                recommendations.append("Urgently update packages with high-severity vulnerabilities")

            recommendations.append("Regularly scan and update dependencies for security patches")

        # Custom check recommendations
        if custom_report:
            recommendations.extend(self._generate_security_recommendations(custom_report.check_results))

        # General recommendations
        if not recommendations:
            recommendations.append("Security scan completed successfully - no critical issues found")
        else:
        ...
        try:
            ...
            self.memory_port.store(
                ...
                data={
                    ...
                    "risk_level": report.risk_level,
                    "total_issues": (
                        (report.bandit_report.total_issues if report.bandit_report else 0) +
                        (report.safety_report.total_vulnerabilities if report.safety_report else 0) +
                        (report.custom_report.issues_found if report.custom_report else 0)
                    ),
                    "recommendations": report.recommendations
                },
                metadata={
                ...
                with open(audit_file, 'r') as f:
                    ...
                    history.append({
                        ...
                        "total_issues": (
                            ...
                            data.get("safety_report", {}).get("total_vulnerabilities", 0) +
    ...
    def get_security_trends(self) -> Dict[str, Any]:
        ...
        if len(history) < 2:
            return {"trend": "insufficient_data", "message": "Need at least 2 audit results for trend analysis"}
        ...
        issue_trend = "improving" if issues[0] < issues[-1] else "declining" if issues[0] > issues[-1] else "stable"
