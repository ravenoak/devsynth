...
class EnhancedEDRRCoordinator(EDRRCoordinator):
    ...
    def _execute_peer_review(
        ...
        if not self.enable_peer_review or phase.name not in self.peer_review_phases:
            logger.debug(f"Peer review skipped for phase {phase.name}: not enabled or not in peer_review_phases")
            return {}
        ...
        try:
            ...
            peer_review_result = run_peer_review(
                ...
                author=author,
                reviewers=reviewers,
                send_message=self.wsde_team.send_message if hasattr(self.wsde_team, "send_message") else None,
            ...
            logger.info(f"Peer review completed for phase {phase.name} with status: {peer_review_result.get('status', 'unknown')}")
            return peer_review_result
            
        except Exception as e:
            logger.error(f"Error executing peer review for phase {phase.name}: {str(e)}")
    ...
    def execute_current_phase(self, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if "error" in results:
            logger.warning(f"Skipping peer review due to phase execution error: {results.get('error')}")
            return results
            
        # Execute peer review if enabled for the current phase
        if self.enable_peer_review and self.current_phase.name in self.peer_review_phases:
            ...
            # Execute the peer review
            peer_review_result = self._execute_peer_review(work_product, self.current_phase)
            ...
            if peer_review_result:
                ...
                if "quality_score" in peer_review_result:
                    ...
                    updated_quality = (current_quality * 0.7) + (peer_review_quality * 0.3)
                    results["quality_score"] = updated_quality
                    
                    logger.info(f"Updated quality score for phase {self.current_phase.name}: {updated_quality}")
                    
                # If the peer review suggests revisions, mark the phase as needing more work
                if peer_review_result.get("status") in ["revision_requested", "revision_suggested"]:
                    results["needs_more_work"] = True
                    results["revision_reason"] = peer_review_result.get("reasons", ["Peer review suggested revisions"])
                    
                    logger.info(f"Phase {self.current_phase.name} marked as needing more work based on peer review")
