...
class PromptVariant:
    ...
    def performance_score(self) -> float:
        ...
        return (success_weight * self.success_rate) + (feedback_weight * self.average_feedback_score)
...
class PromptAutoTuner:
    ...
    def __init__(self, storage_path: str = None):
        ...
        self.selection_strategy = "performance"  # "performance", "exploration", "random"
        self.exploration_rate = 0.2  # Probability of selecting a non-optimal variant for exploration
    ...
    def select_variant(self, template_id: str) -> PromptVariant:
        ...
        """
        if template_id not in self.prompt_variants:
            raise PromptAutoTuningError(f"Template '{template_id}' not registered for auto-tuning")
        ...
        if self.selection_strategy == "performance":
            ...
            # Otherwise, select the best variant most of the time
                ...
                sorted_variants = sorted(variants, key=lambda v: v.performance_score, reverse=True)
        ...
        selected.record_usage()
        logger.info(f"Selected prompt variant '{selected.variant_id}' for template '{template_id}'")

        return selected

    def record_feedback(self, template_id: str, variant_id: str, success: bool = None, 
        ...
        if variant is None:
            raise PromptAutoTuningError(f"Variant '{variant_id}' not found for template '{template_id}'")

        # Record the feedback
        variant.record_usage(success, feedback_score)

        logger.info(f"Recorded feedback for prompt variant '{variant_id}': success={success}, score={feedback_score}")
    ...
    def _generate_variants_if_needed(self, template_id: str) -> None:
        ...
        if len(variants) < 5 and any(v.usage_count >= 5 for v in variants):
            ...
            logger.info(f"Generated new prompt variant '{new_variant.variant_id}' for template '{template_id}'")
        ...
        if len(variants) >= 2 and sum(1 for v in variants if v.usage_count >= 5) >= 2:
            ...
            if len(sorted_variants) >= 2:
                new_variant = self._recombine_variants(sorted_variants[0], sorted_variants[1])
                self.prompt_variants[template_id].append(new_variant)

                # Record initial usage to ensure the variant has usage data
                new_variant.record_usage()

                logger.info(f"Generated new prompt variant '{new_variant.variant_id}' through recombination for template '{template_id}'")
    ...
    def _mutate_variant(self, variant: PromptVariant) -> PromptVariant:
        ...
        if "Focus on: security" not in template and random.random() < 0.7:
            if "Focus on:" in template:
                template = template.replace("Focus on:", "Focus on: security, performance,")
            else:
    ...
    def _recombine_variants(self, variant1: PromptVariant, variant2: PromptVariant) -> PromptVariant:
        """
    ...
    def _change_tone(self, template: str) -> str:
        ...
        tones = {
            "formal": ["Please", "kindly", "would you", "I request", "It would be appreciated if"],
            "direct": ["Do", "Create", "Write", "Analyze", "Explain"],
            "collaborative": ["Let's", "We should", "Together we can", "We need to", "Our goal is"]
        }
        ...
        replaced = False
        for word in ["Please", "kindly", "Do", "Create", "Write", "Analyze", "Explain", "Let's", "We should"]:
            if word in template:
    ...
    def _adjust_emphasis(self, template: str) -> str:
        ...
        if not emphasized and len(words) > 0:
            ...
            important_indices = [
                ...
                if len(word) > 3 and word.lower() not in ["this", "that", "with", "from", "have", "your"]
            ]
    ...
    def _load_variants(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {sum(len(variants) for variants in self.prompt_variants.values())} prompt variants from storage")
