...
class PromptEfficacyTracker:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        """
        self.storage_path = storage_path or os.path.join(os.getcwd(), ".devsynth", "prompts", "efficacy")
        ...
        logger.info(f"Prompt efficacy tracker initialized with storage path: {self.storage_path}")
    ...
    def track_usage(self, template_name: str, version_id: str) -> str:
        ...
        logger.debug(f"Tracked usage of template '{template_name}' version '{version_id}' with ID {tracking_id}")
        return tracking_id
    
    def record_outcome(self, tracking_id: str, success: bool, 
            ...
            for version_id, usages in versions.items():
                for usage in usages:
                    if usage["tracking_id"] == tracking_id:
                        ...
                        self._save_data()
                        logger.debug(f"Recorded outcome for tracking ID {tracking_id}: success={success}")
                        return True
        
        logger.warning(f"No usage found for tracking ID {tracking_id}")
        return False
    
    def get_efficacy_metrics(self, template_name: str, 
        ...
        if version_id:
            ...
            if version_id not in versions:
                return {"error": f"No data for version '{version_id}' of template '{template_name}'"}
            
            return self._calculate_metrics_for_version(template_name, version_id, versions[version_id])
        else:
            # Get metrics for all versions
            all_metrics = {}
            for vid, usages in versions.items():
                all_metrics[vid] = self._calculate_metrics_for_version(template_name, vid, usages)
    ...
    def get_optimization_recommendations(self, template_name: str) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_metrics_for_version(self, template_name: str, version_id: str, 
        ...
        successful_usages = [u for u in usages_with_outcome if u.get("outcome", {}).get("success", False)]
        ...
        metrics = {
            ...
            "success_rate": len(successful_usages) / len(usages_with_outcome) if usages_with_outcome else 0,
    ...
    def _compare_versions(self, version_metrics: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for version_id, metrics in version_metrics.items():
            ...
            if success_rate > best_success_rate and metrics.get("usages_with_outcome", 0) >= 5:
