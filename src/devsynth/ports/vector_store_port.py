...
class VectorStorePort:
    ...
    def store_vector(self, content: Any, embedding: List[float], metadata: Dict[str, Any] = None) -> str:
        """Store a vector in the vector store and return its ID."""
        vector = MemoryVector(id="", content=content, embedding=embedding, metadata=metadata)
