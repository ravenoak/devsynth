...
def enhanced_calculate_expertise_score(
    ...
    if hasattr(agent, "experience_level") and isinstance(
        agent.experience_level, (int, float)
    ):
    ...
    if hasattr(agent, "performance_history") and isinstance(
    ...
    if matched_expertise:
        logger.debug(
            f"Agent {agent.name} expertise matches: {', '.join(matched_expertise)}"
        )
    ...
    for expertise in agent_expertise:
        ...
        for keyword in phase_keywords:
            ...
            if expertise_lower == keyword_lower:
                phase_score += 3.0
                matched_phase_expertise.append(
                    f"{expertise} (exact match with {keyword})"
                )
            ...
            # Word-level partial match
                ...
                if common_words:
                    ...
                    matched_phase_expertise.append(
                        f"{expertise} (word match with {keyword}: {', '.join(common_words)})"
                    )

    # Log the phase expertise matching for debugging
    if matched_phase_expertise:
        logger.debug(
            f"Agent {agent.name} phase expertise matches: {', '.join(matched_phase_expertise)}"
        )
...
def enhanced_select_primus_by_expertise(
    ...
    self.logger.info(
        f"Selecting primus for task - Domain: {task_domain}, Type: {task_type}"
    )
    ...
    # Log the selection
    self.logger.info(
        f"Selected {best_agent.name} as primus with expertise score {expertise_scores[best_index]}"
    )
...
def dynamic_role_reassignment_enhanced(
            ...
            if hasattr(agent, "expertise") and agent.expertise:
                for expertise in agent.expertise:
                    for keyword in keywords:
                        if (
                            keyword.lower() in expertise.lower()
                            or expertise.lower() in keyword.lower()
                        ):
                            score += 1.0
    ...
    for role in ["worker", "supervisor", "designer", "evaluator"]:
        ...
        if best_agent:
            ...
            self.logger.debug(
                f"Assigned {best_agent.name} to {role} role with score {best_score}"
            )
    ...
    remaining_agents = [
        agent for agent in available_agents if agent not in assigned_agents
    ]
    ...
    for i, role in enumerate(remaining_roles):
        if i < len(remaining_agents):
            ...
            self.logger.debug(
                f"Assigned {remaining_agents[i].name} to {role} role (default assignment)"
            )
    ...
    self.logger.info(
        f"Dynamic role assignments for task {task.get('id', 'unknown')}: {role_assignments}"
    )
